...
argument-hint: "[idea|audit|memorize|changelog|schedule|health|search] or leave blank for interactive menu"
---
...
- If args start with `schedule` or `schedules` or `cron` → extract the rest as the goal hint, set `$ARGUMENTS` to that hint, then jump to **Step 2e: Create Schedule**.
...
- **Create Schedule** — "Describe it in plain language — SynaBun infers the rest and creates the template + schedule (and group) in one shot"
...
## Step 2e: Create Schedule (Conversational)

**Directly**: Load/read the file at `$SKILL_DIR/modules/schedule.md`. Follow the instructions in that file exactly, passing through the `$ARGUMENTS` value (the goal hint, if any). That file is your complete schedule-creation procedure — execute it fully.
