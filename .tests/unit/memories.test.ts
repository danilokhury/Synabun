...
describe('memories — zero embedding cost', () => {
  it('stats action: 0 embedding calls', async () => {
  ...
  it('stats action: count calls + scroll for project breakdown', async () => {
    await handleMemories({ action: 'stats' });
    const counts = getDbCallsByMethod('count');
    ...
    console.log(`\n  memories stats: ${counts.length} count calls + ${scrolls.length} scroll calls = ${getDbCalls().length} total`);
  });

  it('recent action: 0 embedding calls, 1 scroll', async () => {
    await handleMemories({ action: 'recent', limit: 10 });
    expect(getEmbeddingCalls()).toHaveLength(0);
    expect(getDbCallsByMethod('scroll')).toHaveLength(1);
    expect(getDbCallsByMethod('count')).toHaveLength(0);
  });

  it('by-category action: 0 embedding calls, 1 scroll', async () => {
    await handleMemories({ action: 'by-category', category: 'architecture', limit: 5 });
    expect(getEmbeddingCalls()).toHaveLength(0);
    expect(getDbCallsByMethod('scroll')).toHaveLength(1);
  });

  it('by-project action: 0 embedding calls, 1 scroll', async () => {
    await handleMemories({ action: 'by-project', project: 'test-project', limit: 5 });
    expect(getEmbeddingCalls()).toHaveLength(0);
    expect(getDbCallsByMethod('scroll')).toHaveLength(1);
  });

  it('stats is the most DB-intensive action', async () => {
    // Run stats
    await handleMemories({ action: 'stats' });
    const statsOps = getDbCalls().length;
