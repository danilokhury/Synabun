<!--
  Thank you for your interest in SynaBun!

  However, this repository does not accept pull requests.
  SynaBun is maintained solely by its author.

  This PR will be closed without review.
-->

> **Pull requests are not accepted for this repository.**
>
> SynaBun is maintained solely by its author. We do not accept external code contributions.
>
> **Instead, please:**
> - Found a bug? [Open a bug report](https://github.com/danilokhury/Synabun/issues/new?template=bug_report.md)
> - Have a feature idea? [Open a feature request](https://github.com/danilokhury/Synabun/issues/new?template=feature_request.md)
>
> You are welcome to **fork** this project and modify it for your own use under the [Apache 2.0 license](../LICENSE). If you fork, please use a different name (see the [Trademark Notice](../CONTRIBUTING.md#trademark-notice)).
>
> See [CONTRIBUTING.md](../CONTRIBUTING.md) for details.
