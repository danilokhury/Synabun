...
**GET /api/memories** — Returns ALL memories with computed graph edges for 3D visualization. Fetches all records from SQLite, then computes edges between memories based on cosine similarity (>0.65 threshold), shared related files, same parent category, and explicit `related_memory_ids`. Response: `{ memories: [...], edges: [...] }`.

**POST /api/search** — Semantic search. Request body: `{ query, category?, project?, tags?, limit?, min_importance? }`. Embeds the query text, searches SQLite with filters, returns ranked results with scores.

**GET /api/stats** — Database statistics. Returns memory count and database status. Quick health check endpoint.
...
**DELETE /api/memory/:id** — Delete a memory from SQLite by UUID. Returns success/failure status.
