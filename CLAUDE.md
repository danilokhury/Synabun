...
| Stop | `stop.mjs` | Blocks only for: compaction, loops, task memory (3+ edits), unstored plans. |
...
**Non-blocking nudges (prompt-submit only):** User learning, recall suggestions. These never block via stop hook.
