...
import { CLI_PROFILES, getModelsForProfile, modelSelectorValue } from './agent-runtime-options.js';
...
export const TUTORIAL_STEPS = [
  ...
  // ── Step 3: Model Picker ──────────────────────────────────
  {
    ...
    render({ svg, dom, seq, vw, vh, nextStep, prevStep, createButton }) {
      ...
      const cliModels = getModelsForProfile(chosenCli);
      // One card per distinct model — drop the 200K twins so the wizard shows a clean
      // spread (the primary/1M variant of each) rather than duplicate-labelled cards.
      const _seenModelIds = new Set();
      const models = (cliModels.length ? cliModels : getModelsForProfile('claude-code'))
        .filter(m => (_seenModelIds.has(m.id) ? false : _seenModelIds.add(m.id)))
        .slice(0, 4);
      const cliLabel = CLI_PROFILES.find(p => p.id === chosenCli)?.label || 'Claude Code';
      ...
      seq.add(() => {
        ...
        models.forEach((model, i) => {
          ...
          card.addEventListener('click', () => {
            storage.setItem(KEYS.ONBOARDING_MODEL, modelSelectorValue(model));
            flushStorage();
            nextStep();
          });
          els.push(card);
