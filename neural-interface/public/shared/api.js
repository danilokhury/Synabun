...
export async function fetchMemories(includeLinks = true) {
  // fields=graph: slim payload (truncated content is flagged with
  // contentTruncated and hydrated on demand by ui-detail.js)
  const url = includeLinks ? '/api/memories?fields=graph' : '/api/memories?links=false&fields=graph';
...
export async function testSchedule(id, { sidepanelWindowId, sidepanelClaimToken } = {}) {
  return jsonFetch(`/api/schedules/${encodeURIComponent(id)}/test`, {
    method: 'POST',
    ...jsonBody({ sidepanelWindowId, sidepanelClaimToken }),
  });
}
