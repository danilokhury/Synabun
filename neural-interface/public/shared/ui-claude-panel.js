...
setCpCtx({
  md: (t) => md(t),
  esc: (t) => esc(t),
  linkifyFilePaths: (el) => linkifyFilePaths(el),
  addCopyButtons: (el) => addCopyButtons(el),
  scrollEnd: () => scrollEnd(),
  activeTab: () => activeTab(),
  saveTabs: () => saveTabs(),
...
const STOR = {
  ...
  permissionMode: 'synabun-claude-panel-permission-mode',     // default permission mode for new tabs (SDK engine)
  commands: 'synabun-claude-panel-commands',                  // cached server slash-command list {at, commands[]}
};
...
// Snapshots live in the per-entry blob store (multi-MB maps used to ride the
// ui-state PATCH cycle — every save re-uploaded ALL snapshots). Hydration is
// async; readers that need it settled await _snapshotsReady, everyone else
// tolerates a transiently empty map (falls back to JSONL rebuild).
let _sessionSnapshots = {};
const _snapshotsReady = fetchBlobNamespace('claude-snapshots')
  .then((map) => {
    // Entries written before hydration finished win over the server copy.
    _sessionSnapshots = { ...map, ..._sessionSnapshots };
    // One-release legacy cleanup: drop the old whole-map storage key.
    try { if (storage.getItem(STOR.sessionSnapshots)) storage.removeItem(STOR.sessionSnapshots); } catch {}
  })
  .catch(() => {});
  ...
  panel.innerHTML = `
    ...
    <div class="cp-bottom">
      ...
      <div class="cp-project-bar">
        ...
        <div class="cp-dropdown cp-dropdown-sm" id="cp-mode" data-placeholder="mode" data-tooltip="Permission mode (Shift+Tab cycles)" hidden><span class="cp-dd-label">mode</span><span class="cp-dd-arrow">&#x25BE;</span><div class="cp-dd-menu"></div></div>
        <div class="cp-bar-actions">
          ...
          <button class="cp-bar-action" id="cp-action-schedule" data-tooltip="Create schedule"><svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="12" height="11" rx="1.5"/><path d="M2 6.5h12M5.5 1.5v3M10.5 1.5v3"/></svg></button>
        </div>
      </div>
...
function injectStyles() {
  ...
  style.textContent = CP_STYLES + `
    ...
    .cp-messages::-webkit-scrollbar { width: 10px; }
    .cp-messages::-webkit-scrollbar-track { background: transparent; }
    .cp-messages::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.2); border-radius: 5px; min-height: 40px; }
    .cp-messages::-webkit-scrollbar-thumb:hover { background: rgba(255,255,255,0.35); }
...
function createTab(sessionId = null, label = 'New chat', { autoSwitch = true, id = null } = {}) {
  ...
  const tab = {
    ...
    pillEl: null,
    draft: '',
    pendingLabel: null,        // authoritative label set before sessionId exists; applied when session starts
    titleState: sessionId || (label && label !== 'New chat') ? 'manual' : 'default',
    titleRequestId: null,
  ...
  // transcript writes are element-scoped, so rendering into a detached
  // element works identically (and skips layout entirely).
  ...
  tab.scrollController = createStickyScrollController(msgDiv, { active: false });
  // Connect WebSocket
  connectTab(tab);
...
// detach in switchTab's exit path. NOTE: background transcripts are NOT in the
// document — any future code must reach them via tab.messagesEl, never via
// document.querySelector.
function _attachMessagesEl(tab) {
  if (!tab?.messagesEl || tab.messagesEl.isConnected) return;
  _panel?.querySelector('#cp-messages-container')?.appendChild(tab.messagesEl);
}
...
function switchTab(idx) {
  ...
  if (prev && prev !== _tabs[idx]) {
    prev.scrollController?.setActive(false);
    flushSessionSnapshotSave(prev);
    const $input = _panel?.querySelector('#cp-input');
    if ($input) prev.draft = $input.value;
    // Animate out, then hide + DETACH from the document
    prev.messagesEl.classList.remove('cp-tab-enter');
    prev.messagesEl.classList.add('cp-tab-exit');
    const prevEl = prev.messagesEl;
    const token = (prevEl._exitToken = (prevEl._exitToken || 0) + 1);
    const finishExit = () => {
      if (prevEl._exitToken !== token) return; // superseded by a later switch
      prevEl.classList.remove('cp-tab-exit');
      if (prev === activeTab()) return;        // re-activated mid-animation
      prevEl.style.display = 'none';
      if (_tabs.includes(prev) && prevEl.isConnected) prevEl.remove();
    };
    prevEl.addEventListener('animationend', finishExit, { once: true });
    // The animation may never fire (panel hidden, prefers-reduced-motion)
    setTimeout(finishExit, 400);
  }
  _activeTabIdx = idx;
  const tab = activeTab();
  tab.messagesEl._exitToken = (tab.messagesEl._exitToken || 0) + 1; // cancel pending exits
  _attachMessagesEl(tab);
  tab.messagesEl.style.display = '';
  tab.scrollController?.setActive(true);
  ...
  // Catch up a stream that kept arriving while this tab was hidden — the
  // throttle tick skips rendering for non-active tabs and sets pendingRender.
  if (tab._stream?.pendingRender) {
    tab._stream.pendingRender = false;
    _renderStreamTick(tab);
  }
  // Refresh the thinking timer text (background tabs skip per-tick DOM writes)
  _refreshThinkTimer(tab);
  // Restore draft
  const $input = _panel?.querySelector('#cp-input');
...
function closeTab(idx, { detachAutomation = false } = {}) {
  if (idx < 0 || idx >= _tabs.length) return;
  const tab = _tabs[idx];
...
// Coalesce repeated renderPills() calls within a tick into one rAF-batched
// pass — several happen per tab create/close/switch/toggle.
let _renderPillsRaf = 0;
function renderPills() {
  if (_renderPillsRaf) return;
  _renderPillsRaf = requestAnimationFrame(() => {
    _renderPillsRaf = 0;
    _renderPillsNow();
  });
}
function _renderPillsNow() {
  ...
  for (let i = 0; i < _tabs.length; i++) {
    ...
    // Animate pill appearing — restart the enter animation by re-adding the
    // class on the next frame instead of forcing a synchronous reflow.
    if (shouldShow && wasHidden) {
      const pillEl = tab.pillEl;
      pillEl.classList.remove('cp-pill-enter');
      requestAnimationFrame(() => {
        pillEl.classList.add('cp-pill-enter');
        pillEl.addEventListener('animationend', () => {
          pillEl.classList.remove('cp-pill-enter');
        }, { once: true });
      });
    }
...
function _forkClonedClaudePersistence() {
  try {
    ...
    const nextTabsKey = `synabun-claude-panel-tabs-${nextWindowId}`;
    ...
    _windowId = nextWindowId;
    STOR.tabs = nextTabsKey;
    sessionStorage.setItem('cp-window-id', nextWindowId);
  } catch {}
}

_forkClonedClaudePersistence();
_cleanStaleWindows();
_updateWindowRegistry();
    ...
    if (raw) {
      ...
      if (data.tabs?.length) {
        for (const saved of data.tabs) {
          ...
          const t = createTab(saved.automationRunId ? null : saved.sessionId, saved.label, { autoSwitch: false, id: saved.id });
          if (t && saved.automationRunId) t.sessionId = saved.sessionId || null;
          if (t && saved.sessionCost) t.sessionCost = saved.sessionCost;
          if (t && saved.running && !saved.automationRunId) t._wasRunning = true; // trigger reattach on WS open
          ...
          if (t && saved.automationRunId) t.automationOwnerId = saved.automationOwnerId || null;
        }
        if (_tabs.length) {
    ...
    if (legacyRaw) {
      ...
      if (data.tabs?.length) {
        for (const saved of data.tabs) {
          ...
          if (t && saved.automationRunId) t.sessionId = saved.sessionId || null;
          if (t && saved.sessionCost) t.sessionCost = saved.sessionCost;
          if (t && saved.running && !saved.automationRunId) t._wasRunning = true;
          if (t && saved.project) t.project = saved.project;
          if (t) t.titleState = saved.titleState || (saved.sessionId || (saved.label && saved.label !== 'New chat') ? 'manual' : 'default');
          if (t) t.model = saved.model || _getDefaultModel();
          if (t && saved.effort) t.effort = saved.effort;
...
window.addEventListener('focus', () => {
  if (!_panel) return;
  // Check EVERY tab, not just the active one — a background tab's WS can die
  // while the window is unfocused (browsers throttle background timers, so
  // its reconnect timer may not have fired yet either).
  ...
  renBtn?.addEventListener('click', (e) => {
    ...
    function commit() {
      ...
      if (curTab && sid === curTab.sessionId) {
        if (val) markClaudeSessionTitleManual(curTab);
        const headerLabel = _panel?.querySelector('.cp-session-label');
...
function markClaudeSessionTitleManual(tab) {
  if (!tab) return;
  tab.titleRequestController?.abort();
  tab.titleRequestController = null;
  tab.titleRequestId = null;
...
function requestClaudeSessionTitle(tab, prompt, { cwd, model, effort, paths, hasImages } = {}) {
  ...
  requestGeneratedSessionTitle({
    ...
    if (tab === activeTab()) {
      const headerLabel = _panel?.querySelector('.cp-session-label');
      if (headerLabel && !headerLabel.querySelector('.cp-rename-input')) headerLabel.textContent = title;
    }
    saveTabs();
  }).finally(() => {
...
function renameSession(sid, currentLabel) {
  ...
  function commit() {
    ...
    if (val === String(currentLabel || '').trim()) {
      btn.textContent = currentLabel || (effectiveSid ? effectiveSid.slice(0, 8) + '...' : 'New chat');
      return;
    }
    if (val) {
      ...
      if (tab && tab.turns === 0 && tab.titleState !== 'generating') tab.titleState = 'default';
...
function _persistSessionSnapshots(changedSid) {
  try {
    ...
    for (const sid of Object.keys(source)) {
      if (!next[sid]) deleteBlob('claude-snapshots', sid);
    }
    _sessionSnapshots = next;
    // Upload ONLY the changed session's entry — the server enforces the same
    // caps via the prune query params.
    if (changedSid && next[changedSid]) {
      putBlob('claude-snapshots', changedSid, next[changedSid], {
        maxEntries: MAX_SESSION_SNAPSHOTS,
        maxChars: MAX_SESSION_SNAPSHOT_CHARS,
      });
    }
  } catch {}
}
...
function writeSessionSnapshot(tab) {
  ...
  _sessionSnapshots[sid] = {
    ...
    todos: Array.isArray(tab.todos) && tab.todos.length ? tab.todos.map(t => ({ ...t })) : null,
...
// ── Snapshot scheduling — boundary writes, not per-append ──
// Serializing a transcript's innerHTML on a 350ms debounce during streaming
// was the single largest source of streaming-time work (and upload churn).
// Appends now only mark the tab dirty; real writes happen at hard boundaries
// (turn end, tab switch, close, pagehide/hidden) plus a 30s safety write for
// very long turns. Crash recovery is still backed by the server JSONL rebuild.
let _snapshotFlushTimer = null;
const SNAPSHOT_IDLE_MS = 5_000;        // flush idle (non-running) dirty tabs after 5s
const SNAPSHOT_MAX_STALE_MS = 30_000;  // running tabs: at most one write per 30s

function _armSnapshotFlusher() {
  if (_snapshotFlushTimer) return;
  _snapshotFlushTimer = setTimeout(() => {
    _snapshotFlushTimer = null;
    _flushDirtySnapshots();
  }, SNAPSHOT_IDLE_MS);
}

function _flushDirtySnapshots() {
  let remaining = false;
  for (const tab of _tabs) {
    if (!tab._snapshotDirty) continue;
    const stale = Date.now() - (tab._snapshotLastWrite || 0) > SNAPSHOT_MAX_STALE_MS;
    if (!tab.running || stale) {
      tab._snapshotDirty = false;
      tab._snapshotLastWrite = Date.now();
      if (typeof requestIdleCallback === 'function') {
        requestIdleCallback(() => writeSessionSnapshot(tab), { timeout: 2000 });
      } else {
        writeSessionSnapshot(tab);
      }
    } else {
      remaining = true; // running and recently written — re-check later
    }
  }
  if (remaining) _armSnapshotFlusher();
}

function scheduleSessionSnapshotSave(tab) {
...
function flushSessionSnapshotSave(tab) {
  if (!tab) return;
  if (tab._snapshotTimer) { clearTimeout(tab._snapshotTimer); tab._snapshotTimer = null; } // legacy field
  tab._snapshotDirty = false;
  tab._snapshotLastWrite = Date.now();
  writeSessionSnapshot(tab);
}
...
function renderStoredSession(snapshot, $msgs, tab) {
  ...
  if (tab) {
    ...
    if (norm.todos?.length) {
      tab.todos = norm.todos.map(t => ({ ...t }));
      // Only the active tab renders the dock — switchTab re-renders for the rest
      if (tab === activeTab()) { try { renderTodoWidget(tab); } catch {} }
    }
    renderGauge(tab);
  }
  return true;
...
async function loadSessionHistory(sid, $msgs) {
  ...
  // Snapshot hydration is async (blob store); wait for it so a fresh page
  ...
  try {
    ...
    // Chunked rebuild: rendering 500 messages in one task froze the UI during
    // cold restore / orphan resync. The generation counter aborts a stale
    // rebuild when a newer one starts (reattach resync, in-tab session switch,
...
function addCopyButtons(el) {
  el.querySelectorAll('pre').forEach(pre => {
    ...
    // No per-button listener — clicks are handled by _onMessagesClick
    // (delegation), which also revives buttons on snapshot-restored DOM.
    pre.appendChild(btn);
...
function linkifyFilePaths(el) {
  ...
  for (const node of nodes) {
    ...
    while ((match = pathRe.exec(text))) {
      ...
      // Click handled by _onMessagesClick (delegation)
      frag.appendChild(a);
...
// Budgeted drain — a permission card held for a minute can queue hundreds of
// messages; processing them in one synchronous loop froze the UI on Allow.
// setTimeout (not rAF) so the drain still progresses in hidden windows.
const MSG_DRAIN_BUDGET_VISIBLE = 25;
const MSG_DRAIN_BUDGET_HIDDEN = 200;

function _flushMsgBuffer(tab) {
  if (tab._draining) return;
  tab._draining = true;
  const step = () => {
    if (tab.closed) { tab._draining = false; return; }
    const budget = document.hidden ? MSG_DRAIN_BUDGET_HIDDEN : MSG_DRAIN_BUDGET_VISIBLE;
    let n = 0;
    while (tab._msgBuffer.length && n++ < budget) {
      // A drained message can re-arm a permission/ask prompt — stop and let
      // that flow's resolution resume the drain (its handlers call us again).
      if (tab._activePerm || tab.pendingAskRequestId) break;
      _processTabMsg(tab, tab._msgBuffer.shift());
    }
    if (tab._msgBuffer.length && !tab._activePerm && !tab.pendingAskRequestId) {
      setTimeout(step, 0);
    } else {
      tab._draining = false;
    }
      ...
      if (msg.ok) {
        ...
        if (msg.userMessageUuid && tab.messagesEl) {
          ...
          if (anchor) {
            ...
            while (n) { n.classList.add('cp-rewound'); n = n.nextElementSibling; }
          }
        }
      } else {
        appendStatus(tab, `Rewind failed: ${msg.error || 'unknown error'}`);
      }
      break;
    case 'control_cancelled': {
      // Server cancelled a pending permission (interrupt/disconnect) — lock the card
      const card = tab.messagesEl?.querySelector(`[data-request-id="${CSS.escape(msg.request_id || '')}"]`);
      if (card) {
        card.classList.remove('active-perm');
        card.classList.add('resolved');
        card.querySelectorAll('button, textarea, input').forEach(n => { n.disabled = true; });
      }
      // A cancelled AskUserQuestion must release the ask state too
      if (tab.pendingAskRequestId === msg.request_id) {
...
function handleTabEvent(tab, ev) {
  ...
  // ── Subagent scope routing (SDK engine) ──
  // Messages emitted by a Task subagent carry parent_tool_use_id; route them to
  // the agent card's nested feed via a prototype-inheriting scope so all the
  // renderers below work unchanged.
  const _parentId = ev.parent_tool_use_id || ev.message?.parent_tool_use_id || null;
  ...
  if (ev.type === 'user') {
    // SDK replays our own user messages with uuids — stamp the latest un-stamped
    // user row so the rewind affordance can target it. Tool results inside user
    // messages arrive separately as synthetic tool_result events from the bridge.
    if (!_parentId && ev.uuid && Array.isArray(ev.message?.content)) {
      const hasText = ev.message.content.some(b => b?.type === 'text' || typeof b === 'string');
  ...
  if (ev.type === 'system' && ev.subtype === 'mcp_status') {
    if (Array.isArray(ev.servers)) {
      tab.mcpServers = ev.servers;
      if (tab === activeTab()) renderStatusline(tab);
    }
    return;
  }
  if (ev.type === 'system' && ev.subtype === 'retry') {
  ...
  if (ev.type === 'stream_event' && ev.event) {
    handleStreamDelta(scope, ev.event);
    return;
  }
...
// Re-parsing the FULL accumulated buffer on every throttle tick is O(n²) over
// a long answer and rebuilds the entire message subtree each time. Instead we
// split the buffer at the last "stable" block boundary: stable blocks are
// parsed ONCE into permanent child nodes of .msg-body, and only the open tail
// (typically the last paragraph) re-parses per tick. The final render at
// block/message stop is still one full parse of the whole buffer, so the
// finished DOM is byte-identical to the pre-incremental output regardless of
// any seam the boundary heuristic might introduce mid-stream.

function _rootOf(tab) {
  return tab?._agentScope ? (tab._rootTab || Object.getPrototypeOf(tab)) : tab;
}

// Largest index B ≤ text.length - minTail such that text.slice(0, B) ends just
// after a blank line OUTSIDE any code fence, and the line starting at B does
// not continue a multi-block construct (list/indent/quote/table) whose
// rendering would differ if parsed apart from what precedes it. Returns `from`
// when no better boundary exists. `from` must itself be a valid boundary
// (start of text or a previously returned value) so fence state can start
// closed.
function _findStableBoundary(text, from = 0, minTail = 120) {
  let best = from;
  let inFence = false, fenceChar = '', fenceLen = 0;
  let pos = from;
  let prevBlank = false;
  const limit = text.length - minTail;
  while (pos < limit) {
    ...
    if (fence) {
      if (!inFence) { inFence = true; fenceChar = fence[1][0]; fenceLen = fence[1].length; }
      else if (fence[1][0] === fenceChar && fence[1].length >= fenceLen) { inFence = false; }
    }
    const lineStart = pos;
    const isBlank = trimmed === '';
    pos = nl + 1;
    if (inFence) { prevBlank = false; continue; }
    if (prevBlank && !isBlank && lineStart <= limit) {
      // Candidate boundary at lineStart. Reject when the new line looks like a
      // continuation: leading whitespace (indented code / nested list), list
      // markers (a loose list split in two renders differently), blockquote,
      // or table rows. Conservative rejections only delay promotion.
      const continuation = /^[ \t]/.test(line) || /^([-*+] |\d{1,9}[.)] |>|\|)/.test(trimmed);
      if (!continuation) best = lineStart;
    }
    prevBlank = isBlank;
  }
  return best;
...
// One incremental tick: promote newly-stabilized blocks (parsed once), then
// re-render only the open tail. Falls back to the legacy full render on any
// error or when marked.js hasn't loaded (md() is then a plain-text escape).
function _renderStreamTick(tab) {
  const s = tab._stream;
  if (!s?.bodyEl) return;
  if (!_marked) {
    s.bodyEl.innerHTML = md(s.textBuf);
    s.stableLen = 0; s.stableNodes = 0;
    return;
  }
  try {
    const prevStable = s.stableLen || 0;
    const newStable = _findStableBoundary(s.textBuf, prevStable);
    // Drop the volatile tail nodes — everything past the stable prefix.
    while (s.bodyEl.childNodes.length > (s.stableNodes || 0)) s.bodyEl.lastChild.remove();
    if (newStable > prevStable) {
      const tpl = document.createElement('template');
      tpl.innerHTML = md(s.textBuf.slice(prevStable, newStable));
      s.bodyEl.appendChild(tpl.content);
      s.stableNodes = s.bodyEl.childNodes.length;
      s.stableLen = newStable;
    }
    const tail = s.textBuf.slice(s.stableLen || 0);
    if (tail) {
      const tpl = document.createElement('template');
      tpl.innerHTML = md(tail);
      s.bodyEl.appendChild(tpl.content);
    }
  } catch {
    s.bodyEl.innerHTML = md(s.textBuf);
...
// Final full render — exact legacy behavior, shared by content_block_stop and
// message_stop. Guarantees the finished DOM matches a single whole-buffer parse.
function _flushStreamRender(tab) {
  const s = tab._stream;
  if (!s) return;
  if (s.mdTimer) { clearTimeout(s.mdTimer); s.mdTimer = null; }
  s.pendingRender = false;
  if (s.bodyEl && s.textBuf.trim()) {
    s.bodyEl._rawMd = s.textBuf;
    s.bodyEl.innerHTML = md(s.textBuf);
    linkifyFilePaths(s.bodyEl);
    addCopyButtons(s.bodyEl);
  }
  s.stableLen = 0; s.stableNodes = 0;
}
...
function handleStreamDelta(tab, apiEvent) {
  ...
  if (evType === 'message_start') {
    ...
    tab._stream = { el: null, textBuf: '', thinkBuf: '', bodyEl: null, thinkEl: null, blockIdx: -1, blockType: null, mdTimer: null, msgId, stableLen: 0, stableNodes: 0, pendingRender: false };
    // Agent scopes inherit thinkTimerInterval through the prototype — calling
    // hideThinking on them would clear the ROOT tab's thinking indicator.
    if (!tab._agentScope) hideThinking(tab);
  ...
  if (evType === 'message_stop') {
    // Finalize: one full whole-buffer render (exact legacy output)
    _flushStreamRender(tab);
    if (tab._stream?.el) {
  ...
  if (evType === 'content_block_start') {
    if (!tab._stream) tab._stream = { el: null, textBuf: '', thinkBuf: '', bodyEl: null, thinkEl: null, blockIdx: -1, blockType: null, mdTimer: null, stableLen: 0, stableNodes: 0, pendingRender: false };
    tab._stream.blockIdx = apiEvent.index ?? (tab._stream.blockIdx + 1);
    tab._stream.blockType = apiEvent.content_block?.type || null;

    // Eager tool stubs: a lightweight pulsing ghost card appears the moment the
    // model opens a tool_use block; renderAssistant swaps it for the real card
    // (with full input) when the complete message arrives. No buildTool side
    // effects fire for ghosts.
    if (tab._stream.blockType === 'tool_use' && apiEvent.content_block?.id && apiEvent.content_block?.name !== 'AskUserQuestion') {
      if (!tab._stream.el) {
        const el = document.createElement('div');
      ...
      if (wrapEl && !wrapEl.querySelector(`.tool-card[data-tool-id="${CSS.escape(cb.id)}"]`)) {
        ...
        ghost.dataset.toolId = cb.id;
        ghost.dataset.toolName = cb.name || '';
        ghost.innerHTML = `<div class="tool-hdr"><span class="tool-icon">${toolIconSvg(cb.name)}</span><span class="tool-name">${esc(cb.name || 'tool')}</span><span class="tool-detail">…</span></div>`;
        wrapEl.appendChild(ghost);
        try { if (!tab._agentScope) setActivity(tab, cpActivityVerb(cb.name, {})); } catch {}
        if (tab === activeTab()) scrollEnd();
      }
      return;
    }
    ...
    // content. thinking-with-effort-off is handled by
    ...
    if (!tab._stream.thinkEl) {
      ...
      // Cache the content element — querySelector per thinking token adds up
      tab._stream.thinkContentEl = thinkEl.querySelector('.msg-thinking-content');
    }
    return;
  }
  ...
  if (evType === 'content_block_delta') {
    ...
    if (delta.type === 'thinking_delta' && delta.thinking && tab._stream.thinkEl) {
      ...
      if (!tab._stream.thinkTimer) {
        tab._stream.thinkTimer = setTimeout(() => {
          tab._stream.thinkTimer = null;
          const contentEl = tab._stream?.thinkContentEl
            || tab._stream?.thinkEl?.querySelector('.msg-thinking-content');
          if (contentEl && tab._stream) contentEl.textContent = tab._stream.thinkBuf;
        }, 100);
      }
      ...
      // Throttle markdown rendering to 100ms ticks. Each tick renders
      // incrementally (stable blocks parsed once, only the open tail
      // re-parses — see _renderStreamTick). Hidden tabs skip rendering
      // entirely and catch up on activation via the pendingRender flag.
      // linkifyFilePaths walks the whole subtree; it runs once at block stop.
      if (tab._stream.bodyEl && !tab._stream.mdTimer) {
        tab._stream.mdTimer = setTimeout(() => {
          tab._stream.mdTimer = null;
          if (!tab._stream?.bodyEl) return;
          if (_rootOf(tab) !== activeTab()) { tab._stream.pendingRender = true; return; }
          _renderStreamTick(tab);
          if (tab === activeTab()) scrollEnd();
        }, 100);
      }
  ...
  if (evType === 'content_block_stop') {
    if (!tab._stream) return;
    // Flush pending throttled renders immediately on block stop
    if (tab._stream.thinkTimer) {
      clearTimeout(tab._stream.thinkTimer);
      tab._stream.thinkTimer = null;
      const thinkContentEl = tab._stream.thinkContentEl
        || tab._stream.thinkEl?.querySelector('.msg-thinking-content');
      if (thinkContentEl) thinkContentEl.textContent = tab._stream.thinkBuf;
    }
    _flushStreamRender(tab);
    if (tab._stream.bodyEl && !tab._stream.textBuf.trim()) {
    ...
    if (wrap) {
      ...
      for (const t of regularTools) {
        if (!existingToolIds.has(t.id || '')) {
          wrap.appendChild(buildTool(t, tab));
        } else {
          // Backfill streaming ghost stubs with the real card now that full input arrived
          const ghost = wrap.querySelector(`.tool-card.tool-streaming[data-tool-id="${CSS.escape(t.id || '')}"]`);
          if (ghost) ghost.replaceWith(buildTool(t, tab));
        }
      }
...
function buildSynaBunTool(block) {
  ...
  // Toggle handled by _onMessagesClick (delegation)
...
function buildTool(block, tab) {
  ...
  if (['Edit', 'MultiEdit', 'NotebookEdit'].includes(block.name) || (block.name === 'Write' && !isPlanFile(block.input?.file_path))) {
    try { return buildDiffCard(block, tab); } catch (e) { console.warn('[claude-panel] diff card failed:', e); }
  }
  // Bash: command card with output area, exit pill, background registry
  if (block.name === 'Bash' && tab) {
    try { return buildBashCard(block, tab); } catch (e) { console.warn('[claude-panel] bash card failed:', e); }
  }
  // Background task monitors: route to the originating bash card, then render generic
  if (['BashOutput', 'KillShell', 'TaskStop'].includes(block.name) && tab) {
    try { handleBgToolUse(block, tab); } catch {}
  }
...
function updateToolResult(tab, ev) {
  const $msgs = tab.messagesEl;
  if (!$msgs) return;
  // Background task output (BashOutput results) routes back to the origin card
  try { handleBgToolResult(ev, tab); } catch {}
  ...
  if (card.classList.contains('cp-agent-card')) {
    let resultText = '';
    if (Array.isArray(ev.content)) resultText = ev.content.map(b => b?.text || '').filter(Boolean).join('\n');
    else if (typeof ev.content === 'string') resultText = ev.content;
    finalizeAgentCard(tab, ev.tool_use_id, { isError: !!ev.is_error, resultText });
    return;
  }
...
function appendUser(tab, text, images, files) {
  ...
  if (tab === activeTab()) scrollEnd(true);
  scheduleSessionSnapshotSave(tab);
}

// ── Checkpoint / rewind (SDK engine) ──
// The SDK replays sent user messages with a uuid; we stamp the newest
// un-stamped user row so a hover affordance can rewind files to that point.
function _stampUserMessageUuid(tab, uuid) {
  const $msgs = tab.messagesEl;
  if (!$msgs || !uuid) return;
  const rows = [...$msgs.querySelectorAll('.msg-user')].reverse();
  ...
  btn.title = 'Restore files to their state before this message (conversation is kept)';
...
// ── Shared 1s think ticker ──
// One interval services every thinking row — N running tabs used to mean N
// independent setIntervals all writing to (mostly hidden) DOM. Only the
// active tab gets a DOM write per tick; background timer text refreshes on
// activation since elapsed time derives from sendStartedAt, not tick counts.
let _thinkTickerId = null;
const _thinkingTabs = new Set();

function _refreshThinkTimer(tab) {
  if (!tab?.thinkingEl || !tab.sendStartedAt) return;
  const sec = Math.round((Date.now() - tab.sendStartedAt) / 1000);
  if (tab._thinkTimerEl) tab._thinkTimerEl.textContent = sec > 0 ? `${sec}s` : '';
}

function _thinkTick() {
  const act = activeTab();
  for (const tab of _thinkingTabs) {
    if (tab.closed || !_tabs.includes(tab)) { _thinkingTabs.delete(tab); continue; }
    if (tab !== act) continue; // hidden timer text refreshes on tab activation
    _refreshThinkTimer(tab);
  }
  if (!_thinkingTabs.size && _thinkTickerId) { clearInterval(_thinkTickerId); _thinkTickerId = null; }
}

function showThinking(tab) {
  if (tab?._agentScope) return; // scopes never own a thinking row
  hideThinking(tab);
  const $msgs = tab.messagesEl;
  if (!$msgs) return;
  ...
  tab._thinkTimerEl = el.querySelector('.think-timer');
  _thinkingTabs.add(tab);
  if (!_thinkTickerId) _thinkTickerId = setInterval(_thinkTick, 1000);
  _refreshThinkTimer(tab);
  if (tab === activeTab()) scrollEnd();
}
function hideThinking(tab) {
  _thinkingTabs.delete(tab);
  if (!_thinkingTabs.size && _thinkTickerId) { clearInterval(_thinkTickerId); _thinkTickerId = null; }
  if (tab.thinkTimerInterval) { clearInterval(tab.thinkTimerInterval); tab.thinkTimerInterval = null; } // legacy per-tab field
  tab._thinkTimerEl = null;
...
function scrollEnd(force) {
  ...
  const $msgs = tab?.messagesEl;
  if (!$msgs) return;
  const controller = tab.scrollController || createStickyScrollController($msgs);
  tab.scrollController = controller;
  controller?.scrollToBottom({ force: !!force });
}
...
// Commands the CLI executes natively under the SDK engine — the panel sends the
// raw "/cmd args" text through and the CLI's real implementation answers.
...
function send({ shift = false } = {}) {
  const tab = activeTab();
  if (!tab) return;
  if ((_cliInstalled === null || _cliInstallFailureForced) && !tab.automationActive) {
  ...
  if (text.startsWith('!') && text.length > 1) {
    ...
    autoResize();
    _pushPromptHistory(tab, text);
    requestClaudeSessionTitle(tab, text, {
      cwd: ddGetValue($project) || undefined,
      model: _getModelId() || undefined,
      effort: _getEffort() || undefined,
    });
  ...
  requestClaudeSessionTitle(tab, text, {
    ...
    model: _getModelId() || undefined,
    effort: _getEffort() || undefined,
    paths: pendingFiles?.map((file) => file.name) || [],
    hasImages: !!pendingImages?.length,
  });
...
// cache each switch pays a network round-trip before the branch dropdown
// settles, which reads as tab-switch lag.
const _branchCache = new Map(); // path → { at, branches, current }
...
async function loadBranches(path) {
  ...
  if (cached && Date.now() - cached.at < BRANCH_CACHE_MS) {
    if (cached.branches?.length) {
      ddPopulate($branch, cached.branches.map(b => ({ value: b, label: b })), cached.current || '');
    }
    return;
  }
  try {
    ...
    if (res.branches?.length) {
      _branchCache.set(path, { at: Date.now(), branches: res.branches, current: res.current });
...
// ── Delegated transcript actions ──
// One listener on the permanent container replaces per-element listeners on
// every copy button / file link / generic tool header across every tab
// (hundreds of cards × N tabs). Bonus: controls on snapshot-restored DOM
// (whose listeners were lost to innerHTML serialization) work again.
// Specialized cards (cp-bash/cp-diff/cp-agent + interactive perm/ask/plan
// widgets) keep their own listeners — the delegate skips them.
function _onMessagesClick(e) {
  const copy = e.target.closest('.cp-copy-btn');
  if (copy) {
  ...
  if (link) {
    e.preventDefault();
    const path = (link.textContent || '').replace(/:\d+$/, '');
    navigator.clipboard.writeText(path);
    link.style.opacity = '0.5';
    setTimeout(() => { link.style.opacity = ''; }, 300);
    return;
  }
...
// ── Public API ──
async function _ensureClaudePanelReady() {
  if (!_panel) {
    injectStyles();
    _panel = buildPanel();
...
export async function attachClaudeAutomation(run, { focus = !!run?.focus, isCancelled = () => false } = {}) {
  ...
  await _ensureClaudePanelReady();
  if (isCancelled()) return { ok: false, reason: 'dismissed' };
  ...
  if (!tab) {
    ...
    if (tab) {
      tab.id = run.runId;
      tab.sessionId = run.providerSessionId;
      if (tab.messagesEl) tab.messagesEl.dataset.tabId = run.runId;
      if (tab.pillEl) tab.pillEl.dataset.tabId = run.runId;
    } else {
      tab = createTab(null, run.title || 'Automation', {
        autoSwitch: false,
        id: run.runId,
      });
    }
  ...
  tab.sdkMode = true;
  tab.sessionId = run.providerSessionId;
  tab.label = run.title || tab.label || 'Automation';
  tab.project = run.cwd || tab.project;
  tab.model = run.model || tab.model;
  tab.effort = run.effort || tab.effort;
  setRunning(tab, running);
  updatePillLabel(tab);
  tab._automationIteration = run.currentIteration;
  if (!tab._automationHydrated) {
    tab._automationHydrated = true;
    if (tab.messagesEl) tab.messagesEl.innerHTML = '';
    await loadSessionHistory(run.providerSessionId, tab.messagesEl).catch(() => {});
    if (isCancelled() || tab.closed) {
      await detachClaudeAutomation(run.runId);
      return { ok: false, reason: 'dismissed' };
    }
  }
  if (run.status === 'failed' && run.error && tab._automationLifecycleError !== run.error) {
...
export function handleClaudeAutomationEvent(runId, payload) {
  const tab = _tabs.find((entry) => entry.automationRunId === runId);
  const event = payload?.event || payload;
  if (!tab || tab.closed || !event) return false;
  if (event.type === 'synabun.user_prompt') {
    const key = `${payload?.iteration || event.iteration || tab._automationIteration || 0}:${event.text || ''}`;
    tab._automationPromptKeys ||= new Set();
    if (!tab._automationPromptKeys.has(key)) {
      tab._automationPromptKeys.add(key);
      appendUser(tab, event.text || '');
    }
    return true;
  }
  if (event.type === 'synabun.error') {
...
export async function detachClaudeAutomation(runId) {
  const idx = _tabs.findIndex((entry) => entry.automationRunId === runId);
  if (idx < 0) return false;
  closeTab(idx, { detachAutomation: true });
  return true;
}
...
// Server-provided slash commands (SDK engine): active tab's commands_list,
// falling back to the cached copy so hints work before the first init.
function _serverSlashCommands() {
  const tab = activeTab();
  let list = tab?.slashCommands;
  if (!list?.length) {
...
// Only collapses a run of identical MCP calls that are GENUINE consecutive DOM
// siblings within the same .msg-content turn. A flat document-wide NodeList is
// NOT adjacency: two same-named cards in different turns (e.g. a `remember` early
// and another `remember` later) would otherwise be treated as a group and the
// second hidden, making it "blink then disappear". The pass also clears stale
// collapse state first so it is idempotent and self-healing across re-renders.
function collapseAdjacentMcpTools($msgs) {
  if (!$msgs) return;
  // Reset prior collapse state so cards never stay hidden once they no longer
  // qualify (turn boundaries shift as the DOM re-renders).
  $msgs.querySelectorAll('.tool-card.cp-mcp-collapsed').forEach(c => c.classList.remove('cp-mcp-collapsed'));
  $msgs.querySelectorAll('.tool-mcp-badge').forEach(b => b.remove());
  ...
  // True if `c` is the next tool-card sibling after `prev` (same parent, only
  // tool-cards between them) — i.e. part of one contiguous run in one turn.
  const isContiguous = (prev, c) => {
    if (!prev || prev.parentNode !== c.parentNode) return false;
    let n = prev.nextElementSibling;
    while (n && n !== c) {
      if (!n.classList.contains('tool-card')) return false;
  ...
  for (const c of cards) {
    if (prev && c.dataset.toolName === prev.dataset.toolName && isContiguous(prev, c)) {
      group.push(c);
    } else {
      flush();
      group = [c];
...
function wireEvents() {
  ...
  // Permission mode lives in the project-bar dropdown (#cp-mode); Shift+Tab still cycles.
  ...
  // Schedule creator — opens a fresh tab and runs the lean /synabun schedule flow so
  // the user's current conversation is left untouched.
  _panel.querySelector('#cp-action-schedule')?.addEventListener('click', () => {
    sendToPanel('/synabun schedule', { newTab: true, tabLabel: 'Schedule', autoSubmit: true });
  });
