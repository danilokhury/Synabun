...
import { updateMemory, deleteMemory, fetchMemory } from './api.js';
import { KEYS } from './constants.js';
import { storage } from './storage.js';
import { catColor } from './colors.js';
import { truncate, formatMemoryContent, exportMemoryAsMarkdown, rafThrottle } from './utils.js';
...
function enterEditMode(card, node) {
  ...
  // Reading scrollHeight forces a synchronous reflow; coalesce to one per frame
  // so fast typing doesn't thrash layout on every keystroke.
  const autoResize = rafThrottle(() => {
...
// The card shell is a large static HTML+SVG template that only varies by
// panelId (3 spots) and the session locale. Parsing this string on every card
// open (i.e. every node click) is wasted work — build it once per locale and
// cloneNode() per open, which is markedly cheaper than re-parsing innerHTML.
let _cardTemplateNode = null;
let _cardTemplateLocale = null;

function createDetailCard(panelId) {
  const loc = getLocale();
  if (!_cardTemplateNode || _cardTemplateLocale !== loc) {
    _cardTemplateNode = _buildCardTemplate();
    _cardTemplateLocale = loc;
  }
  const el = _cardTemplateNode.cloneNode(true);
  el.id = panelId;
  // Patch the panelId-dependent hooks the drag/pin systems key off.
  const dragEl = el.querySelector('.detail-header.drag-handle');
  if (dragEl) dragEl.setAttribute('data-drag', panelId);
  const pinEl = el.querySelector('.pin-btn');
  if (pinEl) pinEl.setAttribute('data-pin', panelId);
  return el;
}

function _buildCardTemplate() {
  ...
  el.innerHTML = `
    ...
    <div class="detail-header drag-handle" data-drag="">
      <div class="detail-header-actions">
        ...
        <button class="detail-action-btn pin-btn" data-pin="" data-tooltip="${t('common.pin')}"><svg viewBox="0 0 24 24"><path d="M9 4v4.5L7.5 10H6v2h4v7l2 1 2-1v-7h4v-2h-1.5L15 8.5V4H9z" stroke-linejoin="round" stroke-linecap="round"/></svg></button>
...
export function openMemoryCard(node, savedPosition) {
  ...
  // Graph payloads may carry truncated content (fields=graph) — hydrate the
  // full memory before the user can read or edit a clipped version.
  if (p.contentTruncated) {
    fetchMemory(node.id).then((full) => {
      const fullContent = full?.payload?.content ?? full?.content;
      if (typeof fullContent !== 'string') return;
      p.content = fullContent;
      delete p.contentTruncated;
      const cardState = _openCards.get(node.id);
      if (cardState && !cardState.isEditing) {
        contentEl.innerHTML = formatMemoryContent(fullContent);
      }
    }).catch(() => {});
  }
...
export function initDetailPanel() {
  ...
  on('detail:layout-changed', (payload) => {
    persistOpenCards();
    // Sync only the card that moved when the event says which one; fall back
    // to syncing all for emitters that don't pass a panelId
    if (!_isRemoteCardOp) {
      for (const [id, c] of _openCards) {
        if (payload?.panelId && c.el.id !== payload.panelId) continue;
        sendSync({
          type: 'card:moved',
          memoryId: id,
          left: c.el.style.left,
