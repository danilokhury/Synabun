...
export function createFrameRenderer(canvas, ctx) {
  ...
  function flush() {
    ...
    // Tab hidden — skip decode entirely; the latest frame stays in `pending`
    // and draws as soon as render() is called again on a visible tab
...
/**
 * Debounce — invoke `fn` only after `wait` ms have elapsed since the last call
 * (trailing edge). Use for input-driven work that shouldn't run per keystroke
 * (filter rebuilds, syntax highlight). The wrapper exposes `.cancel()` to drop a
 * pending call (e.g. on teardown).
 * @param {Function} fn
...
/**
 * rAF throttle — coalesce a burst of calls into a single invocation on the next
 * animation frame, always running with the most recent arguments. Ideal for
 * scroll handlers and "rebuild at most once per frame" render coalescing
 * (vsync-aligned, drops redundant work without deferring the visible result by
 * more than one frame). The wrapper exposes `.cancel()` to drop a pending frame.
 * @param {Function} fn
 * @returns {Function & { cancel(): void }}
 */
