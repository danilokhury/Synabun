...
export class VTermBuffer {
  ...
  /** Monotonic generation counter — bumps on every mutation (write/resize).
   *  Lets the server memoize rendered snapshots: same generation ⇒ identical
   *  buffer contents. Inert on the browser side. */
  get writeGen() { return this._writeGen || 0; }
  ...
  getText(startRow, startCol, endRow, endCol) {
    ...
    for (let r = startRow; r <= endRow; r++) {
      ...
      // Array+join instead of += — string concat is O(n²) over wide rows and
      // this runs over the full scrollback for snapshots
      const chars = [];
      for (let c = s; c <= e; c++) {
