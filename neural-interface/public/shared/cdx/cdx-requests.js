...
export function sendServerRequestReply(requestId, { result, error, label = 'submitted' }) {
  const packet = {
  ...
  lockRequestCard(getRequestCardEntry(requestId), label);
  _ctx.onBlockingServerRequestAnswered?.(requestId);
  return true;
}
...
export function renderUserInputRequest(requestId, params) {
...
export function renderMcpElicitationRequest(requestId, params) {
  ...
  if (params?.mode === 'url') {
    ...
    accept.addEventListener('click', () => sendServerRequestReply(requestId, { result: { action: 'accept' }, label: 'accepted' }));
    decline.addEventListener('click', () => sendServerRequestReply(requestId, { result: { action: 'decline' }, label: 'declined' }));
    cancel.addEventListener('click', () => sendServerRequestReply(requestId, { result: { action: 'cancel' }, label: 'cancelled' }));
...
export function renderApprovalRequest(requestId, method, params) {
  ...
  const actions = createRequestActions();
  if (isPermissions) {
    const grantTurn = createRequestButton('Grant This Turn');
    const grantSession = createRequestButton('Grant This Session', 'secondary');
    const decline = createRequestButton('Decline', 'danger');
    grantTurn.addEventListener('click', () => sendServerRequestReply(requestId, {
      result: { permissions: params?.permissions || {}, scope: 'turn' },
      label: 'granted',
    }));
    grantSession.addEventListener('click', () => sendServerRequestReply(requestId, {
      result: { permissions: params?.permissions || {}, scope: 'session' },
      label: 'granted',
    }));
    decline.addEventListener('click', () => sendServerRequestReply(requestId, {
      error: { code: 4001, message: 'Permission request declined by user' },
      label: 'declined',
    }));
    actions.append(grantTurn, grantSession, decline);
  } else {
    const available = Array.isArray(params?.availableDecisions) && params.availableDecisions.length
      ? new Set(params.availableDecisions)
      : null; // null = show all default buttons
    const decisionMap = {
    ...
    const decisions = available ? [...available] : defaultDecisions;
...
export function handleServerRequest(request) {
  ...
  if (tab?.autoAccept) {
    ...
    if (isApproval) {
      sendServerRequestReply(requestId, { result: { decision: 'acceptForSession' } });
      _ctx.appendSystem(`Auto-accepted: ${request.method.split('/').slice(1, -1).join('/')}`, 'muted');
      return;
    }
    if (request.method === 'mcpServer/elicitation/request') {
      sendServerRequestReply(requestId, { result: { action: 'accept' } });
      _ctx.appendSystem('Auto-accepted MCP elicitation', 'muted');
      return;
    }
  }
  if (tab) {
