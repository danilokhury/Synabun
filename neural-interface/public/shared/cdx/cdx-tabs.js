...
import {
  ...
  appendElement, appendSystem, createMessageShell, appendAssistantMarkdownMessage,
...
let _loadBranches = () => {};
export function setCallbacks({ scrollEnd, autosizeInput, emit, on, loadBranches }) {
...
function scrollEnd() { _onScrollEnd?.(); }
...
export function connectTab(tab) {
  wsConnectTab(tab, buildWsCallbacks(tab), { withTab });
}
...
function buildWsCallbacks(tab) {
  return {
    ...
    onOpen(tab, ws, { shouldReattach, wasConnected }) {
      ...
      if (shouldReattach || wasConnected) {
        // Try to reattach to an orphaned server-side process
        sendSocket({ type: 'reattach', windowId: windowId });
      } else {
        // Fresh bootstrap
        sendSocket({ type: 'bootstrap', windowId: windowId, threadId: _threadId || null, cwd: _project || null });
      }
    },
    ...
    onHistory(msg) {
      if (msg.thread) {
        ...
        setSessionLabel(getThreadLabel(
          msg.thread,
    ...
    onThreadStarted(msg) {
      ...
      if (msg.threadId) {
        ...
        _freshThread = true;
        if (_boundTab) { _boundTab.threadId = _threadId; _boundTab.freshThread = true; }
        const pending = _boundTab?.pendingSessionLabel;
        setSessionLabel(getThreadLabel(msg.thread, pending || _sessionLabel || 'New session'));
    ...
    onThreadRenamed(msg) {
      ...
      if (msg.name) setSessionLabel(msg.name);
    },
...
let _threadSnapshots = parseStoredJson(STOR.threadSnapshots, {});
let _tabs = [];
let _activeTabIdx = -1;
let _boundTab = null;
...
function normalizeThreadSnapshotEntry(entry) {
...
function markPostCompactionPrompt(tab = _boundTab, { defer = _running } = {}) {
...
function flushPostCompactionPrompt(tab = _boundTab) {
  if (!tab || tab.showPostPlanActions || !tab.postCompactionPending) return;
  tab.postCompactionPending = false;
...
export async function withTabAsync(tab, fn) {
  const prev = snapshotBoundState();
  bindTabState(tab);
  try {
    return await fn();
...
export function createTab(saved = {}, { autoSwitch = true } = {}) {
  ...
  const inheritedPlanMode = saved.planMode ?? activeTab()?.planMode ?? false;
  const tab = {
...
export function renderStoredTranscript(snapshot, emptyText = freshThreadText()) {
  ...
  const normalized = normalizeThreadSnapshotEntry(snapshot);
...
async function startNewThread() {
  ...
  await withTabAsync(tab, async () => {
    ...
    syncSessionControls();
    try {
      const data = await requestSocketPayload('thread_start', { cwd: _project || null });
      clearPostCompactionState(_boundTab, { updateTranscript: false });
      clearTranscript(freshThreadText());
...
function cxpIsArchived(id) { try { return storage.getItem(CXP_ARCHIVE_PREFIX + id) === '1'; } catch { return false; } }
function cxpSetArchived(id, val) { try { if (val) storage.setItem(CXP_ARCHIVE_PREFIX + id, '1'); else storage.removeItem(CXP_ARCHIVE_PREFIX + id); } catch {} }
...
// ── Session menu (lines 5000-5428) ──
  return threads.filter(t => {
    if (!_cxpSessShowArchived && cxpIsArchived(t.id)) return false;
    ...
    if (_cxpSessSearch) {
      ...
      const labelHit = label.includes(q) || source.includes(q) || (t.id || '').toLowerCase().includes(q);
...
function cxpSessRenderItem(thread, tab, menu) {
  ...
  const active = thread.id === _threadId ? ' active' : '';
  const archived = cxpIsArchived(thread.id);
  ...
  item.innerHTML = `
    ...
    <div class="cxp-sess-meta">
      ...
      <span class="cxp-sess-source">${esc(source)}</span>
    </div>
  ...
  item.addEventListener('click', (event) => {
    withTab(tab, () => {
      ...
      if (!threadId || !sendSocket({ type: 'thread_resume', threadId, cwd: _project || null })) {
        setStatus('Codex is not connected', 'error');
        return;
      }
  ...
  archBtn?.addEventListener('click', async (e) => {
    e.stopPropagation();
    if (!archived) {
      // Try server-side archive for Codex, plus client-side
      try { await requestSocketPayload('thread_archive', { threadId: thread.id }); } catch {}
    }
    cxpSetArchived(thread.id, !archived);
    item.remove();
  });
  ...
  renBtn?.addEventListener('click', (event) => {
    ...
    const finish = async (cancel = false) => {
      ...
      try {
        await withTabAsync(tab, async () => {
          await requestSocketPayload('thread_rename', { threadId, name: nextLabel });
          promptEl.textContent = nextLabel;
          renBtn.style.display = '';
          if (threadId === _threadId) setSessionLabel(nextLabel);
        });
      } catch (err) {
        promptEl.textContent = currentLabel;
        renBtn.style.display = '';
  ...
  await withTabAsync(tab, async () => {
    ...
    if (_threadId) {
      ...
      actionsEl.querySelector('.cxp-sess-fork')?.addEventListener('click', async () => {
        menu.classList.remove('open');
        if (!_threadId) return;
        appendSystem('Forking thread…', 'working');
        try {
          const result = await requestSocketPayload('thread_fork', { threadId: _threadId });
          const forked = result?.thread;
          if (forked?.id) {
            createTab({ project: _project, threadId: forked.id, title: `Fork of ${_sessionLabel}` });
            appendSystem(`Thread forked: ${forked.id}`, 'muted');
          }
        } catch (err) {
          appendSystem(`Fork failed: ${err.message}`, 'error');
        }
      });
      ...
      actionsEl.querySelector('.cxp-sess-archive-current')?.addEventListener('click', async () => {
        menu.classList.remove('open');
        if (!_threadId) return;
        appendSystem('Archiving thread…', 'working');
        try {
          await requestSocketPayload('thread_archive', { threadId: _threadId });
          cxpSetArchived(_threadId, true);
          appendSystem('Thread archived', 'muted');
          startNewThread();
        } catch (err) {
          appendSystem(`Archive failed: ${err.message}`, 'error');
        }
      });
    }
    ...
    refreshBtn.addEventListener('click', async (e) => {
      ...
      sentinel.innerHTML = '';
      try {
        const data = await requestSocketPayload('thread_list', { cwd: _project, limit: 100 });
        if (activeTab() !== tab) return;
        _cxpSessAllThreads = Array.isArray(data?.threads) ? data.threads : [];
        cxpSessRebuildList(menu, listEl, tab);
      } catch (err) {
...
export function renameActiveSession() {
  ...
  const finish = async (cancel = false) => {
    ...
    if (_threadId && _connected) {
      try {
        await withTabAsync(tab, async () => {
          await requestSocketPayload('thread_rename', { threadId: _threadId, name: nextLabel });
  ...
  const commit = async (cancel) => {
    ...
    if (chosenProject && chosenBranch && branchOriginal && chosenBranch !== branchOriginal) {
      try {
        ...
        console.error('[cdx] checkout failed:', err);
      }
    }

    if (!nextLabel) return;
    if (_boundTab) _boundTab.pendingSessionLabel = nextLabel;
    if (_threadId && _connected) {
      const rt = _boundTab;
      withTabAsync(rt, async () => {
        try {
          await requestSocketPayload('thread_rename', { threadId: _threadId, name: nextLabel });
          setSessionLabel(nextLabel);
          if (rt) rt.pendingSessionLabel = null;
        } catch {
          setSessionLabel(nextLabel);
        }
      }).catch(() => {});
    } else {
      setSessionLabel(nextLabel, { persist: false });
    }
  };
...
function appendLocalUserMessage(text, images) {
  if (!_messagesEl) return;
...
export function syncReservedWidth() {
  if (_getVisible() && _getPanelEl()) {
    reserveRightPanelLayout(PANEL_OWNER, _getPanelEl(), 20);
...
function updateMcpServerStatus(name, status, error) {
  // Track MCP server startup status — currently UI-only via appendSystem in caller
}
  ...
  switch (method) {
    case 'thread/name/updated':
      if (params.threadId === _threadId) {
        setSessionLabel(params.threadName || 'Codex');
      }
      break;
    case 'turn/started':
      _pendingQueryRequestId = null;
    ...
    case 'mcpServer/startupStatus/updated':
      updateMcpServerStatus(params.name, params.status, params.error || null);
      if (params.status === 'failed' && params.error) appendSystem(`${params.name} failed: ${params.error}`, 'error');
      break;
    case 'mcpServer/oauthLogin/completed':
      appendSystem(params?.success
        ? `${params.name || 'MCP'} login completed`
        : `${params.name || 'MCP'} login failed${params?.error ? `: ${params.error}` : ''}`, params?.success ? 'muted' : 'error');
      break;
    case 'fuzzyFileSearch/sessionUpdated':
      appendSystem(`File search updated for "${params?.query || ''}"`, 'working');
      break;
    case 'fuzzyFileSearch/sessionCompleted':
      appendSystem('File search completed', 'muted');
      break;
    ...
    case 'error': {
      ...
      if (_compacting && !params?.willRetry) setCompactingUI(false);
    ...
    case 'deprecationNotice':
      appendSystem(params?.message || method, 'error');
      break;
    case 'model/rerouted':
...
export function cycleEffort() {
  const tab = activeTab();
  if (!tab) return;
  const curIdx = EFFORT_LEVELS.indexOf(tab.effort || 'off');
  const nextIdx = (curIdx + 1) % EFFORT_LEVELS.length;
  tab.effort = EFFORT_LEVELS[nextIdx];
...
export function requestModelList() {
  ...
  _modelListRequest = new Promise((resolve) => {
    ...
    window.__cxpModelListResolve = (models) => {
      ...
      resolve(_modelListCache);
      _modelListRequest = null;
      delete window.__cxpModelListResolve;
    };
    setTimeout(() => {
      if (window.__cxpModelListResolve) {
        window.__cxpModelListResolve([]);
      }
    }, 10000);
  });
...
function enqueuePayload({ tab, prompt, images, paths }) {
  if (!tab) return false;
  tab.queue.push({
...
async function handleLocalSlashCommand(payload = currentComposerPayload()) {
  ...
  const { input, tab, images, paths } = payload;
  if (!tab) return true;
  ...
  const sendInlinePrompt = (nextPrompt, options = {}) => {
    ...
    if (pendingImages?.length) {
      appendLocalUserMessage(nextPrompt, pendingImages);
      tab._pendingSendImages = pendingImages;
    }
    consumeComposer(input, tab);
    return true;
  };
  const updateConfig = async (config) => requestSocketPayload('config_write', { config });

  switch (parsed.name) {
    case 'compact':
      if (_running || _startingThread || _compacting) {
        appendSystem('Compaction is unavailable while Codex is busy.', 'error');
        break;
      }
      startCompaction();
    ...
    case 'clear':
      if (_running) {
        appendSystem('/clear is unavailable while a turn is running.', 'error');
        break;
      }
      consumeComposer(input, tab);
    ...
    case 'new':
      if (_running) {
        appendSystem('/new is unavailable while a turn is running.', 'error');
        break;
      }
      consumeComposer(input, tab);
      await startNewThread();
    ...
    case 'fork':
      if (!_threadId) {
        appendSystem('There is no active thread to fork.', 'error');
        break;
      }
      consumeComposer(input, tab, { clearAttachments: false });
      {
        ...
        const forked = result?.thread;
        if (forked?.id) {
          createTab({ project: _project, threadId: forked.id, title: forked.name || `Fork of ${_sessionLabel}` });
          appendSystem(`Forked current thread into ${forked.id}.`, 'muted');
        }
      }
      break;
    case 'review':
      if (!_threadId) {
        appendSystem('Start or resume a Codex thread before running /review.', 'error');
        break;
      }
      consumeComposer(input, tab, { clearAttachments: false });
      await requestSocketPayload('review_start', { threadId: _threadId });
      appendSystem('Started working tree review.', 'working');
      break;
    case 'status':
      consumeComposer(input, tab, { clearAttachments: false });
      {
        const snapshot = await requestSocketPayload('status_snapshot', { cwd: _project || null });
    ...
    case 'debug-config':
      consumeComposer(input, tab, { clearAttachments: false });
      {
        const [configData, requirements] = await Promise.all([
          requestSocketPayload('config_read', {}),
          requestSocketPayload('config_requirements', {}),
        ]);
    ...
    case 'permissions':
      ...
      }
      {
        ...
        else if (normalized === 'on-request' || normalized === 'ask') config = { approval_policy: 'onRequest' };
        else if (normalized === 'unless-trusted' || normalized === 'trusted') config = { approval_policy: 'unlessTrusted' };
        else if (normalized === 'read-only' || normalized === 'readonly') config = { approval_policy: 'onRequest', sandbox_mode: 'read-only' };
        ...
        if (!config) {
          appendSystem(`Unknown permissions preset: ${args}`, 'error');
          break;
        }
    ...
    case 'fast':
      consumeComposer(input, tab, { clearAttachments: false });
      {
        const configData = await requestSocketPayload('config_read', {});
    ...
    case 'mcp':
      consumeComposer(input, tab, { clearAttachments: false });
      {
        const result = await requestSocketPayload('mcp_status', {});
    ...
    case 'apps':
      consumeComposer(input, tab, { clearAttachments: false });
      {
        const result = await requestSocketPayload('app_list', { threadId: _threadId || null });
    ...
    case 'plugins':
      consumeComposer(input, tab, { clearAttachments: false });
      {
        const result = await requestSocketPayload('plugin_list', {});
    ...
    case 'agent':
      consumeComposer(input, tab, { clearAttachments: false });
      {
        const result = await requestSocketPayload('thread_loaded_list', {});
    ...
    case 'stop':
      consumeComposer(input, tab, { clearAttachments: false });
      if (!_threadId) {
        appendSystem('There is no active thread to stop background terminals for.', 'error');
        break;
      }
      await requestSocketPayload('background_clean', { threadId: _threadId });
    ...
    case 'copy':
      consumeComposer(input, tab, { clearAttachments: false });
      {
        ...
        if (!text) {
          appendSystem('No completed Codex response is available yet.', 'error');
          break;
        }
        await copyTextToClipboard(text);
        appendSystem('Copied the latest completed Codex output.', 'muted');
      }
      break;
    case 'experimental':
      consumeComposer(input, tab, { clearAttachments: false });
      {
        const result = await requestSocketPayload('experimental_features', {});
        if (args) {
    ...
    case 'logout':
      consumeComposer(input, tab, { clearAttachments: false });
      await requestSocketPayload('account_logout', {});
      appendMarkdown('Logged out of Codex.');
      break;
    case 'diff':
      consumeComposer(input, tab, { clearAttachments: false });
      if (!_project) {
        appendSystem('Select a project before running /diff.', 'error');
        break;
      }
      {
        const diff = await requestGitDiff(_project);
    ...
    case 'plan':
      if (_running) {
        appendSystem('/plan is unavailable while a turn is running.', 'error');
        break;
      }
      if (!tab.planMode) togglePlanMode();
      if (!args) {
        consumeComposer(input, tab, { clearAttachments: false });
        appendSystem('Plan mode enabled.', 'muted');
        break;
      }
    ...
    case 'mention':
      consumeComposer(input, tab, { clearAttachments: false });
      if (!args) {
        appendMarkdown('Usage: `/mention path/to/file` to add a file path chip for the next turn.');
        break;
      }
      addPathChip(args);
      appendSystem(`Added path chip for ${args}.`, 'muted');
      break;
    case 'init':
      if (_running) {
        appendSystem('/init is unavailable while a turn is running.', 'error');
        break;
      }
      {
...
export function sendPrompt() {
  ...
  handleLocalSlashCommand(payload).then((handled) => {
    ...
    if (pendingImages?.length) {
      ...
      if (tab) tab._pendingSendImages = pendingImages;
    }
    consumeComposer(input, tab);
  }).catch((err) => {
    appendSystem(err.message || 'Command failed', 'error');
  });
}

export function queueCurrentDraft() {
  const { input, tab, prompt, images, paths } = currentComposerPayload();
  if (!input || !tab) return false;
  if (!prompt && !images.length && !paths.length) return false;
  enqueuePayload({ tab, prompt, images, paths });
  consumeComposer(input, tab);
  return true;
}
...
export function renderPathStrip() {
  ...
  strip.innerHTML = '';
  strip.hidden = paths.length === 0;
  paths.forEach((p, idx) => {
    const chip = document.createElement('span');
    chip.className = 'cxp-path-chip';
    chip.textContent = p;
    const removeBtn = document.createElement('span');
    removeBtn.className = 'cxp-path-chip-remove';
    removeBtn.textContent = '\u00D7';
    removeBtn.addEventListener('click', () => removePathChip(idx));
...
export function showSlashHints(text) {
  ...
  if (!_skillsCache) {
    ...
    _skillsPromise.then(() => {
      const input = panelEl('#cxp-input');
      if (input && input.value.trim() === text.trim()) showSlashHints(input.value.trim());
    }).catch(() => {});
    return;
  }
...
// ── Settings panel & compaction (lines 8588-8844) ──
  ...
  try {
    const [accountData, mcpData, configData, rateLimitData] = await Promise.allSettled([
      requestSocketPayload('account_read', {}),
      requestSocketPayload('mcp_status', {}),
      requestSocketPayload('config_read', {}),
      requestSocketPayload('rate_limits_read', {}),
    ]);
      ...
      if (acctName) {
        ...
        logoutBtn.addEventListener('click', async () => {
          ...
          try {
            ...
            appendSystem(err.message || 'Logout failed', 'error');
          }
        });
        ...
        loginBtn.addEventListener('click', async () => {
          ...
          try {
            const result = await requestSocketPayload('codex_login', {});
      ...
      if (!servers.length) {
        ...
        for (const server of servers) {
          ...
          refreshBtn.addEventListener('click', async () => {
            try {
              await requestSocketPayload('mcp_refresh', { name: server.name });
              refreshBtn.textContent = 'Done';
              setTimeout(() => { refreshBtn.textContent = 'Refresh'; }, 2000);
            } catch (err) {
              appendSystem(`MCP refresh failed: ${err.message}`, 'error');
            }
          });
          row.append(name, status, refreshBtn);
          if (server.requiresOAuth) {
            const loginBtn = document.createElement('button');
            loginBtn.className = 'cxp-btn cxp-btn-sm';
            loginBtn.textContent = 'Login';
            loginBtn.addEventListener('click', async () => {
              try {
                await requestSocketPayload('mcp_oauth_login', { name: server.name });
                loginBtn.textContent = 'OK';
              } catch (err) {
                appendSystem(`MCP OAuth failed: ${err.message}`, 'error');
              }
            });
    ...
    if (configEl) {
      ...
      saveBtn.addEventListener('click', async () => {
        ...
        try {
          await requestSocketPayload('config_write', { config: newCfg });
          ...
          appendSystem(`Config save failed: ${err.message}`, 'error');
          setTimeout(() => { saveBtn.textContent = 'Save Configuration'; saveBtn.disabled = false; }, 2000);
        }
      });
...
export function startCompaction() {
  ...
  setCompactingUI(true);
  sendSocket({ type: 'compact', threadId: _threadId, cwd: _project || null });
}
