...
import {
  setCallbacks, setPanelRef, setPanelState, connectTab, disconnectTab, activeTab, allTabs, isActiveTab,
  ...
  resetStallTimer, stopStallTimer, setRunning,
  handleNotify,
  appendLocalUserMessage, hydrateCodexAutomationHistory,
  clearPlanHandoffState,
  withTab,
    ...
    <div class="cxp-bottom">
      ...
      <div class="cxp-footer-toolbar">
        ...
        <div class="cxp-footer-right">
          <div class="cxp-effort-wrap">
            <button class="cxp-toolbar-toggle" id="cxp-effort-toggle" title="Reasoning effort" data-effort="off" type="button">${ICON_BRAIN}<span class="cxp-btn-label">Effort</span><span class="cxp-effort-dots"><i></i><i></i><i></i><i></i><i></i></span></button>
            <div class="cxp-effort-menu" id="cxp-effort-menu"></div>
          </div>
...
function scrollEnd(force) {
  ...
  if (!el) return;
  const controller = getStickyScrollController(el) || createStickyScrollController(el);
...
async function loadBranches(path) {
  ...
  if (!$branch) return;
  if (!path) { $branch.dataset.current = ''; ddPopulate($branch, [], ''); return; }
  try {
    const res = await fetch(`/api/terminal/branches?path=${encodeURIComponent(path)}`).then(r => r.json());
    $branch.dataset.current = res.current || '';
    const tab = activeTab();
    if (tab && tab.project === path) tab.branch = res.current || '';
...
async function loadCurrentProfile() {
  ...
  try {
    ...
    if (data.ok) {
      _defaultMcpProfile = data.defaultProfile || data.profile || 'full';
      const tab = activeTab();
      if (tab && !tab.mcpProfile) {
        tab.mcpProfile = _defaultMcpProfile;
        saveTabs();
      }
      applyMcpProfileState(tab?.mcpProfile || _defaultMcpProfile, data.presets);
      return;
    }
  } catch {}
...
function populateProfileDropdown($dd) {
  ...
  for (const p of _mcpProfiles) {
    ...
    el.addEventListener('click', async () => {
      ...
      el.classList.add('selected');
      try {
        await updateMcpProfile(p.value);
        populateProfileDropdown($dd);
      } catch (err) {
        _currentProfile = previous;
        populateProfileDropdown($dd);
        _host.appendSystem?.(err.message || 'Could not switch MCP profile', 'error');
      }
    });
    menu.appendChild(el);
  }
...
async function updateMcpProfile(profile) {
  const tab = activeTab();
  if (!tab) throw new Error('No active Codex tab');
  if (tab.running || tab.startingThread || tab.automationActive) {
    throw new Error('Wait for the active Codex turn or automation to finish before changing its MCP profile.');
  }
  const result = await requestSocketPayload('mcp_profile_set', {
    profile,
  ...
  saveTabs();
  // This selector is tab-scoped. The shared default is changed only through
  // an explicit future-runtime/default control, never as a side effect of
  // restarting the active Codex app-server.
  return effective;
}
...
function wireEvents() {
  ...
  input?.addEventListener('input', () => {
    ...
    if (tab) {
      ...
      if (tab.pendingMentions?.length) {
        const nextMentions = tab.pendingMentions.filter((mention) => input.value.includes(`@${mention.name}`));
        if (nextMentions.length !== tab.pendingMentions.length) {
          tab.pendingMentions = nextMentions;
          renderPathStrip();
        }
      }
    }
  ...
  input?.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Escape') {
      const hintsEl = panelEl('#cxp-slash-hints');
      if (hintsEl && !hintsEl.hidden) { hideComposerHints(); event.preventDefault(); return; }
      if (activeTab()?.running) { event.preventDefault(); interruptTurn(); return; }
    }
    if (event.key === 'Tab' && activeTab()?.running) {
      const tab = activeTab();
      const hasDraft = !!input.value.trim() || !!tab?.attachedImages?.length || !!tab?.pendingPaths?.length || !!tab?.pendingMentions?.length;
      if (hasDraft) {
        event.preventDefault();
        queueCurrentDraft();
        return;
  ...
  ddSetup(branchDd);
  branchDd?.addEventListener('change', async () => {
    const tab = activeTab();
    const project = tab?.project || '';
    const branch = ddGetValue(branchDd);
    const previous = branchDd.dataset.current || tab?.branch || '';
    if (!tab || !project || !branch || branch === previous) return;
    if (tab.running) {
      _host.appendSystem?.('Stop the active Codex turn before switching branches.', 'error');
      await loadBranches(project);
      return;
    }
    branchDd.classList.add('cxp-dropdown--busy');
    try {
      const statusResponse = await fetch(`/api/git/status?path=${encodeURIComponent(project)}`);
      const status = statusResponse.ok ? await statusResponse.json() : null;
      if (status?.changes?.length) {
        const confirmed = window.confirm(`Switch from ${previous || 'the current branch'} to ${branch} with ${status.changes.length} uncommitted change${status.changes.length === 1 ? '' : 's'}? Git will preserve them when possible.`);
        if (!confirmed) {
          await loadBranches(project);
          return;
        }
      ...
      const response = await fetch('/api/terminal/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ path: project, branch }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || !result.ok) throw new Error(result.error || 'Branch checkout failed');
      branchDd.dataset.current = result.branch || branch;
      for (const entry of _tabs) {
        if (entry.project === project) entry.branch = result.branch || branch;
      }
      hideComposerHints();
      _host.appendSystem?.(`Switched working tree to ${result.branch || branch}.`, 'muted');
      saveTabs();
      await loadBranches(project);
    } catch (error) {
      _host.appendSystem?.(error.message || 'Branch checkout failed', 'error');
      await loadBranches(project);
    } finally {
      branchDd.classList.remove('cxp-dropdown--busy');
    }
  });
  ...
  on('mcp:profile-changed', (msg = {}) => {
    _defaultMcpProfile = msg.defaultProfile || msg.profile || _defaultMcpProfile;
  ...
  on('changelog-saved', ({ content }) => {
    ...
    if (request?.requestId != null && request?.answerKey) {
      const isMcpChoice = request.responseKind === 'mcp';
      const responseMeta = request.responseMeta || {};
      _host.sendServerRequestReply?.(request.requestId, {
        result: isMcpChoice
          ? {
              action: 'accept',
              content: {
                [request.answerKey]: responseMeta.otherValue || '__synabun_other__',
                [responseMeta.otherField || `${request.answerKey}__other`]: editedText,
              },
            }
          : {
              answers: {
...
export async function attachCodexAutomation(run, { focus = !!run?.focus, isCancelled = () => false } = {}) {
  ...
  let tab = allTabs().find((entry) => entry.automationRunId === run.runId);
  if (!tab) {
    tab = createTab({
      id: run.runId,
  ...
  tab.mcpProfile = run.mcpProfile || tab.mcpProfile || _defaultMcpProfile;
  tab.sessionLabel = run.title || tab.sessionLabel || 'Automation';
  ...
  if (run.status === 'failed' && run.error && tab._automationLifecycleError !== run.error) {
    ...
    withTab(tab, () => appendSystem(run.error, 'error'));
  }
  saveTabs();
  renderPills();
  if (focus) {
    if (!_visible) await toggleCodexPanel();
    const idx = allTabs().indexOf(tab);
    if (idx >= 0) switchTab(idx);
  }
  if (!running && isActiveTab(tab) && _visible) connectTab(tab);
  ...
  return withTab(tab, () => {
    if (event.type === 'synabun.user_prompt') {
      ...
      if (!tab._automationPromptKeys.has(key)) {
        ...
        appendLocalUserMessage(event.text || '');
      }
    } else if (event.type === 'synabun.error') {
      appendSystem(event.message || 'Codex automation failed', 'error');
    } else if (event.type === 'turn.started') handleNotify('turn/started', { turn: { id: turnId } });
...
export async function detachCodexAutomation(runId) {
  const idx = allTabs().findIndex((entry) => entry.automationRunId === runId);
