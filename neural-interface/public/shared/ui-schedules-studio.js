...
const CLI_MODELS = {
  'claude-code': [
    { id: 'claude-opus-4-7', label: 'Opus 4.7', desc: 'Most capable' },
    { id: 'claude-sonnet-4-6', label: 'Sonnet 4.6', desc: 'Balanced', tier: 'default' },
    { id: 'claude-haiku-4-5', label: 'Haiku 4.5', desc: 'Fastest' },
  ],
  codex: [
    { id: 'gpt-5.5', label: 'GPT-5.5', desc: 'Most capable' },
    { id: 'gpt-5.4', label: 'GPT-5.4', desc: 'Strong general' },
    { id: 'gpt-5.4-mini', label: 'GPT-5.4 Mini', desc: 'Fast and cheap', tier: 'default' },
...
function getModelsForProfile(profile) {
  if (profile === 'opencode') return _opencodeModelsCache || [];
  return CLI_MODELS[profile] || [];
}

async function fetchOpencodeModels(force = false) {
  if (!force && _opencodeModelsCache) return _opencodeModelsCache;
  ...
  _opencodeModelsLoading = (async () => {
    try {
      ...
      _opencodeModelsCache = all.map(m => ({
        id: m.id || m.model || m.name,
        label: m.name || m.label || m.id || m.model,
        desc: m.provider || m.desc || '',
      })).filter(m => m.id);
...
function renderScheduleCards(container, list) {
  ...
  container.innerHTML = schedules.map(s => {
    ...
    const model = s.model ? ` &middot; ${esc(s.model)}` : '';
...
function renderTimersTab() {
  ...
  const effortOptions = EFFORT_LEVELS.map(e => `<option value="${e.id}"${e.id === _launchEffort ? ' selected' : ''}>${e.label}</option>`).join('');
  ...
  if (_launchProfile === 'opencode' && !_opencodeModelsCache) {
    fetchOpencodeModels().then(() => { if (_panel && _tab === 'timers') renderTimersTab(); });
  }
  ...
  content.innerHTML = `
    ...
    <section class="sched-tool-section">
      ...
      <div class="sched-form-grid">
        ...
        <label class="sched-field"><span>Think</span><select id="sched-qt-effort">${effortOptions}</select></label>
...
function renderModelOptions(profile, selected) {
  ...
  if (!models.length) return '<option value="" selected>Default</option>';
  const current = selected && models.some(m => m.id === selected) ? selected : '';
  return [
    `<option value=""${!current ? ' selected' : ''}>Template/default</option>`,
    ...models.map(m => `<option value="${esc(m.id)}"${m.id === current ? ' selected' : ''}>${esc(m.label || m.id)}</option>`),
  ].join('');
}
...
function renderActiveTimers() {
  ...
  const quickTimers = _quickTimers.map(timer => `
    <div class="sched-active-timer">
      <span>${esc(timer.templateName)}</span>
      <strong>${formatTimerCountdown(timer.firesAt)}</strong>
...
function renderGroupsTab() {
  ...
  const rows = _groups.map((group, idx) => {
    const count = _schedules.filter(s => s.groupId === group.id).length;
    return `
      <article class="sched-group-card" data-group-id="${esc(group.id)}">
...
function renderScheduleEditor() {
  ...
  const profileForModels = s.profile || 'claude-code';
  const modelOptions = renderModelOptions(profileForModels, s.model || '');
  const effortOptions = [
    `<option value=""${!s.effort ? ' selected' : ''}>Template/default</option>`,
    ...EFFORT_LEVELS.map(e => `<option value="${e.id}"${s.effort === e.id ? ' selected' : ''}>${e.label}</option>`),
  ].join('');
  ...
  if (profileForModels === 'opencode' && !_opencodeModelsCache) {
    fetchOpencodeModels().then(() => { if (_panel && _scheduleEditor) renderScheduleEditor(); });
  }
      ...
      <div class="sched-editor-body">
        ...
        <section class="sched-editor-section">
          ...
          <div class="sched-form-grid">
            ...
            <label class="sched-field"><span>Think</span><select id="sched-ed-effort">${effortOptions}</select></label>
...
async function saveScheduleFromEditor() {
  ...
  const effort = $('sched-ed-effort')?.value || null;
  const browserRaw = $('sched-ed-browser')?.value || '';
...
async function saveNewGroup() {
  ...
  try {
    const group = await createScheduleGroup({ name, color });
...
async function editGroup(id) {
  ...
  if (!trimmed) { showToast('Group name is required'); return; }
  try {
    const updated = await updateScheduleGroup(id, { name: trimmed });
...
async function createQuickTimerFromForm(runNow) {
  ...
  const effort = profile === 'claude-code' && effortRaw && effortRaw !== 'off' ? effortRaw : null;
  const usesBrowser = _qtUsesBrowser !== null ? _qtUsesBrowser : !!template?.usesBrowser;
  try {
    if (runNow) {
      const result = await triggerQuickTimerNow(templateId, { profile, model, effort, usesBrowser });
      showToast(`Running now: ${result.templateName}`);
    } else {
      const result = await createQuickTimer(templateId, minutes, { profile, model, effort, usesBrowser });
