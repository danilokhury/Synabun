...
const PROVIDERS = {
  ...
  opencode: {
    ...
    // OCP v2 stores a plain string[] of session IDs, now scoped per window so
    // a claimed automation cannot become an editable tab in another window.
    tabsKeyPrefix: 'opencode-v2-tabs',
    automationKeyPrefix: 'opencode-v2-automations',
    v2Plain: true,
    windowScopedPlain: true,
...
function scopedTabsKey(meta) {
  if (meta.v2Plain && !meta.windowScopedPlain) return meta.tabsKeyPrefix;
...
function isForeignActiveAutomation(tab) {
  return !!tab?.automationRunId
    && !!(tab.automationActive || tab.running)
...
function readPayload(key, meta = null) {
  if (meta?.v2Plain) {
    ...
    if (Array.isArray(arr) && arr.length) {
      const windowId = windowIdFromKey(meta, key);
      const automationRows = windowId && meta.automationKeyPrefix
        ? parseJson(storage.getItem(`${meta.automationKeyPrefix}-${windowId}`), [])
        : [];
      const automationBySession = new Map((Array.isArray(automationRows) ? automationRows : [])
        .filter((row) => row?.sessionId && row?.runId)
        .map((row) => [row.sessionId, row]));
      const tabs = arr
        .filter((s) => typeof s === 'string' && s)
        .map((sid) => {
          const automation = automationBySession.get(sid);
          return {
            sessionId: sid,
            automationRunId: automation?.runId || null,
            automationActive: !!automation?.running,
            automationOwnerId: automation?.ownerId || null,
...
function providerPayloads(provider) {
  ...
  const currentHasMeaningful = currentPayload?.tabs.some((tab) => !isForeignActiveAutomation(tab) && predicate(tab));
...
function pickMostRecentMeaningfulPayload(provider, currentKey) {
  ...
  for (let i = keys.length - 1; i >= 0; i--) {
    ...
    if (payload) {
      ...
      if (transferableTabs.some(predicate)) return { ...payload, tabs: transferableTabs };
    }
  }
  return null;
}
...
function meaningfulPills(provider) {
  ...
  for (const payload of providerPayloads(provider)) {
    payload.tabs.forEach((tab, index) => {
      if (isForeignActiveAutomation(tab)) return;
...
function writeProviderPayload(provider, payload, removeIndex) {
  ...
  if (payload.v2Plain) {
    ...
    const meta = PROVIDERS[provider];
    const windowId = windowIdFromKey(meta, payload.key);
    if (windowId && meta?.automationKeyPrefix) {
      const automationKey = `${meta.automationKeyPrefix}-${windowId}`;
      const removedSessionId = payload.tabs[removeIndex]?.sessionId;
      const rows = parseJson(storage.getItem(automationKey), []);
      const remaining = (Array.isArray(rows) ? rows : []).filter((row) => row?.sessionId !== removedSessionId);
      if (remaining.length) storage.setItem(automationKey, JSON.stringify(remaining));
      else storage.removeItem(automationKey);
    }
    return;
  }
...
function migrateSourceToCurrentWindow(provider, sourceKey) {
  ...
  const predicate = statePredicate(provider);
  const sourceMeaningful = sourceData.tabs.filter((tab) => !tab.automationRunId && predicate(tab));
  ...
  if (!currentMeaningful.length) {
    ...
    nextTabs = sourceData.tabs.filter((tab) => !tab.automationRunId);
