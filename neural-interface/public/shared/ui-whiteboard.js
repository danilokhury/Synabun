...
function persistDebounced() {
  ...
  _persistTimer = setTimeout(() => {
    ...
    if (_ws && _ws.readyState === 1) {
      ...
      // Socket down — persist via REST so a reload doesn't lose the edit.
...
async function loadPersisted() {
  // Board now lives in its own server store (data/whiteboard.json), not in the
  // client storage / ui-state PATCH cycle. Fetch it on init.
  try {
...
/** Ids of arrows anchored (start or end) to any element id in `movedIds`.
 *  Precomputed at drag start so the per-frame fast path only re-routes the
 *  handful of arrows actually pinned to the element(s) being moved. */
function _anchoredArrowIds(movedIds) {
  const set = movedIds instanceof Set ? movedIds : new Set(movedIds);
  const out = [];
  for (const a of _elements) {
    if (a.type !== 'arrow') continue;
    if (set.has(a.startAnchor) || set.has(a.endAnchor)) out.push(a.id);
  }
  return out;
}
...
// ── Drag fast path ──────────────────────────────────────────
// During a single/multi-element drag we update ONLY the affected DOM node(s)
// and ONLY the arrows anchored to them, instead of the full renderAll()
// (which restyles every element AND tears down/rebuilds the entire arrows SVG
// every mousemove). All synchronous — the 2026-05-11 rAF-batching attempt was
// reverted because deferring breaks pointer responsiveness; the win here is
// eliminating per-frame work, not scheduling it. Each helper returns false if
// a target node is missing so the caller can fall back to renderAll() once.

/** Re-route one arrow/pen's existing <path> nodes in place (hit + visible),
 *  recomputing `d` exactly as renderArrows() does. No innerHTML, no createElementNS. */
function _renderOneArrowPaths(el) {
  if (!_arrowsSvg) return false;
  const nodes = _arrowsSvg.querySelectorAll(`path[data-wb-id="${el.id}"]`);
  ...
  for (const n of nodes) n.setAttribute('d', d);
  return true;
}

/** Update a single element's DOM node via the existing updateElementDOM (which
 *  handles position, size, rotation, shape inner-SVG redraw, section recolor). */
function updateElementPosition(id) {
  if (!_elementsDiv) return false;
  const el = _findEl(id);
  if (!el) return false;
  const dom = _elementsDiv.querySelector(`[data-wb-id="${id}"]`);
  if (!dom) return false;
  updateElementDOM(dom, el);
  return true;
}

/** Per-frame drag update. `ids` = moved element ids (Set|array);
 *  `anchoredArrowIds` = arrows pinned to moved DOM elements (precomputed at
 *  drag start). Returns false → caller does a one-shot renderAll() fallback. */
function _fastDragUpdate(ids, anchoredArrowIds) {
  ...
  for (const id of idSet) {
    ...
    if (el.type === 'arrow' || el.type === 'pen') {
      if (!_renderOneArrowPaths(el)) ok = false;
    } else {
      if (!updateElementPosition(id)) ok = false;
    }
  }
  if (anchoredArrowIds) {
    for (const aid of anchoredArrowIds) {
      if (idSet.has(aid)) continue; // already updated above
      const a = _findEl(aid);
      if (a && !_renderOneArrowPaths(a)) ok = false;
    }
  }
  // Keep selection chrome tracking the element. positionContextMenu* now read
  // the cached gesture rect (_getRootRect), so this adds no per-frame layout.
  if (_ctxMenu && _selectedIds.size > 0) {
    if (_selectedIds.size === 1) {
  ...
  if (el.type === 'text' || el.type === 'list') {
    ...
    _moveEl.addEventListener('mousedown', (ev) => {
      ...
      _drag = {
        ...
        before: { ...el2 },
        anchoredArrows: _anchoredArrowIds([id]),
      };
    });
...
function positionContextMenu(el) {
  const rootRect = _getRootRect(); // cached for the gesture — no per-frame layout flush
  if (!rootRect) return;
...
function _startMultiDrag(e) {
  ...
  _drag = {
    ...
    snapshots,
    anchoredArrows: _anchoredArrowIds(_selectedIds),
  };
}
    ...
    if (e.target.closest('.wb-resize-handle') && targetId) {
      ...
      if (el) {
        ...
        _drag = {
          ...
          before: { ...el },
          anchoredArrows: _anchoredArrowIds([targetId]),
        };
        e.preventDefault();
        return;
      }
...
function onMouseMove(e) {
  ...
  // While actively dragging/drawing, suppress ambient transitions + glow/pulse
  // animations (consumed by the body.ui-interacting CSS kill-switch at
  // styles.css:7231). classList.add is idempotent — cheap to call each move.
  if (_drag || _marquee || _shapeCreating || _sectionCreating || _penPoints) {
  ...
  if (_drag?.type === 'move-multi') {
    ...
    if (!_fastDragUpdate(_drag.snapshots.map(s => s.id), _drag.anchoredArrows)) renderAll();
    return;
  }
  ...
  if (_drag?.type === 'move') {
    ...
    if (el) {
      ...
      if (!_fastDragUpdate([_drag.id], _drag.anchoredArrows)) renderAll();
    }
    return;
  }
  ...
  if (_drag?.type === 'move-arrow') {
    ...
    if (el && el.points) {
      ...
      el.startAnchor = null;
      el.endAnchor = null;
      if (!_fastDragUpdate([_drag.id], null)) renderAll();
    }
    return;
  }
...
export function initWhiteboard() {
  ...
  // Render immediately (likely empty) for an instant paint, then again once the
  // server returns the saved board (loadPersisted is async — fetches /api/whiteboard).
  renderAll();
  updateUndoButtons();
  loadPersisted().then(() => { renderAll(); updateUndoButtons(); });
