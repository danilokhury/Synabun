...
/**
 * Inject a file path into a terminal as if the user pasted it.
 * Uses xterm/htmlTerm `.paste()` so bracketed-paste mode is honored when the
 * running CLI (Claude Code, Codex, etc.) enables it — raw `ws.send({input})`
 * bypasses that wrapping and the CLI silently drops or misinterprets the input.
 * Falls back to raw input only if no terminal paste method is available.
 */
function _injectPathIntoTerm(term, ws, path) {
  try {
    if (term && typeof term.paste === 'function') { term.paste(path); return; }
  } catch {}
...
const PROFILES = [
  ...
  { id: 'opencode',    label: 'OpenCode',    svg: SVG_OPENCODE, color: '#E8E0DC' },
...
const _lastNotified = new Map();    // sessionId → last NOTIF_TYPE fired (ACTION/DONE), cleared on IDLE/WORKING-with-real-work
...
function _notifyStatusChange(sessionId, newStatus) {
  ...
  // Reset dedup guard when session leaves a notified state (back to IDLE).
  // WORKING is not a reset — transient output bursts during an ACTION prompt
  // re-flip to WORKING in _scheduleCliStatusCheck, and we must not re-notify.
  if (newStatus === CLI_STATUS.IDLE) _lastNotified.delete(sessionId);

  if (!prev || prev !== CLI_STATUS.WORKING) return; // only notify on Working → X
  if (newStatus === CLI_STATUS.WORKING) return;

  // Dedup: don't re-fire the same notification type until the session has
  // visited IDLE (i.e., user resolved / prompt cleared).
  if (newStatus === CLI_STATUS.ACTION && _lastNotified.get(sessionId) === NOTIF_TYPE.ACTION) return;
  ...
  if (newStatus === CLI_STATUS.ACTION) {
    notify('cli', NOTIF_TYPE.ACTION, label, { sessionId, provider: session?.profile || 'claude-code' });
    _lastNotified.set(sessionId, NOTIF_TYPE.ACTION);
  } else if (newStatus === CLI_STATUS.DONE) {
    notify('cli', NOTIF_TYPE.DONE, label, { sessionId, provider: session?.profile || 'claude-code' });
    _lastNotified.set(sessionId, NOTIF_TYPE.DONE);
  }
}

// Status-detection regexes — hoisted so the poller doesn't recompile them per tick
const STATUS_SCAN_ROWS = 15;
const RE_ACTION_ALLOW = /\bAllow\b/i;
const RE_ACTION_CHOICES = /\b(Yes|No|Always)\b/;
const RE_ACTION_YN = /\(y\/n\)/i;
const RE_ACTION_PROCEED = /\bDo you want to proceed\b/i;
const RE_TUI_PROMPT_BOX = /[\u2502\u2503\u2551]\s*[❯>\u276F]\s*[\u2502\u2503\u2551]/;
const RE_BOX_DRAWING_ONLY = /^[\u2500-\u257F\s]+$/;
const RE_SHELL_PROMPT = /[❯>\u276F$%]\s*$/;

function _getSessionBuffer(session) {
  const xtermBuffer = session?.term?.buffer?.active;
  if (xtermBuffer) {
    return {
      buffer: xtermBuffer,
      rows: session.term.rows,
      // baseY = scrollback offset of the viewport top. getLine() is
      // scrollback-absolute while cursorY is viewport-relative — every read
      // must add baseY or status scans read ancient history once scrollback fills.
      baseY: xtermBuffer.baseY || 0,
      cursorY: xtermBuffer.cursorY,
    };
  }
  return null;
}

function _getBufferText(bufferState, row) {
  if (!bufferState?.buffer) return '';
  const line = bufferState.buffer.getLine(bufferState.baseY + row);
  return line?.translateToString(true) || '';
}

function _detectSessionStatus(session) {
  const bufferState = _getSessionBuffer(session);
  if (!bufferState || session.dead) return CLI_STATUS.OFF;
  const cursorY = bufferState.cursorY;

  // Read the bottom of the visible viewport — prompts and approval dialogs
  // live there; scanning the full screen costs ~2× more for no extra signal.
  const lines = [];
  for (let r = Math.max(0, bufferState.rows - STATUS_SCAN_ROWS); r < bufferState.rows; r++) {
    lines.push(_getBufferText(bufferState, r));
  }
  const recent = lines.join('\n');

  // Action: permission prompts / approval needed
  if (RE_ACTION_ALLOW.test(recent) && RE_ACTION_CHOICES.test(recent)) {
    return CLI_STATUS.ACTION;
  }
  if (RE_ACTION_YN.test(recent) || RE_ACTION_PROCEED.test(recent)) {
    return CLI_STATUS.ACTION;
  }
  ...
  for (let r = cursorY; r >= Math.max(0, cursorY - 3); r--) {
    const line = _getBufferText(bufferState, r);
    if (RE_TUI_PROMPT_BOX.test(line)) {
      let gapEmpty = true;
      for (let g = r + 1; g <= cursorY; g++) {
        const gl = _getBufferText(bufferState, g).trim();
        if (gl.length > 0 && !RE_BOX_DRAWING_ONLY.test(gl)) { gapEmpty = false; break; }
      }
  ...
  for (let r = cursorY; r >= Math.max(0, cursorY - 3); r--) {
    const line = _getBufferText(bufferState, r);
    if (RE_SHELL_PROMPT.test(line) && line.trim().length < 80) {
      // If prompt found above cursor, verify lines between are empty
      let gapEmpty = true;
      for (let g = r + 1; g <= cursorY; g++) {
        if (_getBufferText(bufferState, g).trim().length > 0) { gapEmpty = false; break; }
      }
      if (gapEmpty) return CLI_STATUS.IDLE;
    }
  }
...
// Badge container refs, cached per session. Tab-bar innerHTML rebuilds orphan
// the nodes — _getBadgeContainers re-queries any slot whose node left the DOM.
const _badgeCache = new Map(); // sessionId → { tab, float, pill }

function _getBadgeContainers(sessionId) {
  let refs = _badgeCache.get(sessionId);
  if (!refs) { refs = { tab: null, float: null, pill: null }; _badgeCache.set(sessionId, refs); }
  if (!refs.tab?.isConnected) {
    refs.tab = document.querySelector(`.term-tab[data-session-id="${sessionId}"]`);
  }
  if (!refs.float?.isConnected) {
    refs.float = document.querySelector(`.term-float-tab[data-session-id="${sessionId}"] .term-float-tab-header`);
  }
  if (!refs.pill?.isConnected) {
    refs.pill = document.querySelector(`.term-minimized-pill[data-session-id="${sessionId}"]`);
  }
  return refs;
}

function _setBadgeStatus(container, status) {
  if (!container) return;
  const badge = _ensureBadge(container);
  if (badge.dataset.status === status) return; // unchanged — skip label write
...
function _updateSessionBadges(sessionId, status) {
  ...
  const refs = _getBadgeContainers(sessionId);
  _setBadgeStatus(refs.tab, status);
  _setBadgeStatus(refs.float, status);
  // Minimized pill badge + icon swap on done
  if (refs.pill) {
    _setBadgeStatus(refs.pill, status);
    const ico = refs.pill.querySelector('.term-minimized-pill-icon');
    if (ico) {
...
function _scheduleCliStatusCheck(sessionId) {
  ...
  // New output supersedes any pending stale-spinner recheck
  if (tracked.idleTimer) { clearTimeout(tracked.idleTimer); tracked.idleTimer = null; }
  ...
  tracked.timer = setTimeout(() => {
    ...
    // Still WORKING with no new output → arm a one-shot recheck at the 5s
    // stale-spinner boundary instead of waiting for the slow safety sweep.
    if (st === CLI_STATUS.WORKING && tracked.lastOutput > 0) {
      const delay = Math.max(250, tracked.lastOutput + 5000 - Date.now());
      tracked.idleTimer = setTimeout(() => {
        tracked.idleTimer = null;
        const s2 = _sessions.find(s => s.id === sessionId);
        if (!s2 || tracked.status !== CLI_STATUS.WORKING) return;
        if (Date.now() - tracked.lastOutput < 4500) return; // output resumed
        let st2 = _detectSessionStatus(s2);
        if (st2 === CLI_STATUS.WORKING) st2 = CLI_STATUS.IDLE; // stale spinner
        tracked.status = st2;
...
// event-driven (output → _scheduleCliStatusCheck → 600ms debounce + one-shot
// stale-spinner recheck); this slow sweep only catches edge cases.
let _cliPollTimer = null;

function _ensureCliPoller() {
  if (_cliPollTimer) return;
  _cliPollTimer = setInterval(() => {
    if (_cliSessionStatus.size === 0) {
      clearInterval(_cliPollTimer);
      _cliPollTimer = null;
      return;
    }
...
function _trackCliSession(sessionId) {
  _cliSessionStatus.set(sessionId, { status: CLI_STATUS.IDLE, lastOutput: 0, timer: null, idleTimer: null });
...
// ── Floating terminal MCP profile dropdown ──

let _floatProfileCache = null;
let _floatProfileCacheTime = 0;

async function _fetchFloatProfiles() {
  const now = Date.now();
  if (_floatProfileCache && now - _floatProfileCacheTime < 60000) return _floatProfileCache;
  try {
    const resp = await fetch('/api/mcp/profile');
    const data = await resp.json();
    if (data.ok) {
      _floatProfileCache = { current: data.profile || 'full', presets: data.presets || {} };
      _floatProfileCacheTime = now;
    }
  } catch {}
  return _floatProfileCache || { current: 'full', presets: {} };
}

function _initFloatProfileDropdown(dd) {
  const btn = dd.querySelector('.term-float-profile-btn');
  const menu = dd.querySelector('.term-float-profile-menu');
  const label = dd.querySelector('.term-float-profile-label');

  // Load current profile label
  _fetchFloatProfiles().then(data => {
    const p = data.presets[data.current];
    label.textContent = p?.label || data.current;
  });

  btn.addEventListener('click', async (e) => {
    e.stopPropagation();
    const wasOpen = menu.classList.contains('open');
    // Close all other open profile menus
    document.querySelectorAll('.term-float-profile-menu.open').forEach(m => m.classList.remove('open'));
    if (wasOpen) return;

    const data = await _fetchFloatProfiles();
    menu.innerHTML = '';
    for (const [id, preset] of Object.entries(data.presets)) {
      const opt = document.createElement('div');
      opt.className = 'term-float-profile-option' + (id === data.current ? ' selected' : '');
      opt.innerHTML = `<span class="tfp-name">${preset.label || id}</span><span class="tfp-hint">${preset.tools} tools</span>`;
      opt.addEventListener('click', async (ev) => {
        ...
        try {
          ...
          _floatProfileCache = null; // bust cache
          // Update all other floating profile dropdowns
          document.querySelectorAll('.term-float-profile-label').forEach(lbl => {
            lbl.textContent = preset.label || id;
          });
        } catch {}
      });
    ...
    const close = (ev) => {
      if (!dd.contains(ev.target)) { menu.classList.remove('open'); document.removeEventListener('mousedown', close); }
    };
    setTimeout(() => document.addEventListener('mousedown', close), 0);
  });
}

// ── Floating terminal Recall profile dropdown ──

const FLOAT_RECALL_META = [
...
let _floatRecallCache = null;
let _floatRecallCacheTime = 0;

async function _fetchFloatRecallProfile() {
  const now = Date.now();
  if (_floatRecallCache && now - _floatRecallCacheTime < 10000) return _floatRecallCache;
  try {
    const data = await fetch('/api/display-settings').then(r => r.json());
    _floatRecallCache = { profile: data.profile || 'balanced', recallDefaults: data.recallDefaults };
    _floatRecallCacheTime = now;
  } catch {}
  return _floatRecallCache || { profile: 'balanced', recallDefaults: FLOAT_RECALL_DEFAULTS.balanced };
}

function _initFloatRecallDropdown(dd) {
  const btn = dd.querySelector('.term-float-recall-btn');
  const menu = dd.querySelector('.term-float-recall-menu');
  const label = dd.querySelector('.term-float-recall-label');

  _fetchFloatRecallProfile().then(data => {
    const matched = FLOAT_RECALL_META.find(p => p.value === data.profile);
    label.textContent = matched ? matched.label : data.profile;
  });

  btn.addEventListener('click', async (e) => {
    e.stopPropagation();
    const wasOpen = menu.classList.contains('open');
    document.querySelectorAll('.term-float-recall-menu.open').forEach(m => m.classList.remove('open'));
    if (wasOpen) return;

    const data = await _fetchFloatRecallProfile();
    menu.innerHTML = '';
    for (const p of FLOAT_RECALL_META) {
      const opt = document.createElement('div');
      opt.className = 'term-float-recall-option' + (p.value === data.profile ? ' selected' : '');
      opt.innerHTML = `<span class="tfr-name">${p.label}</span><span class="tfr-hint">${p.hint}</span>`;
      opt.addEventListener('click', async (ev) => {
        ev.stopPropagation();
        menu.classList.remove('open');
        if (p.value === data.profile) return;
        label.textContent = p.label;
        try {
          const current = await fetch('/api/display-settings').then(r => r.json());
          const defaults = FLOAT_RECALL_DEFAULTS[p.value] || current.recallDefaults || FLOAT_RECALL_DEFAULTS.balanced;
          await fetch('/api/display-settings', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...current, profile: p.value, recallDefaults: p.value !== 'custom' ? defaults : current.recallDefaults }),
          });
          _floatRecallCache = null;
          document.querySelectorAll('.term-float-recall-label').forEach(lbl => {
...
function _scheduleFit(session, _retries) {
  if (!session?.fitAddon || session.dead || session._isBrowser) return;
  if (_fitPending.has(session.id)) return;
  // During edge-drag: WebGL terminals refit every frame (GPU fit is cheap —
  // this is the per-frame native resize feel). DOM-fallback terminals coalesce
  // to 1 per ~4 rAF (~64ms) because each fit triggers a full DOM re-measure
  // over the entire scrollback — the single biggest glitch source on long
  // sessions. PTY resize is debounced below via _resizeTimers either way.
  if (_draggingResize && !_retries) {
    const stride = hasWebgl(session) ? 1 : 4;
    const tick = (session._dragFitTick || 0) + 1;
    session._dragFitTick = tick;
  ...
  const handle = requestAnimationFrame(() => {
    ...
    try {
      ...
      // (Claude Code, Ink, vim) don't get spammed with SIGWINCH per frame.
      const last = session._lastSentResize;
      if (last && last.cols === cols && last.rows === rows) return;

      const send = () => {
        if (session.ws?.readyState !== WebSocket.OPEN) return;
        const c = session.term.cols, r = session.term.rows;
        const l = session._lastSentResize;
        if (l && l.cols === c && l.rows === r) return;
        session._lastSentResize = { cols: c, rows: r };
        session.ws.send(JSON.stringify({ type: 'resize', cols: c, rows: r }));
      };

      if (_draggingResize) {
        // During drag: coalesce cell-boundary crossings into one resize per window.
  ...
  try {
    ...
    if (cols >= 2 && rows >= 2) {
      ...
      if (!l || l.cols !== cols || l.rows !== rows) {
        session._lastSentResize = { cols, rows };
        session.ws.send(JSON.stringify({ type: 'resize', cols, rows }));
      }
    }
    // Final refresh on drag end — repaints the whole visible grid into the new
    // cell geometry so any sub-cell glitches from coalesced drag fits are gone.
    session._dragFitTick = 0;
    try { session.term.refresh(0, Math.max(0, (session.term.rows || 1) - 1)); } catch {}
  } catch {}
}
...
// openSession (fresh PTY) and reconnectSession (existing PTY) build identical
// terminals; these helpers are the single source of truth so the paths can't
// drift. Path-specific concerns (PTY spawn, WS URL timing, replay flags,
// panel/tab-bar behavior) stay in the callers.

// WebGL makes deep scrollback cheap (no DOM re-measure on resize); DOM-fallback
...
function _wireTermSession({ sessionId, profile, term, viewport, writeTerminal, xtermTextarea, ws }) {
  ...
  if (xtermTextarea) {
    xtermTextarea.addEventListener('paste', (e) => {
      const w = liveWs();
      if (!e.clipboardData || w?.readyState !== WebSocket.OPEN) return;
        ...
        if (item.type.startsWith('image/')) {
          ...
          reader.onload = () => {
            ...
            if (w2?.readyState === WebSocket.OPEN) {
              w2.send(JSON.stringify({ type: 'image_paste', data: base64, mimeType: item.type }));
            }
          };
...
// ── Shared WS message dispatch ──
// One handler for all three terminal WS paths (openSession, reconnectSession,
// _reconnectTerminalWs). Sessions register output watchers (Set of fns) on
// session._outputWatchers — used by _sendOnceReady so readiness detection
// survives WS swaps.

function _accumReplay(sess, text) {
  if (!sess || !text) return;
  if (!sess._replayBuf) sess._replayBuf = '';
  sess._replayBuf += text;
...
function _handleTermWsEvent(e, ctx) {
  const { sessionId, profile } = ctx;

  // Session is closing (teardown deferred a frame) — drop in-flight frames so
  // they can't write into a terminal about to be disposed. Covers both the
  // spliced-out lookup and the ctx.writeTerminal/ctx.term fallback below.
  ...
  try {
    ...
    const sess = _sessions.find(s => s.id === sessionId);
    const term = sess?.term || ctx.term;
    const writeTerminal = sess?._terminalWriter || ctx.writeTerminal;
    const ws = sess?.ws || ctx.ws;

    if (msg.type === 'output' || msg.type === 'replay' || msg.type === 'snapshot') {
      const text = (msg.type === 'snapshot' && msg.plain) ? msg.plain : msg.data;
    ...
    if (msg.type === 'error') {
      ...
      if (writeTerminal) writeTerminal(`\r\n\x1b[31mError: ${msg.message}\x1b[0m\r\n`);
      else term?.write(`\r\n\x1b[31mError: ${msg.message}\x1b[0m\r\n`);
    }
    if (msg.type === 'image_saved' && msg.path) {
      _clipCopy(msg.path);
      _injectPathIntoTerm(term, ws, msg.path);
...
function _isSessionVisible(s) {
  ...
  if (!s.viewport?.isConnected) return false;
  if (s.viewport.style.display === 'none') return false;
  const tabState = _detachedTabs.get(s.id);
  if (tabState) return !tabState.minimized;          // floating window
...
// ── Lazy-load xterm.js (vendored locally — see scripts/build-vendor.mjs) ──

const VENDOR_XTERM_VERSION = 'xterm-6.0.0';
...
async function loadXterm() {
  ...
  if (!_xtermCSS) {
    ...
    link.rel = 'stylesheet';
    link.href = `/vendor/xterm/${VENDOR_XTERM_VERSION}.css`;
    document.head.appendChild(link);
    _xtermCSS = true;
  }

  try {
    const m = await import(`/vendor/xterm/${VENDOR_XTERM_VERSION}.min.js`);
    _Terminal = m.Terminal;
    _FitAddon = m.FitAddon;
    _WebLinksAddon = m.WebLinksAddon;
    _SearchAddon = m.SearchAddon;
    _Unicode11Addon = m.Unicode11Addon;
    _WebglAddon = m.WebglAddon;
    initRendererManager({ WebglAddon: m.WebglAddon, isVisible: _isSessionVisible });
  } catch (err) {
    // Show error in terminal container if it exists, or throw
    const container = $('term-container');
    if (container) {
      const msg = err?.message || 'Failed to load terminal bundle';
      ...
      errorEl.textContent = `⚠ ${msg}\n\nTerminal bundle missing — run: npm run build:vendor`;
...
function initFloatDrag() {
  ...
  function _movePanelDrag(x, y) {
    ...
    // Transform during drag (composited, no layout); committed on dragend
    _floatDrag.lastL = finalL;
    _floatDrag.lastT = finalT;
    _panel.style.transform = `translate3d(${Math.round(finalL - _floatDrag.startL)}px, ${Math.round(finalT - _floatDrag.startT)}px, 0)`;
  }
  function _endPanelDrag() {
    if (!_floatDrag) return;
    _flushPanelDrag();
    if (_panel && _floatDrag.lastL != null) {
      _panel.style.left = Math.round(_floatDrag.lastL) + 'px';
      _panel.style.top = Math.round(_floatDrag.lastT) + 'px';
    }
...
async function openSession(profile, cwd, existingSessionId, options = {}) {
  ...
  // PTY spawn is deferred for new sessions: spawning at hardcoded 120×30 then
  // sending a startup SIGWINCH causes Ink TUIs (Claude Code, OpenCode) to
  ...
  // the right width, and Claude Code/Ink doesn't see a startup SIGWINCH
  // that would re-render its splash banner.
  let _initCols = 120;
  let _initRows = 30;
  try {
    fitAddon.fit();
  ...
  if (_targetPane) _paneAssignments.set(sessionId, _targetPane);

  // Connect WebSocket (binary frames = raw PTY bytes)
  ...
  const _wsCtx = { sessionId, profile, term, writeTerminal, ws };
  ws.onmessage = (e) => _handleTermWsEvent(e, _wsCtx);
  ...
  const session = {
    ...
    // Seed with the spawn dimensions so _scheduleFit's first fire (which will
    // measure the same cells) is a no-op rather than an extra resize roundtrip.
...
async function openBrowserSession(url, fresh, _unused, force) {
  ...
  // Attach (CDP) mode → tabs open in the user's real, persistent Chrome window.
  // Never build the in-app floating/screencast viewport for it.
  if (profileMode === 'cdp') {
    console.log('[browser] Opened tab in real Chrome:', sessionId, '→', startUrl);
    notify('cli', NOTIF_TYPE.DONE, `Opened a tab in your Chrome → ${new URL(startUrl).hostname}`, { provider: 'browser' });
    _opening = false;
    return;
  }
  ...
  if (_streamOff) {
    ...
    notify('cli', NOTIF_TYPE.DONE, `Browser opened → ${new URL(startUrl).hostname}`, { provider: 'browser' });
    _opening = false;
    return;
  }
...
// Defer heavy close teardown until after the browser paints the removal. The
// session is already marked dead, untracked, spliced, and its float/viewport
// are display:none, so nothing live reads it in the gap (in-flight PTY frames
// are dropped via the _closingIds guard in _handleTermWsEvent). rAF guarantees
// a paint; the nested setTimeout(0) yields that frame so the GPU/xterm dispose
// can't steal time from it.
function _deferTeardown(fn) {
...
async function closeSession(idx) {
  ...
  // Stays set through the deferred teardown so in-flight frames are dropped.
  if (_closingIds.has(session.id)) return;
  _closingIds.add(session.id);

  // ── Synchronous phase: make it vanish + make it safe (all cheap) ──
  // Detached float: hide instantly so the compositor stops blurring/shadowing
  // it THIS frame; the real DOM detach + listener cleanup happen in teardown.
  const tabState = _detachedTabs.get(session.id);
  if (tabState) {
    if (tabState.el) tabState.el.style.display = 'none';
    if (tabState.pill) tabState.pill.style.display = 'none';
    _detachedTabs.delete(session.id);
  }
  const hadFloatNeighbors = _detachedTabs.size > 0;

  // Mark dead + untrack so nothing live touches it during the teardown gap.
  session.dead = true;
  _untrackCliSession(session.id);
  clearTimeout(session._idleTimer);
  // Hide the viewport now (a docked viewport is not inside the float, so it
  // would otherwise linger visible until the deferred remove()).
  if (session.viewport) session.viewport.style.display = 'none';

  // Heavy teardown — deferred to after the next paint. Preserves the original
  // dispose ordering (disposeRenderer BEFORE term.dispose so GPU frees cleanly).
  const _teardown = () => {
    ...
    if (tabState) {
      ...
      if (tabState.el) tabState.el.remove();
    ...
    // Recompute edge-snapping only for floats that remain (skip when none do).
    if (hadFloatNeighbors) _updateSnappedEdges();
    _closingIds.delete(session.id);
  };

  // Remove from sessions array BEFORE the (deferred) server delete —
  // prevents the dead tab from flashing in the tab bar.
  const currentIdx = _sessions.indexOf(session);
  if (currentIdx < 0) { _deferTeardown(_teardown); return; }
...
let _lastFocusedBrowserId = null;
function _applyBrowserFocusThrottle(focusedSessionId) {
  if (focusedSessionId) _lastFocusedBrowserId = focusedSessionId;
  for (const s of _sessions) {
...
function getRightPanelReservedWidth(cs = getComputedStyle(document.documentElement)) {
  return parseFloat(cs.getPropertyValue('--right-panel-width'))
    || parseFloat(cs.getPropertyValue('--claude-panel-width'))
    || 0;
}

/** Update snapped-edge classes on all floating tabs based on adjacency */
/** Compute viewport/panel snap boundaries (getComputedStyle reads — cache during drag) */
function _snapEnv() {
  const cs = getComputedStyle(document.documentElement);
  ...
  return {
    leftPanelEdge: explorerW + fileExplorerW,
    rightPanelEdge: window.innerWidth - getRightPanelReservedWidth(cs),
    topEdge: parseFloat(cs.getPropertyValue('--navbar-height')) || 48,
    bottomEdge: window.innerHeight,
  };
}

// During drag, _moveDrag publishes synthetic rects + a cached env here so the
// every-8th-frame snap check is pure arithmetic — zero DOM reads per frame.
let _dragSnapOverride = null;

function _updateSnappedEdges(rectsOverride, envOverride) {
  const TOLERANCE = 2;
  const rects = rectsOverride || new Map();
  if (!rectsOverride) {
  ...
  const { leftPanelEdge, rightPanelEdge, topEdge, bottomEdge } = envOverride || _snapEnv();
...
let _snappedEdgesSkip = 0;
function _scheduleSnappedEdgesUpdate() {
  if (_snappedEdgesRaf) return;
  // During drag every 8th frame is plenty; with _dragSnapOverride set the
  // update is pure arithmetic (no DOM reads). _flushSnappedEdgesUpdate runs
  // the exact live-read state on release.
  if (++_snappedEdgesSkip % 8 !== 0) return;
  _snappedEdgesRaf = requestAnimationFrame(() => {
    _snappedEdgesRaf = 0;
    _updateSnappedEdges(_dragSnapOverride?.rects, _dragSnapOverride?.env);
  });
}
...
function _flushSnappedEdgesUpdate() {
  ...
  _snappedEdgesSkip = 0;
  _dragSnapOverride = null; // safety net for drags interrupted by close/dock
  _updateSnappedEdges();
}
...
export function tileFloatingTerminals() {
  ...
  const rightPanelW = getRightPanelReservedWidth(cs);
  ...
  const areaT = TOP_PAD;
  const areaW = window.innerWidth - sidebarW - rightPanelW;
...
function detachTab(idx) {
  ...
  // Create floating window (no .glass — terminal body must stay opaque;
  // the header carries its own glass treatment)
  const win = document.createElement('div');
  win.className = 'term-float-tab';
  ...
  win.innerHTML = `
    <div class="term-float-tab-header">
      ...
      ${CLI_PROFILES_NEED_RECALL.has(session.profile) ? `<div class="term-float-recall-dd" data-tooltip="Recall profile">
        <button class="term-float-recall-btn"><span class="term-float-recall-label">...</span><span class="term-float-recall-arrow">&#x25BE;</span></button>
        <div class="term-float-recall-menu"></div>
      </div>` : ''}
  ...
  // Wire MCP profile dropdown (CLI profiles only)
  const profileDd = win.querySelector('.term-float-profile-dd');
  if (profileDd) {
    _initFloatProfileDropdown(profileDd);
  }

  // Wire recall profile dropdown
  const recallDd = win.querySelector('.term-float-recall-dd');
  if (recallDd) _initFloatRecallDropdown(recallDd);
...
function initTabFloatDrag(win, sessionId) {
  ...
  function _checkHeader(target) {
    ...
    if (!header || target.closest('button, .term-float-profile-dd')) return false;
    const session = _sessions.find(s => s.id === sessionId);
    return !session?.pinned;
  }
  function _startDrag(x, y) {
    const r = win.getBoundingClientRect();
    drag = { startX: x, startY: y, startL: r.left, startT: r.top, startW: r.width, startH: r.height, lastL: r.left, lastT: r.top };
    // Neighbors don't move during this drag — snapshot their rects once
    // instead of calling getBoundingClientRect per tab per frame in _moveDrag
    drag.neighborRects = [];
    drag.neighborRectMap = new Map();
    for (const [sid, dt] of _detachedTabs) {
      if (sid === sessionId || !dt.el || dt.minimized) continue;
      const nr = dt.el.getBoundingClientRect();
      drag.neighborRects.push(nr);
      drag.neighborRectMap.set(sid, nr);
    }
    // Snap-boundary env can't change mid-drag — read getComputedStyle once
    drag.snapEnv = _snapEnv();
  ...
  function _moveDrag(x, y) {
    ...
    // Transform-based positioning: left/top force layout every frame; a
    // composited translate3d doesn't. Integer deltas keep the WebGL canvas
    // pixel-snapped. Final left/top are committed once on dragend.
    drag.lastL = finalL;
    drag.lastT = finalT;
    win.style.transform = `translate3d(${Math.round(finalL - drag.startL)}px, ${Math.round(finalT - drag.startT)}px, 0)`;
    // Publish synthetic rects so the snap-edge pass needs zero DOM reads
    const myRect = { left: finalL, top: finalT, right: finalL + w, bottom: finalT + h };
    const rects = new Map(drag.neighborRectMap);
    rects.set(sessionId, myRect);
    _dragSnapOverride = { rects, env: drag.snapEnv };
  ...
  function _endDrag() {
    if (!drag) return;
    _flushDragFrame();
    // Commit final position to left/top and clear the drag transform
    win.style.left = Math.round(drag.lastL) + 'px';
    win.style.top = Math.round(drag.lastT) + 'px';
    win.style.transform = '';
    drag = null;
    _dragSnapOverride = null;
...
export async function initTerminal() {
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      ...
      for (const s of _sessions) {
        if (!s._isBrowser || s.dead) continue;
        if (_detachedTabs.get(s.id)?.minimized) continue; // already paused
        if (s.ws?.readyState === WebSocket.OPEN) {
          s.ws.send(JSON.stringify({ type: 'throttle-screencast' }));
        }
      }
    ...
    const active = _sessions[_activeIdx];
    if (_sessions.some(s => s._isBrowser && !s.dead)) {
  ...
  registerAction('launch-opencode', () => launchDetached('opencode'));
...
async function attachDetached(terminalSessionId, profile, initialMessage, autoSubmit, snapToPanel) {
  ...
  try {
    ...
    if (existingIdx < 0) {
      // No existing session — create one. CLI profiles go through xterm.js (WebGL/Canvas).
      if (CLI_PROFILES.has(p)) {
        console.log('[SynaBun] attachDetached: creating via reconnectSession (xterm)');
        await reconnectSession(terminalSessionId, p);
      } else {
    ...
    // If snapToPanel requested, position the floating tab next to the active right-side panel
    if (snapToPanel && idx >= 0) {
      const dt = _detachedTabs.get(terminalSessionId);
      if (dt?.el) {
        const panelWidth = getRightPanelReservedWidth();
        if (panelWidth > 0) {
...
function getGitOutputTab() {
  ...
  const term = new _Terminal(XTERM_OPTIONS({
    cursorBlink: false,
    disableStdin: true,
    scrollback: 2000,
  }));
