...
const _projectsByProvider = new Map();
...
export function initResume() {
  ...
  // Warm the saved provider after boot so opening Resume is normally a pure
  // render. The server returns persistent metadata immediately and reconciles
  // transcript changes in the background.
  const prefetch = () => {
    if (!_currentSearch && !_projectsByProvider.has(_selectedProvider)) loadAllProjects();
  };
  if (typeof requestIdleCallback === 'function') requestIdleCallback(prefetch, { timeout: 1500 });
...
async function renderResume() {
  ...
  const cached = !_currentSearch ? _projectsByProvider.get(_selectedProvider) : null;
  if (cached !== undefined) {
    _cachedProjects = cached;
    renderProjectList(container);
    // Revalidate without replacing useful content with a loading state.
...
async function loadAllProjects(search = '') {
  ...
  const loadKey = `${provider}\n${search}`;
  if (_loadPromises.has(loadKey)) return _loadPromises.get(loadKey);
  ...
  const loadPromise = (async () => {
    ...
    try {
      ...
      let projects;
      if (search && search.trim().length >= 2 && provider !== 'gemini') {
        const data = await searchSessions({
          q: search,
          provider,
          limit: 50,
        });
        projects = (data.projects || []).filter(p => p.sessions && p.sessions.length > 0);
      } else {
        const fetchFn = PROVIDER_FETCH[provider] || fetchClaudeSessions;
        const data = await fetchFn({ limit: 50, search });
        projects = data.projects || [];
      }

      if (!search) _projectsByProvider.set(provider, projects);
      if (_selectedProvider === provider && _currentSearch === search) {
        _cachedProjects = projects;
      }
      return projects;
    } catch (err) {
      console.error('[resume] Failed to load sessions:', err);
      if (_selectedProvider === provider && _currentSearch === search && !_cachedProjects) {
        _cachedProjects = [];
      }
      return _cachedProjects || [];
    } finally {
      if (_selectedProvider === provider && _currentSearch === search) _loading = false;
    }
  })();

  _loadPromises.set(loadKey, loadPromise);
  try {
    return await loadPromise;
  } finally {
    if (_loadPromises.get(loadKey) === loadPromise) _loadPromises.delete(loadKey);
  }
}
...
function renderProviderBar(container) {
  ...
  for (const id of RESUME_PROVIDERS) {
    ...
    btn.addEventListener('click', (e) => {
      ...
      _cachedProjects = _projectsByProvider.get(id) || null;
...
function renderProjectList(container) {
  ...
  if (!searchBox) {
    ...
    refreshBtn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (!_currentSearch) _projectsByProvider.set(_selectedProvider, _cachedProjects);
      } catch (err) {
  ...
  if (allSessions.length > 0 && !_currentSearch) {
      ...
      <div class="resume-continue-row">
        ...
        <div class="resume-continue-text">
          ...
          <div class="resume-continue-meta">
            ...
            ${_selectedProvider === 'codex' && latest.accountLabel
              ? `<span class="resume-project-badge" title="Codex account">${escHtml(latest.accountLabel)}</span>`
              : ''}
    ...
    continueEl.addEventListener('click', () => {
      closeMenu();
      launchResume(latest.sessionId, latest.projectPath, latest.accountId);
    });
...
function renderSession(s, projectPath) {
  ...
  const accountHtml = _selectedProvider === 'codex' && s.accountLabel
    ? `<span class="resume-project-badge" title="Codex account">${escHtml(s.accountLabel)}</span>`
    : '';
  ...
  item.addEventListener('click', (e) => {
    ...
    closeMenu();
    launchResume(s.sessionId, projectPath, s.accountId);
  });
    ...
    convResults.forEach(r => {
      ...
      if (sessionId) {
        ...
        item.addEventListener('click', () => {
          ...
          if (proj) launchResume(sessionId, proj.path, r.payload?.account_id);
        });
      }
...
async function launchResume(sessionId, projectPath, accountId = null) {
  ...
  emit('terminal:open-resume', {
    ...
    resume: sessionId,
    label: label || '',
    accountId: _selectedProvider === 'codex' ? (accountId || 'default') : null,
  });
}
