...
let _activeChild = null;        // { panelId, sessionId, store, instance, wsUnsub }
let _started = false;
...
function ensureChildSpawn(parentSid, childSid, childInfo, replay, taskInfo) {
  ...
  // Don't spawn a child for ourselves (event echo).
  if (parentSid === childSid) return;
...
function spawnChildPanel({ childSessionId, childInfo, parentPanelId, parentSessionId, parentTitle, replay, taskInfo }) {
  ...
  const childRunSub = store.subscribe((event, state) => {
    ...
    import('./ocp-v2-plan.js').then(({ maybeFinalizePlanTurn }) => {
      maybeFinalizePlanTurn('child:idle', _primary.store).catch(() => {});
    }).catch(() => {});
  });
  ...
  _activeChild = { panelId, sessionId: childSessionId, store, instance, wsUnsub, runSub: childRunSub };
