...
export function mountCompose(rootEl, store = getDefaultStore(), opts = {}) {
  ...
  async function sendTextMessage(text, options = {}) {
    ...
    const s = store.getState();
    if (opts.canSend && opts.canSend(s) === false) {
    ...
    const notifyManualSend = (phase) => {
      try { opts.onManualSend?.(s, phase, sendMeta); } catch (err) {
    ...
    try {
      ...
      if (isPlanTurn) {
        await maybeFinalizePlanTurn('send:complete', store, { lenient: true });
      }
      store.setRunning(false);
      return true;
    } catch (err) {
  ...
  async function waitForAsyncTurn({ pollHandle, drainOnce, isPlanTurn }) {
    ...
    await new Promise((resolve) => {
      cleanup = async () => {
        ...
        if (isPlanTurn) await maybeFinalizePlanTurn('async:idle', store, { lenient: true });
        resolve();
      };
  ...
  async function doAbort() {
    ...
    if (opts.onAbort && await opts.onAbort(s)) {
