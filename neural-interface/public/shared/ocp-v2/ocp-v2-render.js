...
export function mountRenderer(containerEl, store = getDefaultStore()) {
  ...
  // Sticky auto-follow: stays on until the user scrolls away from the bottom.
  // render() reads this flag to decide whether to pin to scrollHeight.
  _container.__ocpFollow = true;
  const NEAR_BOTTOM_PX = 60;
  const evaluateUserScroll = () => {
    if (!_container) return;
    const distance = _container.scrollHeight - _container.scrollTop - _container.clientHeight;
    _container.__ocpFollow = distance < NEAR_BOTTOM_PX;
  };
  const onUserScroll = () => { requestAnimationFrame(evaluateUserScroll); };
  const onUserKey = (e) => {
    if (e.key === 'PageUp' || e.key === 'PageDown' || e.key === 'ArrowUp'
      || e.key === 'ArrowDown' || e.key === 'Home' || e.key === 'End'
      || e.key === ' ') onUserScroll();
  };
  _container.addEventListener('wheel', onUserScroll, { passive: true });
  _container.addEventListener('touchmove', onUserScroll, { passive: true });
  _container.addEventListener('keydown', onUserKey);

  const doRender = () => render(_container, store);
  ...
  const instance = {
    ...
    unmount: () => {
      ...
      if (_container) {
        _container.removeEventListener('wheel', onUserScroll);
        _container.removeEventListener('touchmove', onUserScroll);
        _container.removeEventListener('keydown', onUserKey);
      }
...
function render(_container, store) {
  ...
  // Sticky follow flag set in mountRenderer — flips off only when the user
  // actually scrolls away from the bottom (wheel / touch / arrow keys).
  // Default true so the first render after content arrives still pins.
  const follow = _container.__ocpFollow !== false;
  ...
  if (follow) {
    // 'instant' bypasses the CSS scroll-behavior: smooth so streaming chunks
    // don't queue half-finished animations. Two rAFs handle late-mounted
    // children (markdown, code blocks) that grow scrollHeight after layout.
    const stick = () => {
      if (!_container) return;
      try {
        _container.scrollTo({ top: _container.scrollHeight, behavior: 'instant' });
      } catch {
...
function renderPermissionCard(perm, store) {
  ...
  const allowOnce = button('Allow once', 'ocpv2-perm-allow', async () => {
    lock();
    await safeReply(perm, 'once', store);
  });
  const allowAlways = button('Always allow', 'ocpv2-perm-allow', async () => {
    lock();
    await safeReply(perm, 'always', store);
  });
  const reject = button('Reject', 'ocpv2-perm-reject', async () => {
    lock();
    await safeReply(perm, 'reject', store);
  });
  ...
  reject.addEventListener('click', async () => {
    ...
    try {
      if (req._viaToolPart) {
        ...
        await api.questionReject({ requestId: req.id });
      }
...
async function submitAnswers(requestOrId, sectionAnswers, store, instantQIdx, instantLabel) {
  ...
  try {
    ...
    const replyResp = await api.questionReply({ requestId: replyRequestId, answers, cwd: getQuestionDirectory(store) });
...
async function resolveQuestionReplyRequestId(req, fallbackRequestId, store) {
  ...
  const s = store.getState();
  const listResp = await api.questionList({ cwd: getQuestionDirectory(store) });
