...
import { truncate, debounce, rafThrottle } from './utils.js';
...
// Reused across all memory rows — constructing an Intl formatter once is far
// cheaper than `new Date().toLocaleString(...)` (which builds a formatter on
// every call) when rendering hundreds of memories per rebuild.
const _dateFmt = new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' });

// Render-time indices so a single delegated listener on the tree container can
// recover the category item / memory node for a clicked row, instead of every
// row carrying its own click/drag closures (hundreds, re-created on each rebuild).
// Rebuilt by buildTreeImpl on every render; keyed by the data-cat / data-id the
// rows already carry, so delegation keeps working under virtualization too.
const _catIndex = new Map();  // category name → { item, isParent }
const _memIndex = new Map();  // memory id → node

// Virtualization: the tree is flattened (respecting collapse + filter) into a
// flat list of row descriptors. Small trees render every row; large trees render
// only a window around the viewport, so paint/layout AND DOM construction stay
// bounded no matter how many memories the store holds.
let _flatRows = [];            // [{ kind:'category'|'memory', depth, item|node, isParent, name }]
let _virtActive = false;       // true when the windowed renderer is in use
let _rowH = 24;                // row height in px (CSS min-height; calibrated from a real row)
let _rowHMeasured = false;
const VIRT_THRESHOLD = 200;    // flat-row count above which windowing kicks in
...
// Render entry points. `buildTreeImpl` does the actual DOM work; callers should
// prefer the coalesced wrappers below so that a burst of rebuilds (several
// event-bus emits in one tick, or rapid keystrokes) collapses into a single
// rebuild instead of N full reflows.
//   • scheduleBuildTree   — rAF-coalesced; one rebuild per frame at most
//   • debouncedFilterBuild — trailing debounce for the filter input
// First-open calls buildTreeImpl directly so the panel paints this frame.
function buildTreeImpl() {
  const container = $('explorer-tree');
  if (!container) return;

  // Reset the delegation indices; flattenTree repopulates them for every visible row.
  _catIndex.clear();
  _memIndex.clear();

  const treeData = buildTreeData();

  // Footer counts — computed over the full subtree (collapsed categories still
  // count their memories), independent of collapse state and the render window.
  let totalMemories = 0;
  let visibleMemories = 0;
  for (const item of treeData) {
    totalMemories += countMemories(item);
    visibleMemories += countFilteredMemories(item);
  }
  const countEl = $('explorer-count');
  if (countEl) {
    countEl.textContent = _filterText
      ? `${visibleMemories} / ${totalMemories} memories`
      : `${totalMemories} memories`;
  }

  // Flatten the visible hierarchy into row descriptors.
  _flatRows = flattenTree(treeData);

  // Wire delegated interaction + drag-and-drop (once per container element).
  if (!container._dragWired) {
    initExplorerDrag(container);
    container.addEventListener('click', onTreeClick);
    container.addEventListener('dragstart', onTreeDragStart);
    container.addEventListener('dragend', onTreeDragEnd);
    container._dragWired = true;
  }

  if (_flatRows.length > VIRT_THRESHOLD) {
    // Windowed render — only the rows near the viewport are built/painted.
    _virtActive = true;
    ensureVirtualDom(container);
    container._vsizer.style.height = `${_flatRows.length * _rowH}px`;
    if (container.firstElementChild !== container._vsizer) {
      container.replaceChildren(container._vsizer);
    }
    container._vwindow._start = container._vwindow._end = -1;  // force re-render
    renderVirtualWindow(container);
  } else {
    // Small tree — render every row in a single off-DOM fragment swap.
    _virtActive = false;
    const frag = document.createDocumentFragment();
    for (const row of _flatRows) frag.appendChild(renderFlatRow(row));
    container.replaceChildren(frag);
  }

  // Highlight selected node (windowed rows are tagged on render; this also clears
  // any stale selection in the non-virtualized path).
  if (state.selectedNodeId) {
    highlightNode(state.selectedNodeId, false);
  }
}

const scheduleBuildTree = rafThrottle(buildTreeImpl);
const debouncedFilterBuild = debounce(buildTreeImpl, 120);

// Flatten the (filtered, collapse-aware) category/memory hierarchy into a flat
// list of row descriptors, populating the delegation indices for every visible
// row. Children of collapsed categories are skipped. Preserves the original
// order: a category, then its child categories, then its direct memories.
  ...
  const walk = (items, depth) => {
    for (const item of items) {
      ...
      const filteredChildCount = item.children
        ...
        : 0;
      // Skip categories with nothing matching the active filter.
      if (_filterText && (filteredMemories.length + filteredChildCount) === 0) continue;

      const isParent = item.type === 'parent';
      _catIndex.set(item.name, { item, isParent });
      rows.push({ kind: 'category', item, isParent, depth });

      // Only descend into expanded categories.
      if (!_collapsed.has(item.name)) {
        if (item.children) walk(item.children, depth + 1);
        for (const mem of filteredMemories) {
          _memIndex.set(mem.id, mem);
          rows.push({ kind: 'memory', node: mem, depth: depth + 1 });
        }
      }
    }
  };
...
function renderFlatRow(row) {
  return row.kind === 'memory'
    ? renderMemoryItem(row.node, row.depth)
    : renderCategoryRow(row.item, row.depth, row.isParent);
}

// One category row (no children container — the hierarchy lives in _flatRows).
function renderCategoryRow(item, depth, isParent) {
  const filteredMemories = item.memories ? item.memories.filter(matchesFilter) : [];
  const filteredChildCount = item.children
    ? item.children.reduce((sum, c) => sum + countFilteredMemories(c), 0)
    : 0;
  ...
  el.dataset.expanded = String(!_collapsed.has(item.name));

  const row = document.createElement('div');
  row.className = 'explorer-row';
  row.style.paddingLeft = `${10 + depth * 16}px`;

  // Chevron (only when there's expandable content)
  ...
  dot.style.background = catColor(item.name);
  ...
  badge.textContent = _filterText ? `${totalFiltered}/${totalMemories}` : String(totalMemories);
  row.appendChild(badge);

  if (!state.activeCategories.has(item.name)) el.classList.add('explorer-hidden-cat');
  ...
  el.appendChild(row);
  // All clicks handled by the delegated onTreeClick listener.
  return el;
}

// ── Windowed (virtualized) rendering ────────
// A tall sizer creates the scrollbar; an absolutely-positioned window holds only
// the rows near the viewport, translated into place. Recomputed on rebuild and on
// scroll (rAF-throttled).
function ensureVirtualDom(container) {
  if (container._vsizer) return;
  const sizer = document.createElement('div');
  sizer.className = 'explorer-vsizer';
  sizer.style.position = 'relative';
  sizer.style.width = '100%';
  const win = document.createElement('div');
  win.className = 'explorer-vwindow';
  win.style.position = 'absolute';
  win.style.top = '0';
  win.style.left = '0';
  win.style.right = '0';
  win._start = win._end = -1;
  sizer.appendChild(win);
  container._vsizer = sizer;
  container._vwindow = win;
  if (!container._vscrollWired) {
    container.addEventListener('scroll', rafThrottle(() => {
      if (_virtActive) renderVirtualWindow(container);
    }), { passive: true });
    container._vscrollWired = true;
  }
}

function renderVirtualWindow(container) {
  const sizer = container._vsizer, win = container._vwindow;
  if (!sizer || !win) return;
  const total = _flatRows.length;
  const viewportH = container.clientHeight || 400;
  const visCount = Math.ceil(viewportH / _rowH);
  const start = Math.max(0, Math.floor(container.scrollTop / _rowH) - VIRT_BUFFER);
  const end = Math.min(total, start + visCount + VIRT_BUFFER * 2);
  if (win._start === start && win._end === end) return;  // window unchanged
  win._start = start; win._end = end;

  const frag = document.createDocumentFragment();
  for (let i = start; i < end; i++) frag.appendChild(renderFlatRow(_flatRows[i]));
  win.replaceChildren(frag);
  win.style.transform = `translateY(${start * _rowH}px)`;

  // One-time calibration of row height against a real row (handles zoom / CSS drift).
  if (!_rowHMeasured && win.firstElementChild) {
    _rowHMeasured = true;
    const h = win.firstElementChild.getBoundingClientRect().height;
    if (h > 0 && Math.abs(h - _rowH) > 0.5) {
      _rowH = h;
      sizer.style.height = `${total * _rowH}px`;
      win._start = win._end = -1;  // force recompute with corrected height
      renderVirtualWindow(container);
    }
  }
}
...
function renderMemoryItem(node, depth) {
  ...
  const created = node.payload.created_at ? _dateFmt.format(new Date(node.payload.created_at)) : '';
  ...
  // Row click (open detail + navigate graph) and drag-to-terminal are handled by
  // the delegated listeners on the container. We still mark the node draggable so
  // the browser initiates native HTML5 drag; the delegated dragstart reads the id.
  if (!guestReadOnly()) {
    el.draggable = true;
  }
...
// ═══════════════════════════════════════════
// DELEGATED TREE INTERACTION
// ═══════════════════════════════════════════
// One set of listeners on the #explorer-tree container handles every row,
// recovering the category/memory via data-cat/data-id + the render-time indices.
// Replaces the per-row closures that were re-created on every rebuild, and keeps
// working under virtualization (rows are recovered by attribute, not by closure).

function onTreeClick(e) {
  // Color dot → open color picker for the category.
  const dotEl = e.target.closest('.explorer-dot');
  if (dotEl) {
    e.stopPropagation();
    const nodeEl = dotEl.closest('.explorer-node');
    const entry = nodeEl && _catIndex.get(nodeEl.dataset.cat);
    if (entry) openColorPicker(entry.item.name, nodeEl);
    return;
  }

  // "..." button → category context menu.
  const moreBtn = e.target.closest('.explorer-more');
  if (moreBtn) {
    e.stopPropagation();
    const nodeEl = moreBtn.closest('.explorer-node');
    const entry = nodeEl && _catIndex.get(nodeEl.dataset.cat);
    if (!entry) return;
    const { item, isParent } = entry;
    const eyeState = {
      toggle: () => {
        ...
        if (state.activeCategories.has(catName)) {
          ...
          nodeEl.classList.add('explorer-hidden-cat');
          scheduleGraphRemoval();
        } else {
          targets.forEach(c => state.activeCategories.add(c));
          nodeEl.classList.remove('explorer-hidden-cat');
  ...
  // Memory row → open detail panel + navigate graph.
  const memEl = e.target.closest('.explorer-memory');
  if (memEl) {
    e.stopPropagation();
    const node = _memIndex.get(memEl.dataset.id);
    if (!node) return;
  ...
  // Category row → toggle collapse. In the flat model children are separate rows,
  // so re-flatten + re-render to show/hide them (rAF-coalesced).
  const catNodeEl = e.target.closest('.explorer-node');
  if (catNodeEl && e.target.closest('.explorer-row')) {
    e.stopPropagation();
    const wasExpanded = catNodeEl.dataset.expanded === 'true';
    catNodeEl.dataset.expanded = String(!wasExpanded);  // instant chevron feedback
    const catName = catNodeEl.dataset.cat;
    if (wasExpanded) _collapsed.add(catName);
    else _collapsed.delete(catName);
    saveCollapsed();
    scheduleBuildTree();
  }
}
...
function onTreeDragStart(e) {
  const memEl = e.target.closest('.explorer-memory');
  if (!memEl) return;
  e.dataTransfer.setData('application/x-synabun-memory', memEl.dataset.id);
  e.dataTransfer.effectAllowed = 'copy';
  memEl.classList.add('dragging');
}

function onTreeDragEnd(e) {
  const memEl = e.target.closest('.explorer-memory');
  if (memEl) memEl.classList.remove('dragging');
}
...
function highlightNode(nodeId, scrollIntoView = true) {
  ...
  // When scrolling to a node, make sure it's visible: if it isn't in the current
  // flattened list, its category (or parent category) is collapsed — expand and
  // rebuild. Do this BEFORE clearing/applying selection so the rebuild's own
  // selection pass doesn't leave a stale highlight.
  if (nodeId && scrollIntoView && !_flatRows.some(r => r.kind === 'memory' && r.node.id === nodeId)) {
    const node = state.allNodes?.find(n => n.id === nodeId);
    const cat = node?.payload?.category || 'uncategorized';
    let changed = _collapsed.delete(cat);
    const parent = state.categoryMetadata?.[cat]?.parent;
    if (parent && _collapsed.delete(parent)) changed = true;
    if (changed) { saveCollapsed(); buildTreeImpl(); }
  }

  // Remove previous highlight (after any rebuild above).
  ...
  // In windowed mode the target row may be outside the rendered slice — scroll it
  // into the viewport and re-render the window so the element exists in the DOM.
  if (_virtActive && scrollIntoView) {
    const idx = _flatRows.findIndex(r => r.kind === 'memory' && r.node.id === nodeId);
    if (idx >= 0) {
      const rowTop = idx * _rowH;
      const viewH = container.clientHeight || 400;
      if (rowTop < container.scrollTop || rowTop + _rowH > container.scrollTop + viewH) {
        container.scrollTop = Math.max(0, rowTop - Math.floor(viewH / 2));
      }
      renderVirtualWindow(container);
    }
  }
  ...
  // Non-virtualized rows live in normal flow — native scrollIntoView is smoothest.
  if (scrollIntoView && !_virtActive) {
...
function collapseAll() {
  // Collapse every category by name (walk the data, not the DOM — under windowed
  // rendering only a slice of rows exists in the DOM).
  const addNames = (items) => {
    for (const it of items) {
      if (it.name) _collapsed.add(it.name);
      if (it.children) addNames(it.children);
    }
  };
  addNames(buildTreeData());
  saveCollapsed();
  scheduleBuildTree();
}
