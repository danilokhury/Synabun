...
export async function sendMessage(content, options = {}) {
  ...
  if (options.clearQueue !== false) {
    questionsModule.clearQuestionQueueAndCards(tab, msgContainer);
  }
...
export function abortTab(tab, container = null) {
  if (!tab?.running) return false;
  // Capture the active question request id BEFORE clearing state — we send
  // question:reject so the server-side pending question is dismissed (not just
  // the local turn aborted). Otherwise the same request stays pending and the
  // next turn can collide with it.
  const activeQId = questionsModule.activeQuestionToolId(tab);
  if (tab.sessionId) {
  ...
  if (container) {
    ...
    // Remove any visible question cards AND clear in-memory state in one call.
    // Done before question:reject so the activeQuestionToolId used below is
    // captured in activeQId above.
    try { questionsModule.clearQuestionQueueAndCards(tab, container); } catch {}
  } else {
    try { questionsModule.clearQuestionQueue(tab); } catch {}
  }
  tab._planStallSoftCardOpen = false;
  tab.running = false;
  tab.turnStartedAt = 0;
  ...
  planModule.clearWatchdogs(tab);
  questionsModule.clearQuestionReplyWatchdog(tab);
  if (activeQId && tab.sessionId) {
    try {
      requestWs('question:reject', { requestID: activeQId, sessionId: tab.sessionId }, 5000)
        .catch((err) => { console.warn('[ocp-send] question:reject on abort failed:', err?.message || err); });
    } catch {}
  }
  return true;
}
