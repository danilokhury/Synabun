...
export function renderQuestionCard(container, toolName, toolInput, toolId, { onAnswer, onSkip } = {}) {
  ...
  for (let qi = 0; qi < questions.length; qi++) {
    ...
    if (q.options.length) {
      ...
      // Hoist customInput reference so option click handlers can clear it
      // without hitting the const TDZ. Stays null until the custom input is
        ...
        btn.addEventListener('click', () => {
          if (q.multiSelect) {
            ...
            optsEl.querySelectorAll('.ocp-question-option.selected').forEach(b => {
              ...
              if (lbl) selected.push(b.dataset.value || lbl.textContent);
            });
      ...
      // Always-visible custom input when q.custom !== false (matches v2 + Claude UX)
      if (q.custom) {
        const customWrap = document.createElement('div');
        customWrap.className = 'ocp-question-custom';
        const customHint = document.createElement('div');
        customHint.className = 'ocp-question-custom-hint';
        customHint.textContent = 'Or type your own answer\u2026';
        customInput = document.createElement('input');
        customInput.type = 'text';
        customInput.className = 'ocp-question-other-input';
        customInput.placeholder = 'Type your answer\u2026';
        customInput.addEventListener('input', () => {
          const val = customInput.value.trim();
          if (val) {
            // Typed answer wins — clear any option selection
            optsEl.querySelectorAll('.ocp-question-option.selected').forEach(b => b.classList.remove('selected'));
            pendingAnswers[questionKey] = val;
          } else {
            delete pendingAnswers[questionKey];
          }
          updateSubmitState();
        });
      ...
      // No options — free-text input (always visible)
      const customWrap = document.createElement('div');
      customWrap.className = 'ocp-question-custom';
      ...
      customWrap.appendChild(textInput);
      block.appendChild(customWrap);
    }
  ...
  skipBtn.className = 'ocp-question-skip';
  skipBtn.type = 'button';
...
function scrollToBottom(container) {
  if (!container) return;
  const controller = getStickyScrollController(container) || createStickyScrollController(container);
