...
/**
 * Clear the question queue AND remove any visible question cards from the DOM
 * for the given container. Pass `container` (the `#ocp-messages` element)
 * when available; otherwise only the in-memory state is cleared. This is the
 * one-call cleanup used by `abortTab` and `sendMessage` to ensure the sidepanel
 * never shows a stale locked/locked card after a turn ends.
 */
export function clearQuestionQueueAndCards(tab, container = null) {
  clearQuestionQueue(tab);
  if (!container) return;
  try {
    container.querySelectorAll('.ocp-question-card').forEach((el) => el.remove());
  } catch {}
}
...
export function processQuestionQueue(tab, container) {
  ...
  renderQuestionCard(container, toolName, toolInput, toolId, {
    ...
    onSkip: () => {
      // Legacy tool-part questions have no server-side QuestionRequest —
      // just abort the in-flight turn and clear the card from the DOM.
      const cardEl = container?.querySelector(`.ocp-question-card[data-tool-id="${CSS.escape(String(toolId))}"]`);
      if (cardEl) cardEl.remove();
      if (activeQuestionToolId(tab) === toolId) setActiveQuestionToolId(tab, null);
      _deps.setTurnStatus(tab, 'Skipped question', 'Skipped');
      _deps.update();
    },
  });
...
export function renderOpencodeQuestion(tab, container, event, requestID) {
  ...
  renderQuestionCard(container, 'question', toolInput, requestID, {
    ...
    onSkip: () => {
      // Remove the card from the DOM and clear active state immediately so the
      // UI doesn't stay stuck on a locked card for the rest of the session.
      const cardEl = container?.querySelector(`.ocp-question-card[data-tool-id="${CSS.escape(String(requestID))}"]`);
      if (cardEl) cardEl.remove();
      if (activeQuestionToolId(tab) === requestID) {
        setActiveQuestionToolId(tab, null);
      }
      trace('q:skip', { sid: tab.sessionId, reqId: requestID });
      _deps.requestWs('question:reject', { requestID, sessionId: tab.sessionId || '' }, 10000)
        .then((resp) => {
          if (resp?.status && resp.status >= 400) {
            console.warn('[ocp-questions] question:reject non-2xx:', resp.status);
          }
        })
        .catch((err) => {
          console.warn('[ocp-questions] question:reject failed:', err?.message || err);
        })
        .finally(() => {
          _deps.setTurnStatus(tab, 'Skipped question', 'Skipped');
          processQuestionQueue(tab, container);
