...
const _modelLoadAttempts = new Set();
...
let _qtAccountsRequested = false; // lazy-fetch guard for quick-timer account chips
let _mcpProfilePresets = null; // cached presets from API

// Render the Codex account selector chips (one button per saved ChatGPT account).
function renderAccountChips(selectedId) {
  const accounts = getCachedCodexAccounts();
  const sel = accounts.some(a => a.id === selectedId) ? selectedId : 'default';
  return accounts.map(a => `
    <button class="as-launch-account-chip${a.id === sel ? ' active' : ''}" data-account="${_escLaunch(a.id)}" title="${_escLaunch(a.email || a.label || a.id)}">
      <span class="as-launch-account-name">${_escLaunch(a.label || a.email || (a.isDefault ? 'Default' : 'Account'))}</span>
      ${a.planType ? `<span class="as-launch-account-org">${_escLaunch(a.planType)}</span>` : ''}
    </button>
  `).join('');
}

function wireAccountChips(container) {
  container.querySelectorAll('.as-launch-account-chip').forEach(btn => {
    btn.addEventListener('click', () => {
      container.querySelectorAll('.as-launch-account-chip').forEach(b => b.classList.remove('active'));
...
// Load accounts from the server and (re)render the chips for the active launch panel.
function loadLaunchAccounts(root) {
  fetchCodexAccounts().then(accounts => {
    if (!_launchPanelActive || _launchProfile !== 'codex') return;
    if (!accounts.some(a => a.id === _launchAccount)) {
      _launchAccount = 'default';
      storage.setItem('as-launch-account', _launchAccount);
    }
    const box = root.querySelector('#as-launch-accounts');
    if (box) {
      box.innerHTML = renderAccountChips(_launchAccount);
...
async function fetchMcpProfiles() {
  ...
  try {
    const presets = await fetchMcpProfilePresets();
    if (presets && Object.keys(presets).length) {
      _mcpProfilePresets = presets;
      ...
      <div class="as-launch-inline-body">
        ...
        <div class="as-launch-section" id="as-launch-effort-section" style="${profileSupportsEffort(profile) ? '' : 'display:none'}">
          ...
          <div class="as-launch-efforts" id="as-launch-efforts">
            ${renderEffortChips(profile, _launchEffort)}
          </div>
        </div>
        ...
        <div class="as-launch-section" id="as-launch-account-section" style="${profile === 'codex' ? '' : 'display:none'}">
          <label class="as-launch-label">OpenAI Account</label>
          <div class="as-launch-accounts" id="as-launch-accounts">
            ${renderAccountChips(_launchAccount)}
          </div>
        </div>
...
function dynamicModelLoadStarted(profileId) {
  ...
  for (const key of _modelLoadRequests) {
    if (String(key).endsWith(`:${profileId}`) || String(key) === profileId) return true;
  }
  return false;
}

function clearDynamicModelLoadState(profileId) {
  _modelLoadAttempts.delete(profileId);
  for (const key of [..._modelLoadRequests]) {
    if (String(key).endsWith(`:${profileId}`) || String(key) === profileId) {
      _modelLoadRequests.delete(key);
    }
  }
}

if (typeof document !== 'undefined') {
  document.addEventListener('ocp-providers-changed', () => clearDynamicModelLoadState('opencode'));
}

function dynamicModelEmptyText(profileId) {
  const loaded = dynamicModelLoadStarted(profileId);
  if (profileId === 'opencode') {
    return loaded
      ? 'No OpenCode models found — start OpenCode or connect a provider in Settings → OpenCode.'
...
function renderModelChips(profileId, selectedModel) {
  ...
  return models.map(m => `
    <button class="as-launch-model${modelMatchesSelector(m, selectedModel) ? ' active' : ''}${m.tier ? ` as-launch-model--${m.tier}` : ''}" data-model="${modelSelectorValue(m)}">
...
function renderLaunchSelect(type, options, selectedId, opts = {}) {
  ...
  const sel = options.find(o => modelMatchesSelector(o, selectedId)) || options[0];
  const selValue = modelSelectorValue(sel);
  ...
  const renderOpt = o => `
    <button class="as-launch-select-option${modelSelectorValue(o) === selValue ? ' active' : ''}${o.tier ? ` as-launch-select-option--${o.tier}` : ''}" type="button" data-value="${esc(modelSelectorValue(o))}" data-search="${searchFor(o)}">
...
function renderEffortChips(profileId, selectedEffort) {
  return getEffortLevelsForProfile(profileId).map(e => `
...
function wireLaunchInline(container) {
  ...
  root.querySelectorAll('.as-launch-profile').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // Show/hide effort section based on profile.
      const effortSection = root.querySelector('#as-launch-effort-section');
      if (effortSection) {
        effortSection.style.display = profileSupportsEffort(profileId) ? '' : 'none';
        const effortBox = root.querySelector('#as-launch-efforts');
        if (effortBox) {
          _launchEffort = 'off';
          storage.setItem('as-launch-effort', _launchEffort);
          effortBox.innerHTML = renderEffortChips(profileId, _launchEffort);
      ...
      // Show/hide OpenAI account section (Codex only — selects which ChatGPT login the loop runs under)
      const accountSection = root.querySelector('#as-launch-account-section');
      if (accountSection) {
        accountSection.style.display = profileId === 'codex' ? '' : 'none';
        if (profileId === 'codex') loadLaunchAccounts(root);
      }
    });
  });
...
async function confirmLaunch() {
  ...
  try {
    ...
    // Launch as floating terminal + floating browser. The server now returns
    // immediately after registering the run (status: 'starting') so the
    // toast updates within ~100ms; the provider identity and stream events
    // arrive via the sidepanel:run-updated WebSocket event after that.
    ...
    if (result.surface === 'sidepanel') {
      emit('terminal:attach-floating', {});
      // routeNativeLoopRun is safe to call with a "starting" descriptor — the
      // sidepanel router will show a placeholder and pick up identity / events
      // as they stream in via the WebSocket.
      await routeNativeLoopRun(result.run || result, { focus: true });
      const profileLabel = CLI_PROFILES.find(p => p.id === params.profile)?.label || params.profile;
      const startedToast = result.run?.status === 'starting'
        ? `Loop queued in ${profileLabel} — connecting to provider…`
        : `Loop started in ${profileLabel}`;
        ...
        <div class="as-runtime-row">
          ...
          <div class="as-runtime-field" id="as-f-effort-field" style="${profileSupportsEffort(tProfile) ? '' : 'display:none'}">
            ...
            <select class="as-input as-select" id="as-f-effort">
              ${getEffortLevelsForProfile(tProfile).map(e => `<option value="${esc(e.id)}"${e.id === tEffort ? ' selected' : ''}>${esc(e.label)} \u2014 ${esc(e.desc)}</option>`).join('')}
            </select>
          </div>
...
function wireLaunchDefaults(isPreset) {
  ...
  const renderEffortOptions = (profileId, preferredId) => {
    ...
    const selectedId = preferredId && levels.some(e => e.id === preferredId) ? preferredId : 'off';
    effortSel.innerHTML = levels.map(e =>
      `<option value="${esc(e.id)}"${e.id === selectedId ? ' selected' : ''}>${esc(e.label)} \u2014 ${esc(e.desc)}</option>`
    ).join('');
  };

  const refreshDynamicModelOptionsIfNeeded = (profileId, preferredId) => {
    if (!isDynamicModelProfile(profileId)) return;
    const key = `detail:${profileId}`;
  ...
  renderEffortOptions(profileSel.value, effortSel?.value || 'off');
  if (effortField) effortField.style.display = profileSupportsEffort(profileSel.value) ? '' : 'none';
  refreshDynamicModelOptionsIfNeeded(profileSel.value, modelSel?.value || null);
}
...
function renderSchedulesMain() {
  ...
  const qtModelChips = qtModels.length
    ? (qtModels.length > LAUNCH_CHIP_MAX
        ...
        : qtModels.map(m => `
        <button class="as-launch-model${modelMatchesSelector(m, qtCurrentModel) ? ' active' : ''}${m.tier ? ` as-launch-model--${m.tier}` : ''}" data-action="qt-model" data-model="${modelSelectorValue(m)}">
          <span class="as-launch-model-name">${m.label}</span>
          <span class="as-launch-model-desc">${m.desc}</span>
        </button>
      `).join(''))
    : `<span class="as-launch-no-models">${esc(dynamicModelEmptyText(qtProfile))}</span>`;

  const qtEffortChips = getEffortLevelsForProfile(qtProfile).map(e => `
  ...
  // Codex account chips (which ChatGPT login the timer fires under). Lazy-load on first codex render.
  if (qtProfile === 'codex' && !_qtAccountsRequested) {
    _qtAccountsRequested = true;
    fetchCodexAccounts().then(() => { if ((_launchProfile || 'claude-code') === 'codex') renderSchedulesMain(); });
  }
  const qtAccountChips = getCachedCodexAccounts().map(a => `
    <button class="as-launch-account-chip${a.id === (_launchAccount || 'default') ? ' active' : ''}" data-action="qt-account" data-account="${esc(a.id)}" title="${esc(a.email || a.label || a.id)}">
      <span class="as-launch-account-name">${esc(a.label || a.email || (a.isDefault ? 'Default' : 'Account'))}</span>
      ${a.planType ? `<span class="as-launch-account-org">${esc(a.planType)}</span>` : ''}
    </button>
  `).join('');
  ...
  const qtCreatorHTML = `
    <div class="as-qt-creator">
      ...
      <div class="as-qt-group" id="as-qt-account-section" style="${qtProfile === 'codex' ? '' : 'display:none'}">
        <span class="as-launch-label">OpenAI Account</span>
        <div class="as-launch-accounts" id="as-qt-accounts">${qtAccountChips}</div>
      </div>
      <div class="as-qt-group">
