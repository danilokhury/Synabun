...
import { rafThrottle } from '../../shared/utils.js';
...
// Cached container rect — reading getBoundingClientRect() on every pointermove
// forces a synchronous reflow. The graph canvas only moves on resize (sidepanel
// reserve dispatches a window 'resize'), so cache it and invalidate in _onResize.
let _containerRect = null;
function _invalidateContainerRect() { _containerRect = null; }
function _getContainerRect() {
  if (!_containerRect) {
    _containerRect = _graphContainer
...
let _bloomPass = null;
const BLOOM_RES_DIVISOR = 4;

// Idle throttling — full rate while interacting/animating, reduced when still.
// Cosmetic breathing/shimmer keep ticking at IDLE_FPS so the scene stays alive.
const IDLE_AFTER_MS = 2000;
const IDLE_FPS = 24;
let _externallyStopped = false;   // stopAnimation() called (viz toggle / focus mode)
let _lastActivityTime = 0;
let _lastIdleRenderTime = 0;
...
// `_instancesDirty` = full O(N) rebuild pending (data/search/category/drag).
// `_partialDirtyIds` = a handful of memory nodes whose hover/select state
// toggled — refreshed in O(k) without re-looping all nodes. A pending full
// rebuild always wins and clears the partial set (see _updateMemoryNodesInstanced).
let _instancesDirty = true;
const _partialDirtyIds = new Set();
function _markNodeDirty(id) {
  if (id != null) _partialDirtyIds.add(id);
}
...
export function saveNodePositions() {
  ...
  try { posStore.setItem(_posStorageKey, JSON.stringify(positions)); } catch (_) {}
}

function restoreNodePositions(nodes) {
  try {
    const saved = JSON.parse(posStore.getItem(_posStorageKey) || '{}');
...
export function applyGraphData() {
  ...
  const savedPositions = JSON.parse(posStore.getItem(_posStorageKey) || '{}');
...
function _onPointerMove(e) {
  ...
  // Hover picking (raycast over anchors + O(N) ray-sphere over memory nodes) is
  // expensive; a fast mouse fires pointermove 120Hz+. Coalesce to one pick per
  // frame — the latest `_mouse` is used at pick time, so this is imperceptible.
  _scheduleHoverPick();
}

const _scheduleHoverPick = rafThrottle(() => {
  if (_focusPaused || _pointerOverUI || _drag.active) return;
...
function _initPointerEvents(canvas) {
  ...
  // ONE persistent tooltip-follow listener. This replaces the old per-hover
  // add/removeEventListener pair in handleNodeHover, which leaked a listener on
  // every direct node→node hover transition (the common case never hit the
  // hover-off branch that removed the previous handler).
  document.addEventListener('mousemove', _moveTooltip);
}

let _tooltipEl = null;
function _moveTooltip(e) {
  const tip = _tooltipEl || (_tooltipEl = document.getElementById('tooltip'));
...
function handleNodeHover(node, prevNode) {
  ...
  const _prevHoverId = state.hoveredNodeId;
  state.hoveredNodeId = node ? node.id : null;
  // Only the previously- and newly-hovered nodes change visual state.
  _markNodeDirty(_prevHoverId);
  _markNodeDirty(state.hoveredNodeId);
...
export function navigateToNode(node, { zoom = 'close' } = {}) {
  ...
  const _prevSelId = state.selectedNodeId;
  state.selectedNodeId = node.id;
  // Only the previously- and newly-selected nodes change (unless the category
  // was hidden — that emits 'categories-changed' below → full rebuild wins).
  _markNodeDirty(_prevSelId);
  _markNodeDirty(node.id);
...
// Resolve one node's visual state (opacity/glow/color/scale) into the
// gi-indexed buffers. Single source of truth shared by the full rebuild and
// the O(k) partial hover/select update — so both stay in lockstep.
function _computeNodeVisual(gi, node) {
...
function _updateMemoryNodesInstanced() {
  ...
  if (_instancesDirty) {
    ...
    _partialDirtyIds.clear();   // a full rebuild supersedes any pending partials
    _fullRebuildInstanced(nodeCount);
  } else if (_partialDirtyIds.size > 0) {
    _partialUpdateInstanced();
    _partialDirtyIds.clear();
  }
}

function _fullRebuildInstanced(nodeCount) {
  // Phase 1: resolve every node's visual state
  for (let gi = 0; gi < nodeCount; gi++) {
    _computeNodeVisual(gi, _glowNodeOrder[gi]);
  }
...
// Refresh only the handful of nodes whose hover/select state toggled — instead
// of re-looping all ~N nodes + re-uploading every tier buffer. Hover/select
// changes neither position nor draw count, so only color/opacity/scale (and the
// glow color/opacity/size) need re-writing for the affected indices. Any other
// dirty cause routes through _fullRebuildInstanced (see the dispatcher).
function _partialUpdateInstanced() {
  const tiers = [_wireInstLow, _wireInstMed, _wireInstHigh];
  const t0 = _tierNodes[0].length, t1 = _tierNodes[1].length;
  const tierOffset = [0, t0, t0 + t1]; // gi = tierOffset[tier] + index
  const touchedTier = [false, false, false];
  let glowTouched = false;

  for (const id of _partialDirtyIds) {
    const m = _nodeInstanceMap.get(id);
    if (!m) continue;                       // not a memory node (e.g. anchor/tag) — nothing to update
    const tier = m.tier, i = m.index;
    const gi = tierOffset[tier] + i;
    const node = _glowNodeOrder[gi];
    if (!node) continue;

    _computeNodeVisual(gi, node);

    const inst = tiers[tier];
    if (inst) {
      const scale = _nodeScale[gi] * _nodeBaseRadius[gi];
      _instPos.set(node.x || 0, node.y || 0, node.z || 0);
      _instScaleV.set(scale, scale, scale);
      ...
      colorArr[ci3 + 2] = _nodeColorB[gi];
      opacityArr[i] = _nodeOpacity[gi];
      touchedTier[tier] = true;
    }

    if (_glowPoints && _glowPointsGeo) {
      const colArr = _glowPointsGeo.attributes.glowColor.array;
      const opArr  = _glowPointsGeo.attributes.glowOpacity.array;
      const sizeArr = _glowPointsGeo.attributes.glowSize.array;
      const gi3 = gi * 3;
...
function animate() {
  ...
  // Idle gate: anything in-flight counts as activity; after 2s of stillness
  // drop to IDLE_FPS instead of rendering every vsync
  if (TWEEN.getAll().length > 0 || _instancesDirty || _linkBatchDirty ||
      _linkPosDirty || _drag.active || _mouseMoved) {
    _lastActivityTime = now;
  }
  if (now - _lastActivityTime > IDLE_AFTER_MS &&
      now - _lastIdleRenderTime < 1000 / IDLE_FPS) {
    return;
  }
  _lastIdleRenderTime = now;
  ...
  if (graph) {
    ...
    for (let _ni = 0, _nlen = _anchorTagArray.length; _ni < _nlen; _ni++) {
      ...
      // Frustum culling — anchor/tag groups are direct children of the
      // untransformed scene, so local position == world position (skips a
      // per-object updateWorldMatrix walk every frame)
      _worldPos.copy(obj.position);
      let _cullZone = 0;
      ...
      if (obj.userData.isAnchor) {
        ...
        if (label) {
          ...
          if (label.material.uniforms) {
            ...
            labelOp = label.material.uniforms.uOpacity.value;
          } else {
            label.material.opacity += (labelTarget - label.material.opacity) * fade;
            labelOp = label.material.opacity;
          }
          // Skip billboard + scale work for labels that are fully faded out
          if (labelOp > 0.01 || labelTarget > 0) {
            // Billboard: face camera
            label.quaternion.copy(_cam.quaternion);
            if (obj.userData.baseLabelScale) {
  ...
  // Camera motion from any source (orbit, damping, WASD, tween flights) = active
  if (_camera && (_camera.position.distanceToSquared(_lastCamPos) > 1e-6 ||
      Math.abs(1 - _lastCamQuat.dot(_camera.quaternion)) > 1e-9)) {
    _lastActivityTime = now;
...
export function initGraph(container, options = {}) {
  ...
  // Cap DPR: on 2x retina an uncapped ratio renders 4x the pixels through the
  // whole post-processing chain (2D variant already caps at 1.5)
  ...
  // Wake from idle throttle the instant a pointer grabs the canvas
  _controls.addEventListener('start', () => { _lastActivityTime = performance.now(); });
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    } else if (!_externallyStopped && !animFrameId) {
      _lastActivityTime = performance.now(); // render immediately on return
      animate();
    }
  });
  ...
  if (storage.getItem(KEYS.LAYOUT_VERSION) !== 'v6-polished') {
    try { posStore.removeItem(_posStorageKey); } catch (_) {}
  ...
  // Bloom post-processing (quarter resolution — the blur mip chain scales from
  // this base, and the soft glow is visually indistinguishable from half res)
  if (gfx.bloomEnabled !== false) {
    _bloomPass = new UnrealBloomPass(
      new THREE.Vector2(Math.floor(container.offsetWidth / BLOOM_RES_DIVISOR), Math.floor(container.offsetHeight / BLOOM_RES_DIVISOR)),
      0.5,   // strength
      0.4,   // radius
...
function _onResize() {
  ...
  if (_bloomPass) {
    _bloomPass.resolution.set(Math.floor(w / BLOOM_RES_DIVISOR), Math.floor(h / BLOOM_RES_DIVISOR));
  }
}
