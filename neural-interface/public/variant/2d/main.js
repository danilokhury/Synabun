...
// The render loop has two independent stop reasons: viz disabled or focus mode
// (whiteboard) active. It runs ONLY when viz is on AND not in focus — so toggling
// viz while the whiteboard is up never restarts it, and re-entering focus re-pauses.
...
on('focus:enter', () => {
  ...
  clearLasso();
  hideContextMenu();
  // Whiteboard is up — stop the canvas loop (it ran at 15fps). _applyGraphAnimation
  // re-evaluates viz + focus together, so a viz toggle during focus won't restart it.
  _inFocusMode = true;
  _applyGraphAnimation();
  pauseInteraction();
});
...
on('workspace:restore-scene', ({ nodePositions }) => {
  if (nodePositions && Object.keys(nodePositions).length > 0) {
    posStore.setItem(KEYS.NODE_POS_2D, JSON.stringify(nodePositions));
  }
  applyGraphData();
});
