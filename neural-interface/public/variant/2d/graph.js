...
import { truncate, normalizeNodes, rafThrottle } from '../../shared/utils.js';
...
// Idle throttling — full rate while panning/zooming/animating, reduced when
// still (background breathing is a 4s sine; 15fps keeps it smooth enough)
const IDLE_AFTER_MS = 2000;
const IDLE_FPS = 15;
let _externallyStopped = false;   // stopAnimation() called (focus mode / viz toggle)
...
export function initGraph(container, callbacks = {}) {
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    } else if (!_externallyStopped && !_animFrameId && _canvas) {
...
export function applyGraphData() {
  if (!_canvas) return;
  _lastActivityTime = performance.now(); // new data → render at full rate
...
export async function refreshGraph() {
  const res = await fetch('/api/memories?fields=graph');
...
function _savePositions() {
  ...
  try { posStore.setItem(KEYS.NODE_POS_2D, JSON.stringify(positions)); } catch (_) {}
}
...
function _onPointerMove(e) {
  ...
  if (_pointer.isPanning) {
    ...
    // Hover detection — _hitTest allocates flattenRegions() + per-region zoom
    // math, so coalesce to one test per frame (cursor at frame rate is fine).
    _lastHoverClientX = e.clientX;
    _lastHoverClientY = e.clientY;
    _scheduleHover2D();
  }
}

let _lastHoverClientX = 0, _lastHoverClientY = 0;
const _scheduleHover2D = rafThrottle(() => {
  if (_focusPaused || _pointer.isPanning || _pointer.isDragging) return;
  const hit = _hitTest(_lastHoverClientX, _lastHoverClientY);
