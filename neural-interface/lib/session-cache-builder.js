...
import { getDb, upsertSessionCache, upsertSessionFts, getSessionCacheEntry } from './db.js';
...
export function rebuildCodexCache({ project, onProgress } = {}) {
  const emit = onProgress || (() => {});
  const homeDir = process.env.USERPROFILE || process.env.HOME;
  const sessionsDir = join(homeDir, '.codex', 'sessions');
  if (!existsSync(sessionsDir)) return { sessions: 0, updated: 0 };
  ...
  const walk = (dir) => {
    try {
      for (const ent of readdirSync(dir, { withFileTypes: true })) {
        ...
        else if (ent.name.endsWith('.jsonl')) allFiles.push(join(dir, ent.name));
      }
    } catch { /* ignore */ }
  };
  walk(sessionsDir);
  ...
  for (const filePath of allFiles) {
    ...
    if (projLower && parsed.cwd && !parsed.cwd.toLowerCase().includes(projLower)) continue;
