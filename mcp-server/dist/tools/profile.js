...
export const profileSchema = {
    action: z.enum(['get', 'set']).describe('get = show current profile, available presets, and tool counts. set = switch to a different profile (enables/disables tool groups at runtime).'),
    profile: z.string().optional().describe('Profile to switch to (required for "set"). Preset names: core, standard, browser, full. Or comma-separated groups: "git,browser,discord".'),
};
export const profileDescription = 'Switch MCP tool profiles at runtime without restarting. Use "get" to see current profile and available presets. Use "set" to enable/disable tool groups dynamically — the client will be notified of the tool list change.';
const ALWAYS_ON_TOOL_COUNT = 10;
const GROUP_TOOL_ESTIMATES = {
    git: 1,
    image: 1,
    whiteboard: 3,
    card: 4,
