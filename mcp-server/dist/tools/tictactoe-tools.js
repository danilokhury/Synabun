...
export const tictactoeSchema = {
    action: z.enum(['start', 'move', 'state', 'end']).describe('Game action. "start" = set up board (optional piece: X or O). "move" = place piece (requires cell 1-9). "state" = get current board. "end" = tear down game.'),
    cell: z.number().min(1).max(9).optional().describe('Cell number 1-9 (required for "move" action). Layout:\n  1 | 2 | 3\n  ---------\n  4 | 5 | 6\n  ---------\n  7 | 8 | 9'),
    piece: z.enum(['X', 'O']).optional().describe('Which piece you play as (for "start" action). Default: X. X always goes first.'),
};
export const tictactoeDescription = 'Play Tic Tac Toe on the whiteboard. Actions: "start" sets up the board (piece defaults to X), "move" places the current turn\'s piece in cell 1-9, "state" shows the current board, "end" closes the game. The board renders visually on the whiteboard. X always goes first. Returns ASCII board + game status after each action.';
export async function handleTictactoe(args) {
    switch (args.action) {
        case 'start':
...
async function handleStart(piece) {
    const result = await ni.tictactoeStart(piece);
    if (result.error) {
        return { content: [{ type: 'text', text: `Failed to start game: ${result.error}` }] };
    }
...
async function handleMove(cell) {
    if (cell === undefined) {
        return { content: [{ type: 'text', text: 'cell is required for move action (1-9)' }] };
    }
...
async function handleState() {
    ...
    if (result.error) {
        return { content: [{ type: 'text', text: `Failed to get state: ${result.error}` }] };
    }
    if (!result.active) {
        return { content: [{ type: 'text', text: 'No active TicTacToe game. Use action "start" to begin.' }] };
    }
...
async function handleEnd() {
    ...
    if (result.error) {
        return { content: [{ type: 'text', text: `Failed to end game: ${result.error}` }] };
    }
    return { content: [{ type: 'text', text: 'Game ended. Board cleared.' }] };
}
//# sourceMappingURL=tictactoe-tools.js.map