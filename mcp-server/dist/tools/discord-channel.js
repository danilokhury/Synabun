...
export const discordChannelSchema = {
    ...
    type: z.enum(['text', 'voice', 'category', 'announcement', 'forum', 'stage']).optional()
...
export const discordChannelDescription = 'Discord channel management. Actions: create, edit, delete, list, permissions. Handles text, voice, categories, forums, stages.';
// ── Create ─────────────────────────────────────────────────────
async function handleCreate(guildId, args) {
    ...
    if (args.slowmode !== undefined)
        data.rate_limit_per_user = args.slowmode;
    if (args.bitrate !== undefined)
        data.bitrate = args.bitrate;
    if (args.user_limit !== undefined)
        data.user_limit = args.user_limit;
...
async function handlePermissions(guildId, args) {
    ...
    try {
        if (args.allow)
            allowBits = discord.resolvePermissions(args.allow);
        if (args.deny)
