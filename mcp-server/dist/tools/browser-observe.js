...
function filterTree(node, mode, keep) {
    if (!node)
        return null;
    const role = node.role || 'generic';
    const children = node.children || [];
    ...
    if (keptChildren.length > 0)
        return { role: 'generic', name: '', children: keptChildren };
    return null;
}
// Exported so navigate/click/scroll can format an attached returnSnapshot consistently.
export function formatInlineSnapshot(result, opts = {}) {
    const mode = opts.mode || result.mode || 'full';
    const format = opts.format || 'text';
    const maxChars = opts.maxChars && opts.maxChars > 0 ? opts.maxChars : 12000;
    if (mode === 'interactive') {
        const flat = result.interactive || [];
        if (format === 'json') {
...
function flattenInteractive(node) {
    const out = [];
    const role = node.role || '';
    if (INTERACTIVE_ROLES.has(role)) {
        const entry = { role, name: node.name || '' };
        if (node.value)
