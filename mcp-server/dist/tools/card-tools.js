...
export const cardListDescription = 'List all currently open memory cards in the Neural Interface. Returns each card\'s memory UUID, position, size, compact/pin state, content preview, and the current viewport dimensions (width x height). Use this before placing cards to know the available screen size.';
...
export const cardCloseDescription = 'Close a memory card by its UUID, or close all open cards if no memoryId is provided. Use card_list first to see which cards are open.';
...
export const cardUpdateDescription = 'Update an open memory card\'s position, size, compact/expand state, or pin state. Set coordMode to "pct" to use percentage-based positioning. Only specified fields are changed; others remain untouched. Changes appear in real-time.';
export async function handleCardUpdate(args) {
    ...
    if (r) {
        if (r.left !== undefined)
            parts.push(`pos: (${r.left}, ${r.top})`);
        if (r.isCompact !== undefined)
            parts.push(r.isCompact ? 'compact' : 'expanded');
        if (r.isPinned !== undefined)
            parts.push(r.isPinned ? 'pinned' : 'unpinned');
        if (r.width)
...
export const cardScreenshotDescription = 'Take a visual screenshot of the Neural Interface showing all open memory cards in their current positions. Returns a JPEG image. Requires the Neural Interface to be open in a browser.';
