...
export const syncDescription = 'Check for stale memories whose related files have changed. Compares stored file checksums against current hashes. Returns compact output (ID + changed files only, no content). IMPORTANT: Always pass "categories" to scope the scan — calling without categories scans ALL memories and may produce output too large to return inline. For large memory sets, call iteratively per category rather than globally. Default limit is 50; increase only when using a narrow category filter.';
export async function handleSync(args) {
    ...
    for (const point of withChecksums) {
        ...
        for (const filePath of Object.keys(storedChecksums)) {
            ...
            if (!currentHash)
                continue; // File not found — can't compare, skip
