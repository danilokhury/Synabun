...
function formatClickHints(result) {
    ...
    if (!hints?.length)
        return '';
    const heading = result.hintsLabel;
...
export const browserClickDescription = 'Click an element on the page. Accepts Playwright selectors: CSS, text="...", :has-text("..."), role=button[name="..."], [data-testid="..."]. NEVER use :contains() — use :has-text(). Run browser_snapshot first (or mode="interactive") to find the element. Call browser_cheatsheet for per-platform stable selectors.';
...
export const browserFillDescription = 'Clear an input/textarea and fill it with new text. Accepts Playwright selectors: CSS, text="...", :has-text("..."), [data-testid="..."]. For contenteditable editors prefer browser_type. Call browser_cheatsheet for per-platform input selectors.';
export async function handleBrowserFill(args) {
    ...
    if (result.error)
        return text(`Fill failed: ${result.error}${formatClickHints(result)}`);
    const healed = result.healed ? ' (auto-healed via textHint)' : '';
...
export const browserTypeDescription = 'Type text character-by-character (simulates real keystrokes; appends). Provide a selector to target, or omit to type into the focused element. Prefer over browser_fill for contenteditable/rich-text editors. Call browser_cheatsheet for per-platform compose selectors.';
export async function handleBrowserType(args) {
    ...
    if (result.error)
        return text(`Type failed: ${result.error}${formatClickHints(result)}`);
...
export const browserHoverDescription = 'Hover over an element. Useful for revealing dropdowns, tooltips, or hover-triggered content. Accepts Playwright selectors.';
export async function handleBrowserHover(args) {
    ...
    if (result.error)
        return text(`Hover failed: ${result.error}${formatClickHints(result)}`);
    const healed = result.healed ? ' (auto-healed via textHint)' : '';
...
export const browserSelectDescription = 'Select an option from a <select> dropdown by CSS selector and option value.';
export async function handleBrowserSelect(args) {
    ...
    if (result.error)
        return text(`Select failed: ${result.error}${formatClickHints(result)}`);
...
export const browserPressDescription = 'Press a keyboard key or key combination. Supports modifiers like Control+A, Shift+Enter, etc.';
...
export const browserUploadDescription = 'Upload one or more files via a file input element. For platforms with hidden/gated inputs, click the visible upload/media button first to reveal the input, then pass its selector. Call browser_cheatsheet for per-platform upload flows.';
export async function handleBrowserUpload(args) {
    ...
    if (result.error)
        return text(`Upload failed: ${result.error}${formatClickHints(result)}`);
