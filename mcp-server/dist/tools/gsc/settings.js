...
async function loadAndExtract(sessionId, tabIdArg, property, path) {
    const r = await resolve(sessionId, tabIdArg);
    if ('error' in r)
        return { err: r.error, r: null };
    const prop = property || (await currentProperty(r));
    if (!prop && !path.startsWith('http'))
        return { err: 'No active property.', r };
    ...
    if (nav.error)
        return { err: `Navigation failed: ${nav.error}`, r };
    const authErr = await ensureAuth(r);
    if (authErr)
        return { err: authErr, r };
...
export const gscSettingsSchema = {
    action: z.enum(['get', 'set_address', 'change_address']).optional().describe('Default get. set_address sets the Change-of-address target. change_address opens flow.'),
    newProperty: z.string().optional().describe('Target property for change_address.'),
    property: z.string().optional(),
    sessionId: z.string().optional(),
    tabId,
};
export const gscSettingsDescription = 'Read or modify property settings — ownership/verification, users, address. action=get returns the settings page summary; action=set_address/change_address mutate.';
export async function handleGscSettings(args) {
    const action = args.action || 'get';
    if (action === 'get') {
        const { err, r } = await loadAndExtract(args.sessionId, args.tabId, args.property, '/settings');
        if (err)
            return text(err);
        const data = await ni.evaluate(r.sessionId, `(() => {
    ...
    if (action === 'change_address' || action === 'set_address') {
        ...
        if (action === 'set_address') {
            if (!args.newProperty)
                return text('newProperty required for set_address.');
            await ni.evaluate(r.sessionId, `(() => { const sel = document.querySelector('select, [role="combobox"]'); if (sel) sel.click(); })()`, r.tabId);
            await wait(700);
            await ni.evaluate(r.sessionId, `(() => { const items = Array.from(document.querySelectorAll('[role="option"], option')); const t = items.find(i => i.textContent.includes(${JSON.stringify(args.newProperty)})); if (t) t.click(); })()`, r.tabId);
            await wait(500);
            await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const sub = btns.find(b => /^(submit|enviar|validate|validar)$/i.test((b.textContent || '').trim())); if (sub) sub.click(); })()`, r.tabId);
...
export const gscCrawlStatsDescription = 'Read Crawl Stats — totals (requests, download size, response time) and breakdowns by host status, response code, file type, Googlebot type.';
export async function handleGscCrawlStats(args) {
    ...
    let scalar = {
        totalRequests: tr.result ?? null,
        totalDownloadSize: td.result ?? null,
        avgResponseTime: ar.result ?? null,
    };
    if (!scalar.totalRequests || !scalar.totalDownloadSize || !scalar.avgResponseTime) {
        const fallback = await ni.evaluate(r.sessionId, `(() => {
    ...
    const sections = await ni.evaluate(r.sessionId, `(() => {
      const main = document.querySelector('[role="main"], main') || document.body;
...
export const gscUsersSchema = {
    action: z.enum(['list', 'add', 'remove', 'change_role']),
    email: z.string().optional(),
    role: z.enum(['owner', 'full', 'restricted']).optional(),
    property: z.string().optional(),
    sessionId: z.string().optional(),
    tabId,
};
export const gscUsersDescription = 'Manage property users. list returns email + role. add invites a user (mutating). remove revokes (mutating). change_role updates permissions (mutating).';
export async function handleGscUsers(args) {
    ...
    if (args.action === 'list') {
        ...
        const structured = await ni.evaluate(r.sessionId, `(() => {
    ...
    if (args.action === 'add') {
        if (!args.email)
            return text('email required for add.');
        await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const a = btns.find(b => /add\\s*user|adicionar\\s*usuário/i.test(b.textContent || '')); if (a) a.click(); })()`, r.tabId);
        await wait(900);
        await ni.evaluate(r.sessionId, `(() => { const i = document.querySelector('input[type="email"], input[type="text"]'); if (i) { i.focus(); i.value = ${JSON.stringify(args.email)}; i.dispatchEvent(new Event('input', { bubbles: true })); } })()`, r.tabId);
        if (args.role) {
            await wait(300);
            const roleRe = args.role === 'owner' ? /owner|proprietário/i : args.role === 'full' ? /full|total/i : /restricted|restrito/i;
            await ni.evaluate(r.sessionId, `(() => { const sel = document.querySelector('select, [role="combobox"]'); if (sel) sel.click(); })()`, r.tabId);
            await wait(500);
            await ni.evaluate(r.sessionId, `(() => { const items = Array.from(document.querySelectorAll('[role="option"], option')); const t = items.find(i => ${roleRe.toString()}.test(i.textContent || '')); if (t) t.click(); })()`, r.tabId);
        }
        await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const add = btns.find(b => /^(add|adicionar)$/i.test((b.textContent || '').trim())); if (add) add.click(); })()`, r.tabId);
        return text(`Add user requested: ${args.email}${args.role ? ` (${args.role})` : ''}.`);
    }
    if (args.action === 'remove' || args.action === 'change_role') {
        if (!args.email)
            return text('email required.');
        ...
        if (!click.result)
            return text(`User not found: ${args.email}`);
        await wait(500);
        if (args.action === 'remove') {
            await ni.evaluate(r.sessionId, `(() => { const items = Array.from(document.querySelectorAll('[role="menuitem"], button')); const d = items.find(i => /remove|excluir|remover/i.test(i.textContent || '')); if (d) d.click(); })()`, r.tabId);
            await wait(700);
            await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const conf = btns.find(b => /^(remove|excluir|remover)$/i.test((b.textContent || '').trim())); if (conf) conf.click(); })()`, r.tabId);
            return text(`Remove user requested: ${args.email}.`);
        }
        if (!args.role)
            return text('role required for change_role.');
        const roleRe = args.role === 'owner' ? /owner|proprietário/i : args.role === 'full' ? /full|total/i : /restricted|restrito/i;
        await ni.evaluate(r.sessionId, `(() => { const items = Array.from(document.querySelectorAll('[role="menuitem"], button')); const c = items.find(i => /change\\s*role|alterar\\s*função/i.test(i.textContent || '')); if (c) c.click(); })()`, r.tabId);
        await wait(700);
        await ni.evaluate(r.sessionId, `(() => { const items = Array.from(document.querySelectorAll('[role="option"], option, [role="menuitem"]')); const t = items.find(i => ${roleRe.toString()}.test(i.textContent || '')); if (t) t.click(); })()`, r.tabId);
...
export const gscAssociationsSchema = {
    ...
    service: z.enum(['google_analytics', 'merchant_center', 'google_ads', 'play_store', 'youtube', 'actions_on_google', 'chrome_web_store']).optional(),
    identifier: z.string().optional().describe('Service-specific ID (e.g. GA4 measurement ID, Merchant Center account ID).'),
    property: z.string().optional(),
    sessionId: z.string().optional(),
    tabId,
};
export const gscAssociationsDescription = 'Manage Associations (Analytics / Merchant / Ads / Play / YouTube / Actions / Chrome Web Store). list returns existing associations; add/remove mutate.';
export async function handleGscAssociations(args) {
    ...
    if (args.action === 'add') {
        await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const a = btns.find(b => /associate|associar|connect|conectar/i.test(b.textContent || '')); if (a) a.click(); })()`, r.tabId);
    ...
    if (args.action === 'remove') {
        if (!args.identifier)
            return text('identifier required for remove.');
        ...
        await wait(500);
        await ni.evaluate(r.sessionId, `(() => { const items = Array.from(document.querySelectorAll('[role="menuitem"], button')); const d = items.find(i => /disassociate|disconnect|remover|desassociar/i.test(i.textContent || '')); if (d) d.click(); })()`, r.tabId);
...
export const gscDisavowSchema = {
    action: z.enum(['download', 'upload', 'delete']),
    filePath: z.string().optional().describe('Absolute path of disavow .txt file (action=upload).'),
    property: z.string().optional(),
    sessionId: z.string().optional(),
    tabId,
};
export const gscDisavowDescription = '[mutating for upload/delete — affects how Google trusts links to your site] Manage the Disavow Links file. download exports current; upload replaces (use sparingly).';
export async function handleGscDisavow(args) {
    ...
    if (args.action === 'download') {
        await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"], a')); const d = btns.find(b => /download|baixar|export/i.test(b.textContent || '')); if (d) d.click(); })()`, r.tabId);
        return text('Disavow file download triggered. File saved to browser default download dir.');
    }
    if (args.action === 'upload') {
        if (!args.filePath)
            return text('filePath required for upload.');
        await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const r = btns.find(b => /replace|substituir|upload|carregar/i.test(b.textContent || '')); if (r) r.click(); })()`, r.tabId);
        await wait(800);
        const up = await ni.upload(r.sessionId, 'input[type="file"]', [args.filePath], undefined, r.tabId);
        if (up.error)
            return text(`Upload failed: ${up.error}`);
        await wait(1000);
        await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const sub = btns.find(b => /^(submit|enviar|done|concluir)$/i.test((b.textContent || '').trim())); if (sub) sub.click(); })()`, r.tabId);
        return text(`Disavow file uploaded: ${args.filePath}.`);
    }
    if (args.action === 'delete') {
        await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const d = btns.find(b => /delete\\s*list|excluir\\s*lista|remove/i.test(b.textContent || '')); if (d) d.click(); })()`, r.tabId);
        await wait(700);
        await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const conf = btns.find(b => /^(delete|excluir|remove)$/i.test((b.textContent || '').trim())); if (conf) conf.click(); })()`, r.tabId);
...
export const gscShoppingDescription = 'Read Shopping/Merchant Listings report — counts and per-issue example URLs.';
export async function handleGscShopping(args) {
    const { err, r } = await loadAndExtract(args.sessionId, args.tabId, args.property, '/r/merchant-listings');
    if (err)
        return text(err);
    const noData = await ni.evaluate(r.sessionId, `(() => /Nenhum dado|No data|No merchant listings|Sem listagens|This report isn't available|Este relat[oó]rio n[aã]o est[aá] dispon[ií]vel/i.test(document.body.innerText))()`, r.tabId);
...
export const gscExtractTableDescription = 'Generic GSC grid → JSON extractor. Useful when no specific gsc_*_report tool covers a panel.';
export async function handleGscExtractTable(args) {
    ...
    if (res.error)
        return text(`Extract failed: ${res.error}`);
...
export const gscScreenshotDescription = 'Capture a full-page screenshot of the active GSC tab. Saved via SynaBun image staging.';
export async function handleGscScreenshot(args) {
    const r = await resolve(args.sessionId, args.tabId, false);
    if ('error' in r)
        return text(r.error);
