...
export const gscCwvReportDescription = 'Read Core Web Vitals report for mobile or desktop. Returns counts of poor / needs-improvement / good URLs and per-issue URL group examples.';
export async function handleGscCwvReport(args) {
    ...
    if (err)
        return text(err);
    const noData = await ni.evaluate(r.sessionId, `(() => /Nenhum dado|No data|No CWV data/i.test(document.body.innerText) && !document.querySelector('[role="grid"], [role="table"]'))()`, r.tabId);
    if (noData.result) {
    ...
    const summary = {
        poor: poorRes.result ?? null,
        needsImprovement: niRes.result ?? null,
        good: goodRes.result ?? null,
    };
...
export const gscHttpsReportDescription = 'Read the HTTPS report — counts of HTTPS vs non-HTTPS URLs and reasons for non-HTTPS URLs with examples.';
export async function handleGscHttpsReport(args) {
    ...
    const summary = await ni.evaluate(r.sessionId, `(() => {
    ...
    let s = summary.result || { httpsUrls: null, nonHttpsUrls: null };
    if (s.httpsUrls === null && s.nonHttpsUrls === null) {
        ...
        s = {
            httpsUrls: hRes.result ?? null,
            nonHttpsUrls: nRes.result ?? null,
        };
    }
...
export const gscSecurityIssuesSchema = {
    action: z.enum(['list', 'request_review']).optional().describe('list (default) returns active issues; request_review submits a reconsideration request.'),
    property: z.string().optional(),
    sessionId: z.string().optional(),
    tabId,
};
export const gscSecurityIssuesDescription = 'Read Security Issues. action=list returns active + history. action=request_review clicks "Request review" (mutating, requires fixed issues).';
export async function handleGscSecurityIssues(args) {
    const { err, r } = await loadAndExtract(args.sessionId, args.tabId, args.property, '/security-issues');
    if (err)
        return text(err);
    const action = args.action || 'list';
    if (action === 'request_review') {
        const click = await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const req = btns.find(b => /request\\s*review|solicitar\\s*revisão/i.test(b.textContent || '')); if (req) { req.click(); return true; } return false; })()`, r.tabId);
        if (!click.result)
            return text('Request review button not found — no fixable issues, or review already pending.');
        return text('Review request flow opened. Use browser_snapshot to inspect form, then submit via browser_click.');
    }
    const issues = await ni.evaluate(r.sessionId, `(() => {
...
export const gscManualActionsSchema = {
    action: z.enum(['list', 'reconsideration']).optional(),
    body: z.string().optional().describe('Reconsideration request body text. Required for action=reconsideration.'),
    property: z.string().optional(),
    sessionId: z.string().optional(),
    tabId,
};
export const gscManualActionsDescription = 'Read Manual Actions. action=list returns penalties (or "No issues detected"). action=reconsideration submits a reconsideration request with `body` (mutating).';
export async function handleGscManualActions(args) {
    ...
    if (action === 'reconsideration') {
        if (!args.body)
            return text('body required for reconsideration.');
        await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const req = btns.find(b => /request\\s*review|solicitar\\s*reconsider/i.test(b.textContent || '')); if (req) req.click(); })()`, r.tabId);
        await wait(1500);
        await ni.evaluate(r.sessionId, `(() => { const ta = document.querySelector('textarea'); if (ta) { ta.focus(); ta.value = ${JSON.stringify(args.body)}; ta.dispatchEvent(new Event('input', { bubbles: true })); } })()`, r.tabId);
        await wait(400);
        await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const sub = btns.find(b => /^(submit|enviar)$/i.test((b.textContent || '').trim())); if (sub) sub.click(); })()`, r.tabId);
...
export const gscEnhancementsSchema = {
    report: z.enum(Object.keys(ENHANCE_PATHS)),
    property: z.string().optional(),
    sessionId: z.string().optional(),
    tabId,
};
export const gscEnhancementsDescription = `Read a structured-data Enhancements report. Returns valid / warnings / errors counts and per-issue URL examples. Reports: ${Object.keys(ENHANCE_PATHS).join(', ')}.`;
export async function handleGscEnhancements(args) {
    const path = ENHANCE_PATHS[args.report];
    if (!path)
        return text(`Unknown enhancement report: ${args.report}`);
    const { err, r } = await loadAndExtract(args.sessionId, args.tabId, args.property, path);
    if (err)
        return text(err);
    ...
    const summaryRes = await ni.evaluate(r.sessionId, `(() => {
      const main = document.querySelector('[role="main"], main') || document.body;
...
export const gscLinksReportSchema = {
    section: z.enum(['top_linked_external', 'top_linking_sites', 'top_linking_text', 'top_linked_internal']),
    limit: z.coerce.number().int().positive().optional().describe('Max rows to scroll-load (default 100).'),
    property: z.string().optional(),
    sessionId: z.string().optional(),
    tabId,
};
export const gscLinksReportDescription = 'Read a Links report section. top_linked_external / top_linking_sites / top_linking_text / top_linked_internal. Returns full table.';
export async function handleGscLinksReport(args) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        if (Number(count.result || 0) >= limit)
            break;
        await ni.scroll(r.sessionId, { direction: 'down', distance: 1500 }, r.tabId);
        await wait(600);
    }
    const tableRes = await ni.evaluate(r.sessionId, TABLE_EXTRACTOR_SCRIPT('[role="main"] [role="grid"], [role="main"] [role="table"], main [role="grid"], main [role="table"]'), r.tabId);
...
export const gscLinksExportSchema = {
    scope: z.enum(['external', 'internal', 'sample_external', 'sample_internal']).describe('Export scope from the Links report Export menu.'),
    property: z.string().optional(),
    sessionId: z.string().optional(),
    tabId,
};
export const gscLinksExportDescription = 'Trigger Links report Export menu. Browser handles file download.';
export async function handleGscLinksExport(args) {
    const { err, r } = await loadAndExtract(args.sessionId, args.tabId, args.property, '/links');
    if (err)
        return text(err);
    await ni.evaluate(r.sessionId, `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const exp = btns.find(b => /export\\s*external\\s*links|exportar\\s*links\\s*externos/i.test(b.textContent || '') || /^export$|^exportar$/i.test((b.textContent || '').trim())); if (exp) exp.click(); })()`, r.tabId);
    await wait(800);
    const labels = {
    ...
    await ni.evaluate(r.sessionId, `(() => { const items = Array.from(document.querySelectorAll('[role="menuitem"], a, button')); const t = items.find(i => ${re.toString()}.test(i.textContent || '')); if (t) t.click(); })()`, r.tabId);
    await wait(2000);
