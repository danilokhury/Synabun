...
const VERDICT_RE_SRC = 'URL is on Google|O URL está no Google|URL está no Google|URL is not on Google|O URL não está no Google|URL não está no Google|page is indexed|A página está indexada|not indexed|não indexada|não está indexada';
 ...
 */
async function submitUrlInspection(r, prop, url) {
    ...
    if (directNav.error) {
        ...
        const got404 = !!(direct && typeof direct === 'object' && direct.error === 'google_404');
        if (!got404) {
            ...
            if (typeof href.result === 'string' && href.result.includes('/inspect'))
                return null;
        }
    }
    ...
    if (nav.error)
        return `Navigation failed: ${nav.error}`;
    const authErr = await ensureAuth(r);
    if (authErr)
        return authErr;
    ...
    if (fill.error) {
        ...
        if (fallback.error)
            return `Could not fill URL inspection field: ${fallback.error}`;
        if (!fallback.result)
            return 'Could not find the GSC URL inspection input.';
    }
    const press = await ni.pressKey(r.sessionId, 'Enter', r.tabId);
    if (press.error)
        return `Could not submit URL inspection: ${press.error}`;
    const routed = await pollFor(r, `(() => {
...
export const gscInspectUrlDescription = 'Run URL Inspection on a URL — opens the inspect tool, fills the URL, polls up to 30s for the result panel, returns structured JSON ' +
...
export async function handleGscInspectUrl(args) {
    const r = await resolve(args.sessionId, args.tabId);
    if ('error' in r)
        return text(r.error);
    const prop = args.property || (await currentProperty(r));
    if (!prop)
        return text('No active property. Call gsc_navigate or gsc_property action=select first.');
    const submitErr = await submitUrlInspection(r, prop, args.url);
    if (submitErr)
        return text(submitErr);
    const ready = await pollFor(r, INSPECT_READY_SCRIPT, args.timeoutMs ?? 30000, 500);
...
export const gscInspectTestLiveDescription = 'Click the "Test live URL" button on the URL Inspection page and poll up to 90s for results. ' +
...
export async function handleGscInspectTestLive(args) {
    const r = await resolve(args.sessionId, args.tabId);
    if ('error' in r)
        return text(r.error);
    if (args.url) {
        ...
        if (typeof inspectRes.content === 'undefined')
            return inspectRes;
    }
    ...
    if (!click.result)
        return text('Could not find "Test live URL" button. Run gsc_inspect_url first.');
    const ready = await pollFor(r, `(() => /Live test results|Resultados do teste|test (?:complete|finished)|live URL/i.test(document.body.innerText) && !/Testing|Loading|Carregando/i.test(document.body.innerText) ? true : null)()`, args.timeoutMs ?? 90000, 1000);
    if (!ready)
        return text(`Live test timed out after ${(args.timeoutMs ?? 90000) / 1000}s.`);
    const res = await ni.evaluate(r.sessionId, INSPECT_RESULT_SCRIPT, r.tabId);
    if (res.error)
        return text(`Extract failed: ${res.error}`);
...
export const gscInspectRequestIndexingDescription = '[mutating — affects Google\'s crawl queue] Submit a URL to Google\'s indexing queue. ' +
...
export async function handleGscInspectRequestIndexing(args) {
    const r = await resolve(args.sessionId, args.tabId);
    if ('error' in r)
        return text(r.error);
    if (args.url) {
    ...
    if (!click.result)
        return text('Could not find "Request indexing" button. URL may already be queued or page not loaded.');
    const result = await pollFor(r, `(() => {
      ...
      return null;
    })()`, args.timeoutMs ?? 60000, 800);
    if (!result)
        return text(`Request submitted but confirmation not detected within ${(args.timeoutMs ?? 60000) / 1000}s. Check browser_snapshot.`);
    return text(`Indexing request:\n${JSON.stringify(result, null, 2)}`);
}
// ── gsc_inspect_view_crawled ──────────────────────────────────
export const gscInspectViewCrawledSchema = {
    view: z.enum(['html', 'screenshot', 'http_response', 'more_info']).describe('Which crawled-page view to open.'),
    sessionId: z.string().optional(),
    tabId,
};
export const gscInspectViewCrawledDescription = 'Open the "View crawled page" panel from the URL Inspection result and switch to the requested view ' +
    '(html / screenshot / http_response / more_info). Use browser_screenshot or browser_snapshot afterwards to capture content.';
export async function handleGscInspectViewCrawled(args) {
    const r = await resolve(args.sessionId, args.tabId);
    if ('error' in r)
        return text(r.error);
    ...
    if (!open.result)
        return text('Could not find "View crawled page" button. Run gsc_inspect_url first.');
    await wait(1500);
    const tabMap = {
        html: /HTML/i,
    ...
    if (!re)
        return text(`Unknown view: ${args.view}`);
    const click = await ni.evaluate(r.sessionId, `(() => {
    ...
    if (!click.result)
        return text(`Tab "${args.view}" not found in panel.`);
    await wait(800);
