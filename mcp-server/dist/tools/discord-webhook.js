...
async function handleEdit(args) {
    if (!args.webhook_id)
        return text('webhook_id is required for edit action.');
    const data = {};
    if (args.name)
        data.name = args.name;
    if (args.channel) {
...
async function handleDelete(args) {
    if (!args.webhook_id)
        return text('webhook_id is required for delete action.');
...
async function handleExecute(args) {
    if (!args.webhook_id)
        return text('webhook_id is required for execute action.');
    if (!args.webhook_token)
        return text('webhook_token is required for execute action.');
    if (!args.content && !args.embeds)
        return text('content or embeds is required for execute action.');
    const data = {};
    if (args.content)
