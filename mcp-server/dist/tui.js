...
function getCategoryColorFn(cat) {
...
function colorCategoryName(cat) {
    const fn = getCategoryColorFn(cat);
    return fn(cat);
}
function importanceBadge(imp) {
    if (imp >= 9)
        return chalk.bgHex('#FF5252').white.bold(` ${imp} `);
    if (imp >= 8)
        return chalk.bgHex('#FF7043').white(` ${imp} `);
    if (imp >= 6)
        return chalk.bgHex('#FFA726').black(` ${imp} `);
    if (imp >= 4)
        return chalk.bgHex('#78909C').white(` ${imp} `);
...
function boxRow(width, content, pad = 1) {
...
function dboxRow(width, content) {
    const stripped = content.replace(/\x1B\[[0-9;]*m/g, '');
...
function printScreenHeader(title, subtitle) {
    const W = 56;
    console.log('');
...
// ── Format memories ───────────────────────────────────────────────
function formatMemoryDetail(id, p) {
    const W = 62;
    const lines = [];
...
function formatMemoryLine(id, p) {
    const catColorFn = getCategoryColorFn(p.category);
...
// ── Screens ───────────────────────────────────────────────────────
async function detailView(id, payload) {
    while (true) {
        console.log('\n' + formatMemoryDetail(id, payload));
        const action = await safePrompt(() => select({
        ...
        if (action === null || action === 'back')
            return;
        if (action === 'delete') {
            const yes = await safePrompt(() => confirm({ message: danger(`  ${ICONS.trash} Delete this memory permanently?`), default: false }));
            if (yes) {
...
async function editMemory(id, payload) {
    printScreenHeader(`${ICONS.edit} Edit Memory`, 'Select a field to modify');
    const field = await safePrompt(() => select({
    ...
    if (field === 'content') {
        const val = await safePrompt(() => input({ message: '  New content:', default: payload.content }));
        if (val === null || val === payload.content)
            return;
        const merged = { ...payload, content: val, updated_at: now };
        ...
        const val = await safePrompt(() => select({
            message: '  Category:',
            choices: cats.map(c => ({
        ...
        if (val === null || val === payload.category)
            return;
        ...
        const val = await safePrompt(() => input({
            message: '  Importance (1-10):',
            default: String(payload.importance),
            validate: (v) => { const n = Number(v); return (Number.isInteger(n) && n >= 1 && n <= 10) || 'Must be 1-10'; },
        }));
        if (val === null)
            return;
        const num = Number(val);
        if (num === payload.importance)
            return;
        ...
        const val = await safePrompt(() => input({ message: '  Subcategory:', default: payload.subcategory || '' }));
        if (val === null)
            return;
        ...
        const val = await safePrompt(() => input({ message: '  Project:', default: payload.project }));
        if (val === null)
            return;
...
async function browseScreen() {
    ...
    while (true) {
        ...
        points.sort((a, b) => {
            const pa = a.payload.created_at;
            const pb = b.payload.created_at;
            return pb.localeCompare(pa);
        });
        const choices = points.map(pt => ({
            name: `  ${formatMemoryLine(pt.id, pt.payload)}`,
            value: pt.id,
        }));
        ...
        const choice = await safePrompt(() => select({
            message: accent(`  ${ICONS.arrow} Select memory`),
            choices,
            pageSize: 15,
        }));
        if (choice === null || choice === '__back__')
            return;
        if (choice === '__more__') {
            offset = nextOffset;
            continue;
        ...
        if (mem) {
            await detailView(mem.id, mem.payload);
        }
        offset = undefined;
    }
}
async function searchScreen() {
    while (true) {
        ...
        const query = await safePrompt(() => input({ message: accent(`  ${ICONS.search} Query:`) }));
        if (query === null || query.trim() === '')
            return;
        // Optional category filter
        const cats = getAllCategories();
        const catFilter = await safePrompt(() => select({
        ...
        const minImpStr = await safePrompt(() => input({
            message: accent('  Min importance (1-10, Enter = any):'),
            default: '',
            validate: (v) => v === '' || (Number.isInteger(Number(v)) && Number(v) >= 1 && Number(v) <= 10) || 'Must be 1-10 or empty',
        }));
        if (minImpStr === null)
            return;
        // Build search filter
        ...
        if (results.length === 0) {
            ...
            const again = await safePrompt(() => confirm({ message: accent('  Try another search?'), default: true }));
            if (again === null || !again)
                return;
            continue;
        }
        ...
        const choices = results.map(r => {
            ...
            return {
                name: `  ${scoreBar.padEnd(18)} ${formatMemoryLine(r.id, p)}`,
                value: r.id,
            };
        });
...
async function createScreen() {
    ...
    // Content
    const content = await safePrompt(() => input({
    ...
    // Category
    const cats = getAllCategories();
    const category = await safePrompt(() => select({
    ...
    const project = await safePrompt(() => input({ message: accent(`  ${ICONS.folder} Project:`), default: defaultProject }));
    if (project === null)
        return;
    // Importance
    const impStr = await safePrompt(() => input({
    ...
    // Tags
    const tagsStr = await safePrompt(() => input({ message: accent(`  ${ICONS.tag} Tags (comma-separated):`), default: '' }));
    if (tagsStr === null)
        return;
    const tags = tagsStr.split(',').map(t => t.trim()).filter(Boolean);
    // Source
    const source = await safePrompt(() => select({
        message: accent(`  ${ICONS.arrow} Source:`),
        choices: [
            { name: `  ${chalk.white('user-told')}       ${dim('User explicitly shared')}`, value: 'user-told' },
            { name: `  ${chalk.white('self-discovered')} ${dim('Found during work')}`, value: 'self-discovered' },
            { name: `  ${chalk.white('auto-saved')}      ${dim('Session context')}`, value: 'auto-saved' },
        ],
        default: 'user-told',
    }));
    if (source === null)
    ...
    const subcategory = await safePrompt(() => input({ message: accent(`  ${ICONS.dot} Subcategory (optional):`), default: '' }));
    if (subcategory === null)
        return;
    // Build payload
    const now = new Date().toISOString();
    ...
    const ok = await safePrompt(() => confirm({ message: accent(`  ${ICONS.sparkle} Save this memory?`), default: true }));
    if (ok === null || !ok) {
...
async function categoriesScreen() {
    while (true) {
        ...
        if (action === 'create') {
            const name = await safePrompt(() => input({
            ...
            if (name === null)
                continue;
            const description = await safePrompt(() => input({ message: accent('  Description:'), validate: (v) => v.trim().length > 0 || 'Required' }));
            if (description === null)
                continue;
        ...
        if (action === 'delete') {
            ...
            const name = await safePrompt(() => select({
            ...
            if (count > 0) {
                ...
                const reassign = await safePrompt(() => confirm({ message: accent('  Reassign them to another category?'), default: true }));
                if (reassign === null || !reassign)
                    continue;
                const target = await safePrompt(() => select({
                ...
                if (target === null)
                    continue;
                await updatePayloadByFilter({ must: [{ key: 'category', match: { value: name } }] }, { category: target, updated_at: new Date().toISOString() });
                console.log(success(`  ${ICONS.check} Reassigned ${count} memories to ${colorCategoryName(target)}.`));
            }
            const ok = await safePrompt(() => confirm({ message: danger(`  ${ICONS.trash} Delete category "${name}"?`), default: false }));
            if (ok) {
                removeCategory(name);
...
async function main() {
    try {
        ...
        console.warn = (...args) => {
            ...
            if (msg.includes('unsecure connection'))
                return;
