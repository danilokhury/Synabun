...
export async function handleBrowserNavigate(args: { url: string; sessionId?: string; tabId?: string }) {
  ...
  const result = await ni.navigate(resolved.sessionId, args.url, resolved.tabId);
