...
export const imageStagedSchema = {
  action: z.enum(['list', 'clear', 'remove']).describe(
    'Action: "list" = show all images with paths. "clear" = delete all attachment images. "remove" = delete a specific image.',
  ),
  filename: z.string().optional().describe('Filename to remove (for "remove" action).'),
  type: z.enum(['all', 'attachment', 'screenshot', 'whiteboard', 'paste']).optional().describe(
    'Filter by image type. Default: "all". "attachment" = images pasted in chat.',
  ),
};

export const imageStagedDescription =
  'List, clear, or remove images from SynaBun\'s image store (data/images/). Images arrive when users paste them in the Neural Interface chat. ' +
...
export async function handleImageStaged(args: { action: string; filename?: string; type?: string }) {
  const { action, filename, type } = args;

  if (action === 'list') {
    const result = await ni.listImages();
    if (result.error) return text(`Failed to list images: ${result.error}`);
    ...
    if (images.length === 0) return text('No images in store.');

    // Filter by type if requested
    const filter = type || 'all';
    const filtered = filter === 'all' ? images : images.filter(i => i.type === filter);

    if (filtered.length === 0) return text(`No images of type "${filter}" found.`);
  ...
  if (action === 'remove') {
    if (!filename) return text('Filename required for "remove" action.');
    const result = await ni.deleteImage(filename);
    if (result.error) return text(`Failed to remove: ${result.error}`);
  ...
  if (action === 'clear') {
    ...
    const images = (result.images || []) as Array<{ filename: string; type: string }>;
    const attachments = images.filter(i => i.type === 'attachment');

    if (attachments.length === 0) return text('No attachment images to clear.');
...
function _relativeAge(ts: number): string {
