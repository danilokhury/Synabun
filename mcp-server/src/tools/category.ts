...
export const categorySchema = {
  action: z
    .enum(['create', 'update', 'delete', 'list'] as const)
  ...
  format: z
    .enum(['flat', 'tree', 'parents-only'] as const)
    .optional()
    .describe('Output format for list action: flat, tree (default), or parents-only.'),
};

export const categoryDescription =
  'Manage memory categories. Actions: create, update, delete, list. Categories persist across sessions.';
...
async function handleList(args: { format?: 'flat' | 'tree' | 'parents-only' }) {
