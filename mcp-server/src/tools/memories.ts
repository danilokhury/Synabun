import { z } from 'zod';
import { scrollMemories, getMemoryStats, getMemory } from '../services/sqlite.js';
...
export function buildMemoriesSchema() {
  return {
    action: z
      .enum(['recent', 'stats', 'by-category', 'by-project', 'get'] as const)
      .describe(
        'recent=latest memories, stats=counts and health, by-category=filter by type, by-project=filter by project, get=FULL untruncated content of one memory by id (no vector search).'
      ),
    id: z
      .string()
      .optional()
...
export async function handleMemories(args: {
  ...
  if (action === 'get') {
    ...
    return text(`[${mem.id}] ${p.category}${sub} | ${p.project} | imp:${p.importance}${tagStr}\n\n${p.content}`);
  }
