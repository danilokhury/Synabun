...
const PAGE_KEYS = Object.keys(PAGE_PATHS) as (keyof typeof PAGE_PATHS)[];

// ── gsc_navigate ──────────────────────────────────────────────

export const gscNavigateSchema = {
  page: z.enum(PAGE_KEYS as [string, ...string[]]).describe('GSC section to open. Property is preserved from current resource_id unless `property` is set.'),
...
export const gscNavigateDescription =
  'Open a Google Search Console page (overview, performance, inspect, pages, sitemaps, removals, links, security, settings, etc.). Always call before any gsc_* tool to land on the right report. ' +
  `Pages: ${PAGE_KEYS.join(', ')}.`;

export async function handleGscNavigate(args: { page: string; property?: string; sessionId?: string; tabId?: string }) {
  const r = await resolve(args.sessionId, args.tabId);
  if ('error' in r) return text(r.error);

  if (!(args.page in PAGE_PATHS)) return text(`Unknown page: ${args.page}`);
  ...
  await pollFor(
    r,
    `(() => { const b = document.querySelector('[aria-busy="true"]'); return !b ? location.href : null; })()`,
    8000,
    300,
  );
  await wait(800);
...
export const gscPropertySchema = {
  action: z.enum(['list', 'select', 'current', 'add'] as const).describe(
    'list = enumerate properties from picker; select = switch active property; current = read URL property; add = open Add Property dialog.',
  ),
  ...
  type: z.enum(['domain', 'url_prefix'] as const).optional().describe('For add action: property type. Default: domain.'),
  sessionId: z.string().optional(),
  tabId,
};

export const gscPropertyDescription =
  'Manage GSC property selection — list registered properties, switch the active one (re-navigates current page), read the active property from the URL, or open the Add Property dialog. ' +
  '[mutating for action=add — adds a new property to the Google account]';

export async function handleGscProperty(args: { action: string; property?: string; type?: string; sessionId?: string; tabId?: string }) {
  const r = await resolve(args.sessionId, args.tabId);
  if ('error' in r) return text(r.error);
  ...
  if (args.action === 'list') {
    ...
    const open = await ni.evaluate(
      r.sessionId,
      `(() => { const btn = document.querySelector('[aria-label*="property" i], [aria-label*="propriedade" i], [aria-label*="Pesquise" i]'); if (btn) { btn.click(); return true; } return false; })()`,
      r.tabId,
    );
    if (open.error || !open.result) return text('Could not open property picker. Navigate to GSC first.');
    await wait(800);
    const list = await ni.evaluate(
      r.sessionId,
      `(() => {
    ...
    const props = (list.result as Array<{ property: string; label: string }>) || [];
    if (props.length === 0) return text('No properties found in picker.');
    return text(`${props.length} propert${props.length === 1 ? 'y' : 'ies'}:\n${JSON.stringify(props, null, 2)}`);
  }

  if (args.action === 'select') {
    if (!args.property) return text('property is required for action=select.');
    ...
    if (nav.error) return text(`Switch failed: ${nav.error}`);
    await wait(1500);
    return text(`Switched active property to ${args.property}.`);
  }

  if (args.action === 'add') {
    if (!args.property) return text('property is required for action=add (domain or URL prefix value).');
    const open = await ni.evaluate(
      r.sessionId,
      `(() => {
    ...
    if (!open.result) return text('Could not find Add Property button. Open the property picker first.');
    await wait(1000);
    const fieldType = args.type === 'url_prefix' ? 'url' : 'domain';
    const fill = await ni.evaluate(
      r.sessionId,
      `(() => {
    ...
    if (!fill.result) return text('Could not fill property field.');
    await wait(400);
    await ni.evaluate(
      r.sessionId,
      `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const cont = btns.find(b => /continue|continuar|verify|verificar/i.test(b.textContent || '')); if (cont) cont.click(); })()`,
      r.tabId,
    );
