...
export const gscPagesReportDescription =
  'Read the Pages (Coverage/Indexação) report — totals indexed/notIndexed and per-reason buckets with counts and trends. ' +
  'Pass `reason` to drill into a bucket and list URL examples.';

export async function handleGscPagesReport(args: { reason?: string; property?: string; sessionId?: string; tabId?: string }) {
  ...
  if (!prop) return text('No active property.');
  ...
  const metrics = {
    result: {
      indexed: (indexedRes.result as string | null) ?? null,
      notIndexed: (notIndexedRes.result as string | null) ?? null,
    },
  };

  const tableRes = await ni.evaluate(r.sessionId, TABLE_EXTRACTOR_SCRIPT('[role="grid"], [role="table"], table'), r.tabId);
  const reasons = (tableRes.result || { rows: [] }) as { rows: Array<Record<string, unknown>> };
  ...
  if (args.reason) {
    ...
    if (!click.result) return text(`Reason not found: "${args.reason}". Available reasons:\n${JSON.stringify(reasons.rows, null, 2)}`);
...
export const gscPagesValidateFixDescription =
  '[mutating — starts a Google validation cycle] Open a Pages-report reason bucket and click "Validate fix" so Google re-checks the URLs. Returns confirmation status.';

export async function handleGscPagesValidateFix(args: { reason: string; property?: string; sessionId?: string; tabId?: string }) {
  const r = await resolve(args.sessionId, args.tabId);
  if ('error' in r) return text(r.error);
  ...
  if (!click.result) return text('"Validate fix" button not found — reason may not support validation, or all issues already fixed.');
...
export const gscVideosReportDescription =
  'Read the Video indexing report — same shape as gsc_pages_report but for video pages.';

export async function handleGscVideosReport(args: { property?: string; sessionId?: string; tabId?: string }) {
  const r = await resolve(args.sessionId, args.tabId);
  if ('error' in r) return text(r.error);
  const prop = args.property || (await currentProperty(r));
  if (!prop) return text('No active property.');
  ...
  await pollFor(
    r,
    `(() => (document.querySelector('[role="grid"], [role="table"]') || /no data|nenhum dado|no video pages|nenhuma página de vídeo/i.test(document.body.innerText)) ? true : null)()`,
    20000,
    500,
  );
  await wait(800);
...
export const gscSitemapSchema = {
  action: z.enum(['list', 'submit', 'delete', 'view_errors'] as const),
...
export const gscSitemapDescription =
  'Manage sitemaps. action=list returns all submitted sitemaps; action=submit adds a sitemap (mutating); ' +
  'action=delete removes one (mutating); action=view_errors opens the per-sitemap detail and returns issues.';

export async function handleGscSitemap(args: { action: string; url?: string; property?: string; sessionId?: string; tabId?: string }) {
  const r = await resolve(args.sessionId, args.tabId);
  if ('error' in r) return text(r.error);
  const prop = args.property || (await currentProperty(r));
  if (!prop) return text('No active property.');
  ...
  if (args.action === 'submit') {
    if (!args.url) return text('url required for submit.');
    await ni.evaluate(
      r.sessionId,
      `(() => { const i = document.querySelector('input[type="text"], input:not([type])'); if (i) { i.focus(); i.value = ${JSON.stringify(args.url)}; i.dispatchEvent(new Event('input', { bubbles: true })); } })()`,
      r.tabId,
    );
    await wait(300);
    await ni.evaluate(
      r.sessionId,
      `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const sub = btns.find(b => /submit|enviar/i.test(b.textContent || '')); if (sub) sub.click(); })()`,
      r.tabId,
    );
    const result = await pollFor<Record<string, unknown>>(
      r,
  ...
  if (args.action === 'delete') {
    if (!args.url) return text('url required for delete.');
    const click = await ni.evaluate(
      r.sessionId,
      `(() => {
        const rows = Array.from(document.querySelectorAll('[role="row"], tr'));
    ...
    if (!click.result) return text(`Sitemap not found: ${args.url}`);
    await wait(1500);
    await ni.evaluate(
      r.sessionId,
      `(() => {
  ...
  if (args.action === 'view_errors') {
    if (!args.url) return text('url required for view_errors.');
    await ni.evaluate(
      r.sessionId,
      `(() => {
        const rows = Array.from(document.querySelectorAll('[role="row"], tr'));
...
export const gscRemovalsSchema = {
  action: z.enum(['list_temporary', 'list_outdated', 'list_safesearch', 'new'] as const),
  removalType: z.enum(['temporary', 'outdated_content', 'safe_search'] as const).optional().describe('Required for action=new.'),
  target: z.string().optional().describe('URL or prefix for action=new.'),
  scope: z.enum(['url', 'prefix'] as const).optional().describe('For action=new + temporary: remove only this URL or all URLs starting with this prefix.'),
  property: z.string().optional(),
  sessionId: z.string().optional(),
  tabId,
};

export const gscRemovalsDescription =
  'Manage URL removals. list_* returns existing requests; ' +
...
export async function handleGscRemovals(args: {
  action: string; removalType?: string; target?: string; scope?: string;
  property?: string; sessionId?: string; tabId?: string;
}) {
  const r = await resolve(args.sessionId, args.tabId);
  if ('error' in r) return text(r.error);
  const prop = args.property || (await currentProperty(r));
  ...
  if (args.action.startsWith('list_')) {
    ...
    await ni.evaluate(
      r.sessionId,
      `(() => { const t = Array.from(document.querySelectorAll('[role="tab"]')).find(t => ${re.toString()}.test(t.textContent || '')); if (t) t.click(); })()`,
      r.tabId,
    );
    await wait(1500);
    const tableRes = await ni.evaluate(r.sessionId, TABLE_EXTRACTOR_SCRIPT('[role="grid"], [role="table"], table'), r.tabId);
    ...
    t.rows = t.rows.filter(row => {
      ...
      if (keys.length === 1 && row.url === 'https://search.google.com/#') return false;
      return true;
    });
    if (t.rows.length === 0) {
  ...
  if (args.action === 'new') {
    if (!args.removalType || !args.target) return text('removalType and target required for action=new.');
    await ni.evaluate(
      r.sessionId,
      `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const n = btns.find(b => /new\\s*request|nova\\s*solicitação/i.test(b.textContent || '')); if (n) n.click(); })()`,
      r.tabId,
    );
    await wait(1200);
    // Click the right type tab inside the dialog
    const typeRe: Record<string, RegExp> = {
    ...
    await ni.evaluate(
      r.sessionId,
      `(() => { const tabs = Array.from(document.querySelectorAll('[role="tab"], button, [role="button"]')); const t = tabs.find(t => ${tre.toString()}.test(t.textContent || '')); if (t) t.click(); })()`,
      r.tabId,
    );
    await wait(700);
    // Fill URL input
    await ni.evaluate(
      r.sessionId,
      `(() => { const i = document.querySelector('input[type="text"], input[type="url"]'); if (i) { i.focus(); i.value = ${JSON.stringify(args.target)}; i.dispatchEvent(new Event('input', { bubbles: true })); } })()`,
      r.tabId,
    );
    await wait(300);
    ...
    if (args.removalType === 'temporary' && args.scope === 'prefix') {
      await ni.evaluate(
        r.sessionId,
        `(() => { const radios = Array.from(document.querySelectorAll('[role="radio"], input[type="radio"]')); const p = radios.find(r => /prefix|prefixo|all\\s*URLs/i.test(r.closest('label')?.textContent || r.parentElement?.textContent || '')); if (p) p.click(); })()`,
        r.tabId,
      );
    }
    await ni.evaluate(
      r.sessionId,
      `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const n = btns.find(b => /^(next|próximo|continue)$/i.test((b.textContent || '').trim())); if (n) n.click(); })()`,
      r.tabId,
    );
    await wait(800);
    await ni.evaluate(
      r.sessionId,
      `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const sub = btns.find(b => /^(submit\\s*request|submit|enviar)$/i.test((b.textContent || '').trim())); if (sub) sub.click(); })()`,
      r.tabId,
    );
    const result = await pollFor<Record<string, unknown>>(
      r,
...
export const gscRemovalsCancelDescription =
  '[mutating] Cancel a pending temporary removal request. URL must match the listed entry exactly.';

export async function handleGscRemovalsCancel(args: { url: string; property?: string; sessionId?: string; tabId?: string }) {
  const r = await resolve(args.sessionId, args.tabId);
  if ('error' in r) return text(r.error);
  ...
  if (!click.result) return text(`Removal request not found: ${args.url}`);
  await wait(500);
  await ni.evaluate(
    r.sessionId,
    `(() => { const items = Array.from(document.querySelectorAll('[role="menuitem"], button')); const c = items.find(i => /cancel|cancelar/i.test(i.textContent || '')); if (c) c.click(); })()`,
    r.tabId,
  );
  await wait(800);
  await ni.evaluate(
    r.sessionId,
    `(() => { const btns = Array.from(document.querySelectorAll('button, [role="button"]')); const conf = btns.find(b => /^(cancel\\s*request|cancel|cancelar)$/i.test((b.textContent || '').trim())); if (conf) conf.click(); })()`,
    r.tabId,
  );
