...
export function encodeProperty(property: string): string {
 ...
 */
export async function currentProperty(r: Resolved): Promise<string | null> {
  const res = await ni.evaluate(
    r.sessionId,
    `(() => { const u = new URL(location.href); return u.searchParams.get('resource_id'); })()`,
    r.tabId,
  );
 ...
 */
export async function gscUrl(r: Resolved, path: string, property?: string): Promise<string> {
...
export function metricRegex(key: keyof typeof LOCALE_LABELS | string): RegExp {
 ...
 */
export function readMetricTileScript(key: keyof typeof LOCALE_LABELS | string): string {
  const list = LOCALE_LABELS[key] || [];
  const alt = [...list].sort((a, b) => b.length - a.length).map(escapeRegex).join('|');
  if (!alt) return `(() => null)()`;
  return `(() => {
 ...
 */
export const TABLE_EXTRACTOR_SCRIPT = (scope: string) =>
  `(() => {
 ...
 */
export async function safeNavigate(r: Resolved, url: string): Promise<{ error?: string }> {
...
export async function clickSidebarIcon(r: Resolved, iconText: string): Promise<{ ok: boolean; error?: string }> {
  const res = await ni.evaluate(
    r.sessionId,
    `(() => {
