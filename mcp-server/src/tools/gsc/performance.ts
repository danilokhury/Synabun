...
const RANGE_SLUGS: Record<string, string> = {
  '24h': '1',
  '7d': '7',
  '28d': '28',
  '3m': '90',
...
const DIMENSIONS = ['query', 'page', 'country', 'device', 'searchAppearance', 'date'] as const;
...
export const gscPerformanceQuerySchema = {
  ...
  dateRange: z.enum(['24h', '7d', '28d', '3m', '6m', '12m', '16m', 'custom'] as const).optional().describe('Preset range or "custom" with customStart/customEnd.'),
...
export const gscPerformanceQueryDescription =
  'Run a Performance report query — sets searchType, dateRange (preset or custom), primary dimension, chip filters, and optional comparison; ' +
...
export async function handleGscPerformanceQuery(args: {
  searchType?: string; dateRange?: string; customStart?: string; customEnd?: string;
  dimension?: string; filters?: Array<{ type: string; op?: string; value: string }>;
  compare?: boolean; limit?: number; property?: string; sessionId?: string; tabId?: string;
}) {
  ...
  if (!prop) return text('No active property. Call gsc_navigate first.');
  ...
  if (searchType === 'discover') params.set('page_type', 'discover');
  else if (searchType === 'googleNews' || searchType === 'news') params.set('page_type', 'googleNews');

  if (args.dateRange === 'custom') {
    if (!args.customStart || !args.customEnd) return text('customStart and customEnd required for dateRange=custom.');
  ...
  await pollFor(
    r,
    `(() => document.querySelector('[role="grid"], [role="table"]') ? true : null)()`,
    20000,
    500,
  );
  await wait(800);
  ...
  for (let i = 0; i < 12; i++) {
    const count = await ni.evaluate(
      r.sessionId,
      `(() => document.querySelectorAll('[role="grid"] [role="row"], [role="table"] tr').length)()`,
      r.tabId,
    );
  ...
  const totals = {
    totalClicks: (tcRes.result as string | null) ?? null,
    totalImpressions: (tiRes.result as string | null) ?? null,
    avgCTR: (ctrRes.result as string | null) ?? null,
    avgPosition: (apRes.result as string | null) ?? null,
  };

  const tableRes = await ni.evaluate(r.sessionId, TABLE_EXTRACTOR_SCRIPT('[role="grid"], [role="table"], table'), r.tabId);
  if (tableRes.error) return text(`Table extract failed: ${tableRes.error}`);
  const table = (tableRes.result || { headers: [], rows: [] }) as { headers: string[]; rows: Record<string, unknown>[] };
...
export const gscPerformanceExportSchema = {
  format: z.enum(['csv', 'excel', 'google_sheets'] as const).describe('Export format.'),
  sessionId: z.string().optional(),
  tabId,
};

export const gscPerformanceExportDescription =
  'Trigger Performance report export menu in the active tab. Format: csv → ZIP download; excel → XLSX; google_sheets → opens a new Sheet. ' +
  'Browser handles the actual download to its default download dir.';

export async function handleGscPerformanceExport(args: { format: string; sessionId?: string; tabId?: string }) {
  const r = await resolve(args.sessionId, args.tabId);
  if ('error' in r) return text(r.error);
  ...
  if (!click.result) return text(`Could not find ${args.format} option in Export menu.`);
  await wait(2000);
...
export const gscPerformanceChartScreenshotDescription =
  'Capture a screenshot scoped to the Performance chart panel only — useful for visual reports.';

export async function handleGscPerformanceChartScreenshot(args: { sessionId?: string; tabId?: string }) {
  const r = await resolve(args.sessionId, args.tabId);
  if ('error' in r) return text(r.error);
