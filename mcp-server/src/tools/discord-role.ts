...
export const discordRoleDescription =
  'Discord role management. Actions: create, edit, delete, list, assign (role to member), remove (role from member).';
...
async function handleCreate(guildId: string, args: {
  ...
  if (args.hoist !== undefined) data.hoist = args.hoist;
  if (args.mentionable !== undefined) data.mentionable = args.mentionable;
  if (args.permissions) {
    try {
...
async function handleEdit(guildId: string, args: {
  role?: string; name?: string; color?: string; hoist?: boolean;
