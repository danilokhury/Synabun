...
import { searchMemories, searchMemoriesFTS, touchMemories, searchSessionChunks } from '../services/sqlite.js';
...
export async function handleRecall(args: {
  ...
  // Update access tracking — one batched UPDATE (fire-and-forget)
  touchMemories(scored.map((r) => r.id), new Date().toISOString()).catch(() => {});
