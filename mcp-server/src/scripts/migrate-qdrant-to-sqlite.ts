...
function getQdrantConfig(env: Record<string, string>): QdrantConfig {
...
async function qdrantFetch(cfg: QdrantConfig, endpoint: string, options: RequestInit = {}): Promise<any> {
...
async function scrollAll(cfg: QdrantConfig, collection: string, withVector = false): Promise<QdrantPoint[]> {
...
async function initEmbeddings(): Promise<void> {
  ...
  embeddingPipeline = await (pipeline as any)('feature-extraction', 'Xenova/all-MiniLM-L6-v2', {
    dtype: 'fp32',
  });
...
function jsonStr(val: any): string | null {
...
async function main(): Promise<void> {
  ...
  const existingCount = (db.prepare('SELECT COUNT(*) as cnt FROM memories').get() as any).cnt;
  if (existingCount > 0) {
  ...
  if (catPoint?.payload?.categories) {
    const cats: any[] = catPoint.payload.categories;
    console.log(`  Found ${cats.length} categories in system metadata`);
    db.exec('BEGIN');
    try {
      const catStmt = db.prepare(
        'INSERT OR REPLACE INTO categories (name, description, created_at, parent, color, is_parent) VALUES (?, ?, ?, ?, ?, ?)'
      );
    ...
    try {
      for (let j = 0; j < batch.length; j++) {
        ...
        memStmt.run(
          ...
          jsonStr(p.tags) || '[]',
          p.importance ?? 5,
          p.source || 'self-discovered',
          p.created_at || now,
          p.updated_at || now,
          p.accessed_at || now,
          p.access_count ?? 0,
          jsonStr(p.related_files),
          jsonStr(p.related_memory_ids),
          jsonStr(p.file_checksums),
          p.trashed_at || null,
          jsonStr(p.source_session_chunks),
        );
        migrated++;
      }
      db.exec('COMMIT');
      ...
      try {
        for (let j = 0; j < batch.length; j++) {
          ...
          scStmt.run(
            ...
            p.cwd || null,
            p.chunk_index ?? 0,
            p.start_timestamp || null,
            p.end_timestamp || null,
            jsonStr(p.tools_used) || '[]',
            jsonStr(p.files_modified) || '[]',
            jsonStr(p.files_read) || '[]',
            jsonStr(p.user_messages) || '[]',
            p.turn_count ?? 0,
            jsonStr(p.related_memory_ids) || '[]',
            p.dedup_memory_id || null,
            p.indexed_at || null,
          );
          scMigrated++;
        }
  ...
  // Summary
  const finalMem = (db.prepare('SELECT COUNT(*) as cnt FROM memories').get() as any).cnt;
  const finalSc = (db.prepare('SELECT COUNT(*) as cnt FROM session_chunks').get() as any).cnt;
  const finalCat = (db.prepare('SELECT COUNT(*) as cnt FROM categories').get() as any).cnt;
