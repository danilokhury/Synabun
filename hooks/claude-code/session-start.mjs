...
import { getHookFeatures, detectProject, ensureProjectCategories, cleanupStaleLoops, cleanupStalePendingRemember, appendCapped, DATA_DIR } from './shared.mjs';
...
function healSynaBunMcp() {
  try {
    ...
    try {
      appendCapped(join(DATA_DIR, 'compact-debug.log'), `[${new Date().toISOString()}] MCP-HEAL re-registered SynaBun in ~/.claude.json\n`);
    } catch { /* ok */ }
  } catch { /* non-critical */ }
}
...
async function main() {
  ...
  try {
    ...
    appendCapped(DEBUG_LOG, `[${new Date().toISOString()}] SESSION-START source=${source} session_id=${sessionId} isCompactRestart=${isCompactRestart} cacheExists=${cacheExists} convMemory=${features.conversationMemory}\n`);
  } catch { /* ok */ }
  ...
  // Sweep pending-remember flags from long-dead sessions (never deleted otherwise).
  cleanupStalePendingRemember();
  const LOOP_STALE_MS = 10 * 60 * 1000;
  const terminalSessionEnv = process.env.SYNABUN_TERMINAL_SESSION || '';
  let isLoopSession = false;
  let isNativeLoopSession = false;
  try {
      ...
      for (const f of loopFiles) {
        try {
          ...
          if (data.active || data.pending) {
            ...
            isLoopSession = true;
            isNativeLoopSession = data.driverType === 'native';
            break;
          }
        } catch { /* skip */ }
      }
  ...
  if (isLoopSession) {
    ...
    context.push(
      ...
      ``,
      isNativeLoopSession
        ? `**This is a native autonomous loop session.** Do NOT output any greeting or call recall at startup. The server sends the complete iteration task directly; execute it immediately.`
  ...
  if (isCompactRestart) {
    context.push(
      ...
      ``,
      `**Plan files**: Write plan files to \`data/plans/YYYY-MM-DD/your-plan-slug.md\` (e.g., \`data/plans/2026-04-06/fix-session-crosstalk.md\`). Do NOT create \`PLAN.md\` at the project root. Do NOT write to \`~/.claude/plans/\`. Use a short descriptive slug from the plan title.`,
      ``,
      `---`,
      ``,
    );
