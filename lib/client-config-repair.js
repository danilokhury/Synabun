...
export function auditAndRepairClientConfigs({
  ...
  if (codexHealth.connected) {
    ...
    if (apply) {
      ...
      upsertCodexMcpConfig(codexPath, codexDefinition);
      changed = before !== readFileSync(codexPath, 'utf-8');
    } else {
      const content = readFileSync(codexPath, 'utf-8');
      changed = !codexDefinition.args.every(value => content.includes(value))
        || !Object.values(codexDefinition.env).every(value => content.includes(value));
    }
